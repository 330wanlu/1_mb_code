#define _base64_c
#include "_base64.h"
unsigned short base64_enc(unsigned char *dest, const unsigned char *src, signed short count)
{
...
unsigned short base64_dec(unsigned char* dest, unsigned char* src, signed short count)
{
	unsigned char ucs[4];
	unsigned char *pt = (unsigned char *)src;
	unsigned short len = 0, nfag = 0, i = 0;
	//    /*count小于0，则自行计算src的长度*/
	//    if( count < 0 )
	//    {
	//        while( *pt++ ){ count++; }
