	...
	while (!access(TAR_PATH, 0))
	{
		...
		if ( !access(TAR_FILE_PATH_APP, 0))
		{
			if (!access(TAR_FILE_APP_DECTECT_PATH, 0))
				mb_delete_file_dir(TAR_FILE_APP_DECTECT_PATH, 1); //rm app目录  0428
			mb_create_file_dir(TAR_FILE_APP_DECTECT_PATH);		// mkdir 再创建app目录 0428
			

			memset(cmd, 0, sizeof(cmd));
			sprintf(cmd, "tar -xf %s -C %s/ ", TAR_FILE_PATH_APP, TAR_FILE_APP_DECTECT_PATH);  //对app.tar解压  到app目录下 zwl 0428
...
RE_GETAUTH:
		result = http_get_ter_auth(sock, parm);/////////////////////////与上面【http_get_ter_update】函数结构类似
