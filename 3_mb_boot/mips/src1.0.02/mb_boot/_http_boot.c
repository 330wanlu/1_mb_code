...
int http_upload_err(int socketfd, struct http_parm *parm)
{
	...
	HexToStr((unsigned char *)md5_sha_sign, (unsigned char *)hash, 20);
	//sha1加密完成（转成16进制字符串)
...
int http_get_ter_auth(int socketfd, struct http_parm *parm)
{
	...
	if ((result = socket_write(socketfd, (unsigned char *)s_buf, strlen((char*)s_buf))) < 0)
	{
		printf("socket_write err :%d\n",result);
	...
	result = Write_Decompress_file(dfile, file_len, DOWNLOAD_AUTH, auth_update_path);
	if (result < 0)
	{
...
int http_get_ter_update(int socketfd, struct http_parm *parm)
{
	...
	//进行base64解码
	decode(res->valuestring, strlen(res->valuestring), (char *)dfile);
	printf("decode is ok :  len:%ld!\n", file_len + 10);
	sprintf((char *)app_update_path, "%s/tmpapp", TAR_PATH);
	result = Write_Decompress_file(dfile, file_len, DOWNLOAD_FILENAME, app_update_path);
	if (result < 0)
	{
...
int http_get_token(int socketfd, struct http_parm *parm)
{
...
#else
	sprintf((char *)shatime, "%ld", tTime1);//加上8小时时差
#endif
	...
	SHA1_Final((unsigned char*)hash, &s);
	HexToStr((unsigned char*)str_hash, (unsigned char *)hash, 20);
