...
{   
	...
	system("echo 1 > /proc/sys/vm/overcommit_memory &");//防止MTK设备升级的时候受到动态内存空间的影响导致升级失败
	printf("BOOT 程序启动：版本:1.1.04  发布时间：2020-03-25\n");
#if 0
	...
	system("rm -rf /home/share/exe/tar/app");
	write_log("boot start\n");
	result = pthread_create(&thread_main_app, NULL, main_app_thread, &parm);
	if (result != 0)
	{
		printf("create thread thread_main_app err!\n");
	...
	while (!access(TAR_PATH, 0))
	{
		...
		if ( !access(TAR_FILE_PATH_APP, 0))
		{
			...
			//system(cmd);
			mb_system_read(cmd, buf_read, sizeof(buf_read),"/tmp/system_cmd_app.log");
			...
			result = parse_config_copy_file(config_path, TAR_FILE_APP_DECTECT_PATH,CHECK);
		...
		if (!access(TAR_FILE_PATH_AUTH, 0))
		{
			...
			mb_system_read(cmd, buf_read, sizeof(buf_read),"/tmp/system_cmd_auth.log");
			//system(cmd);
			...
			result = parse_config_copy_file(config_path, TAR_FILE_AUTH_DECTECT_PATH, CHECK);
			if (result < 0)//解析比较tar包与环境中的文件出现错误异常--未替换（以原有的程序执行），上报一下信息?
			{
...
void *main_app_thread(void *arg)
{
	...
	while (1)
	{
		...
		{
			printf("主程序运行失败\n");
			system("cp /home/share/exe/tar/tmpapp/mb_usbshare_n /home/share/exe/.");
			system("chmod 777  /home/share/exe/mb_usbshare_n");
		}
		sleep(10);
	}
}
...
void *update_thread(void *arg)
{
	...
	while (1)
	{
		...
		result = get_ter_id_ver((char *)TER_ID_INFO_PATH, (char *)parm->ter_id, (char *)parm->app_ver, hostname);
		if (result < 0)
		{
		...
		if (get_real_host(hostname, real_host) < 0)
		{
			printf("主域名，备份域名均无效!\n");
			sleep(60);
			continue;
		...
		result = http_socket_open(&sock, ip);
		if (result <0)
		{
...
RE_GET_AUTH:
		...
		if (result < 0)
		{
			if (result == BOOT_ERROR_HTTP_SEND)
			{
				shutdown(sock, SHUT_RDWR);
				close(sock);
				write_log("reget auth !\n");
				result = http_socket_open(&sock, ip);
				if (result <0)
				{
					printf("socket create failed\n");//socket 创建失败直接返回
			...
			write_log("http get ter auth err!\n");
		...
		if (strlen((char *)update_msg) != 0)
		{
			...
			if (result < 0)
			{
				write_log("err write_update_record:%d\n", result);
			}
		}
...
int dectec_app(char *errinfo)
{
	...
	{
		result = app_file_get_new_detect(soft_d[i], &file_note);
		if (result < 0)
			continue;
		else
		{
	...
	if (flag_found_app == 1)
	{
		mb_chmod_file_path("777", "/tmp/mb_usbshare_detect_n_d");
		
		result = app_file_detect("/tmp/mb_usbshare_detect_n", errinfo);
		//result = app_file_detect(soft_d[i], errinfo);

		//mb_delete_file_dir("/tmp/mb_usbshare_detect_n_d", 0);
		if (result < 0)
			return result;
		return 0;
	}
...
int run_main_app()
{
	...
	struct _app_file_note   file_note;
	mb_chmod_file_path("777", BOOT_APP_PATH);
	write_log("boot detect main app file\n");
	for (i = 0; i < sizeof(soft_d) / sizeof(const char *); i++)
	{
	...
	if (flag_found_app == 1)
	{
		...
		write_log("boot start main app\n");
		mb_chmod_file_path("777", BOOT_APP_PATH);
		//result = app_file_oper(soft_d[i]);
		result = app_file_oper("/tmp/mb_usbshare_n");
		if (result < 0)//程序未正常运行，标识位置为0
		{
			return -1;
		}
		write_log("boot finish start main app\n");
		return 0;
	}
