...
extern char update_msg[1024];
extern char auth_msg[1024];
...
-   Other :
***********************************************************************/
int Write_Decompress_file(char *file, long file_len, char *filename, char * Dest_dir)
{
	int file_fd, result;
	...
	printf("/tmp/app.tar chmod 777 ok!\n");
	mb_tar_decompress((char *)filename, (char *)Dest_dir);

	printf("/tmp/app.tar decompress to /home/share/exe/tar/tmpapp ok!\n");
	close(file_fd);
	sleep(1);//等待解压完成
	//进行解压文件的校验和复制到当前系统
	sprintf((char*)config_path, "%s/mb_config.cnf", Dest_dir);
	result = parse_config_copy_file(config_path, Dest_dir,UPDATE);
	if (result < 0)
		return result;
...
int parse_config_del_backfile(char *configname, char *now_path)
{
	...
	while (fgets(lineBuff, 256, file) != NULL)
	{
		...
		{
			printf("err format configfile Path1 not found 1:%s!\n", file_real_path);
			fclose(file);
...
int parse_config_back_oldver(char *configname, char *now_path)
{
	...
	//	printf("back path = %s\n", file_real_path);
		if (access(file_real_path, 0) != 0)
		{
			printf("err format configfile Path1 not found 2:%s!\n", file_real_path);
			fclose(file);
			return BOOT_ERROR_OPER_PARSE_CONFIG_ERR_PATH_NOT_FOUND;
		}
		//return -4;
...
int parse_config_copy_file(char *configname,char *now_path,int update_or_check)
{
	FILE *file;
	int result = 0;
	char lineBuff[256] = { 0 };// 存储读取一行的数据
	...
	while (fgets(lineBuff, 256, file) != NULL)
	{
		...
		if (access(file_real_path, 0) != 0)
		{
			printf("err format configfile Path1 not found 3:%s!\n", file_real_path);
			fclose(file);
			return BOOT_ERROR_OPER_PARSE_CONFIG_ERR_PATH_NOT_FOUND;
		}
		if (access(tar_file_path, 0) != 0)
		{
			printf("err forrmat configfilePathh1 not found! :%s\n", tar_file_path);
			fclose(file);
			return BOOT_ERROR_OPER_PARSE_CONFIG_ERR_FILE_NOT_FOUND;
		}
		memset(real_file_path, 0, sizeof(real_file_path));
...
int get_ter_id_ver(char *filename, char *ter_id,char *ver,char *hostname)
{
	FILE *file;
	char lineBuff[256] = { 0 };
	int find_flag = 0;
...
int mb_system_read(char *cmd, char *r_buf, int r_buf_len,char *log_path)
{
	char order[200];
	//char tmp_file[100];
	int fd;
	...
	sprintf(order, "%s  > %s 2>&1", cmd, log_path);
	system(order);
...
int write_log(const char *format, ...)
{
	...
	return 0;
#else
	FILE* pFile = fopen("/etc/boot.log", "a");
	va_start(arg, format);
	memset(buf1, 0, sizeof(buf1));
	vsprintf(buf1, format, arg);

	char time_asc[200] = { 0 };
	timer_read_asc(time_asc);
