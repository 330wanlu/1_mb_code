...
static void analysis_request_qddengl(void *arg)
{
	...
	char related_type[32];		//登录员身份
	char full_name[32];			//登录员名称
	char sz_password[32];		//登录员密码
	char mobile[32];			//登录员手机号
	char busid[32];
	char inbusid[32];
	char idcard[32];
	char sys_type[32]; 			 //场景类型,目前江苏需要区分进入页面类型
	...
	if(plate_train == 1){				//测试盘不允许
		...
		result = mbi_etax_login_Interface(usb_port,inbusid,related_type,full_name,mobile,idcard,sz_password,errinfo,outgoto,outcode,outtoken,sys_type);	
		if(result == 0){
			cJSON_AddStringToObject(head_layer, "result", "s");
			cJSON_AddItemToObject(json, "data", data_array = cJSON_CreateArray());
			cJSON *data_layer;
...
static void analysis_request_qdqiyeqiehuan(void *arg)
{
	...
	char inbusid[32];
	char idcard[32];
	char uniqueIdentity[32];	//切换的企业税号
	char realationStatus[32];	//切换的企业人身份
	char sys_type[32];
...
static void analysis_request_qdfpcx(void *arg)
{
	...
	char uniqueIdentity[32];		//需要切换企业的税号
	char realationStatus[32];		//需要切换企业的登陆身份		
	char plate_num[32];				//盘号
	char related_type[32];			//登录员身份
	char full_name[32];				//登录员名称
	char sz_password[32];			//登录员密码
	char mobile[32];				//登录员手机号
	...
	char tmp_cookie[600*1024] = {0x00};
	char etax_cookie[600*1024] = {0x00};
	char new_etax_cookie[600*1024] = {0x00};
	char tpass_cookie[600*1024] = {0x00};
	char new_tpass_cookie[600*1024] = {0x00};
	char dppt_cookie[600*1024] = {0x00};
	char new_dppt_cookie[600*1024] = {0x00};
...
static void analysis_request_qd_get_cookie(void *arg)
{
	...
	{
		result = mbi_etax_get_cookie_Interface(usb_port,inbusid,related_type,mobile,idcard,sz_password,realationtype,uniqueIdentity,realationStatus,errinfo,&etax_cookie,&tpass_cookie,&dppt_cookie);		
		if(result == 0){
			cJSON_AddStringToObject(head_layer, "result", "s");
			cJSON_AddItemToObject(json, "data", data_array = cJSON_CreateArray());
			cJSON *data_layer;
			cJSON_AddItemToObject(data_array, "dira", data_layer = cJSON_CreateObject());
			/*memset(tmp_cookie,0x00,sizeof(tmp_cookie));
			if(strlen(etax_cookie))
			{
				len = sizeof(tmp_cookie);
				strcpy(tmp_cookie,etax_cookie);
			}
			cJSON_AddStringToObject(data_layer, "etax_cookie", tmp_cookie);
			memset(tmp_cookie,0x00,sizeof(tmp_cookie));
			if(strlen(tpass_cookie))
			{
				len = sizeof(tmp_cookie);
				strcpy(tmp_cookie,tpass_cookie);
			}
			cJSON_AddStringToObject(data_layer, "tpass_cookie", tmp_cookie);
			memset(tmp_cookie,0x00,sizeof(tmp_cookie));
			if(strlen(dppt_cookie))
			{
				len = sizeof(tmp_cookie);
				strcpy(tmp_cookie,dppt_cookie);
			}*/
			if(etax_cookie != NULL)
