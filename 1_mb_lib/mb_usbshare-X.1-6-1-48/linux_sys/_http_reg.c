...
int http_get_appid_secert_token(struct _http_comon *parm)
{
	...
	if (result < 0)
	{
		...
		out("获取token失败,http post error,result = %d\n",result);
		return -1;
	}
	//out("token获取到的是数据为%s\n", returndata);
...
int http_token_get_mqtt_connect_md5(struct _http_comon *parm, char *s_data, int s_len,char **r_data,int *r_len)
{
	...
	{
		printf("%s\n", returndata);
		out("http_token_get_mqtt_connect_md5 :json err2\n");
		cJSON_Delete(root);
		return -4;
	}
	memcpy(res_result, item->valuestring, strlen(item->valuestring));
	if (strcmp(res_result, "s") != 0)
	{
		...
		if (get_json_value_can_not_null(item, err, 1, sizeof(err)) < 0)
		{
			out("http_token_get_mqtt_connect_md5:json err3\n");
			cJSON_Delete(root);
			return -5;
		}
		item = cJSON_GetObjectItem(object, "msg");
		if (get_json_value_can_not_null(item, msg, 1, sizeof(msg)) < 0)
		{
			out("http_token_get_mqtt_connect_md5:json err4\n");
			cJSON_Delete(root);
			return -6;
		}
		item = cJSON_GetObjectItem(object, "errinfo");
		if (get_json_value_can_not_null(item, errinfo, 1, sizeof(errinfo)) < 0)
		{
			out("http_token_get_mqtt_connect_md5:json err5\n");
			cJSON_Delete(root);
			return -7;
		}
		//out(" 服务器应答错误：\n");
		cJSON_Delete(root);
		out("err = %s,msg = %s,errinfo = %s\n", err, msg, errinfo);
		return -8;
	}
	else
	{
...
int http_token_get_mqtt_connect_info(struct _http_comon *parm, char *s_data, int s_len,char **r_data, int *r_len)
{
	...
	if (item == NULL)
	{
		out("http_token_get_mqtt_connect_info:json err2\n");
		cJSON_Delete(root);
		return -4;
	}
	memcpy(res_result, item->valuestring, strlen(item->valuestring));
	if (strcmp(res_result, "s") != 0)
	{
		...
		if (get_json_value_can_not_null(item, err, 1, sizeof(err)) < 0)
		{
			out("http_token_get_mqtt_connect_info:json err3\n");
			cJSON_Delete(root);
			return -5;
		}
		item = cJSON_GetObjectItem(object, "msg");
		if (get_json_value_can_not_null(item, msg, 1, sizeof(msg)) < 0)
		{
			out("http_token_get_mqtt_connect_info:json err4\n");
			cJSON_Delete(root);
			return -6;
		}
		item = cJSON_GetObjectItem(object, "errinfo");
		if (get_json_value_can_not_null(item, errinfo, 1, sizeof(errinfo)) < 0)
		{
			out("http_token_get_mqtt_connect_info:json err5\n");
			cJSON_Delete(root);
			return -7;
		}
		//out(" 服务器应答错误：\n");
