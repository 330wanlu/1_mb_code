...
//static struct mosq_config cfgp;
...
{
	cfg->qos = 0;//2020年6月修改为0，由应用端应答确认数据是否收到         2022年6月改为不通接口使用不同质量
	cfg->port = 1883;
...
static void pub_disconnect_callback(struct mosquitto *mosq, void *obj, int rc, const mosquitto_property *properties)
{
	...
	struct pub_private_data *pub_data;
	pub_data = (struct pub_private_data *)obj;
	printf("enter pub_disconnect_callback,name %s\n", pub_data->cfgp.name);
	if (rc == 0){
...
int mqtt_pub_message(struct _mqtt_parms  *mqtt_client)
{
	...
	client_config_init_pub(&(mqtt_client->pub_data.cfgp), mqtt_client->server_address, mqtt_client->server_port, "123", "123456", mqtt_client->user_name, mqtt_client->user_passwd, mqtt_client->user_id_pub);
#ifdef WITH_TLS
