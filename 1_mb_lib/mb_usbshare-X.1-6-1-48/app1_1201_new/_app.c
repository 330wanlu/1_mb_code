...
{   
	...
	//result = ty_ctl(stream.machine_fd, DF_MACHINE_INFOR_CM_GET_MQTT_USER, "192.168.0.139", "1883", "liuzhide", "123456");
	printf("11111111111111111111111111111\r\n");
	// strcpy(stream.mqtt_server,"112.124.26.97");
	// stream.mqtt_port = 19983;
#ifndef CA_1201
	strcpy(stream.mqtt_server,"112.124.26.97");
	stream.mqtt_port = 19983;
#else
	...
	#ifndef CA_1201_37
		strcpy(stream.mqtt_server,"192.168.0.139");
		stream.mqtt_port = 1883;
	#else
		strcpy(stream.mqtt_server,"60.204.142.202");
		stream.mqtt_port = 19983;
	#endif

#endif
	strcpy(stream.user_name,"liuzhide");
	strcpy(stream.user_passwd,"123456");
	// strcpy(stream.mqtt_server,"192.168.0.211");
	// stream.mqtt_port = 1883;
	// strcpy(stream.user_name,"user1");
	// strcpy(stream.user_passwd,"123456");

	printf("11111111111111111111111111112\r\n");
	/*printf("***result=[%d]***\n",result);
	if (result < 0)
	{
	...
	//{
		...
		sprintf(stream.topic, "mb_fpkjxt_%s", "000000001234");
		// sprintf(id_sub, "sub%s", stream.user_name);
		// sprintf(id_pub, "pub%s", stream.user_name);
		sprintf(id_sub, "sub%s", "000000001234");
		sprintf(id_pub, "pub%s", "000000001234");
//#ifdef MTK_OPENWRT
		//ty_ctl(stream.ty_usb_fd, DF_TY_USB_CM_PORT_RED_OFF_GREN_ON, 1001);//
//#endif
		printf("00000000000000000000000000000000111111111111111\r\n");
	...
	printf("11111111111111111111111111113\r\n");
	...
	out("【system启动】 终端软件启动完成,机器编号:【000000001234】,版本:【%s】,迭代【%s】,日期:%s\r\n", app_note.ver, app_note.v_num, app_note.r_date);
...
static void analysis_request_qddengl(void *arg)
{
	...
	if(plate_train == 1){				//测试盘不允许
		...
		if (0 == strcmp(sys_type,"2"))
		{
			...
			result = mbi_etax_get_cookie_Interface(1,"1",related_type,mobile,"",sz_password, 0,"","0",errinfo,&etax_cookie,&tpass_cookie,&dppt_cookie,svr_container,soft_cert_ip,soft_cert_port,login_method,area_code,temp_data);	
			if(result == 0){
...
static void analysis_request_qdyongyxxhq(void *arg)
{
	...
	else 
	{
		...
		result = mbi_etax_user_query_Interface(1,"1",errinfo,&outinfo,svr_container,soft_cert_ip,soft_cert_port,login_method,area_code);	
		if(result == 0){
			cJSON_AddStringToObject(head_layer, "result", "s");
			cJSON_AddItemToObject(json, "data", data_array = cJSON_CreateArray());
			cJSON *data_layer;
...
static void analysis_request_qdqiyeqiehuanlbcx(void *arg)
{
	...
	else 
	{
		...
		result = mbi_etax_relationlist_Interface(1,"1",related_type,full_name,mobile,"",sz_password,errinfo,&outenterpriselist,svr_container,soft_cert_ip,soft_cert_port,login_method,area_code);	
		if(result == 0){
			cJSON_AddStringToObject(head_layer, "result", "s");
			cJSON_AddItemToObject(json, "data", data_array = cJSON_CreateArray());
			cJSON *data_layer;
...
// 		result = mbi_etax_relationchange_Interface(1,"1",related_type,full_name,mobile,"",sz_password,errinfo,uniqueIdentity,realationStatus,outgoto,outcode,outtoken,sys_type,svr_container,soft_cert_ip,soft_cert_port,login_method,area_code);	
// 		if(result == 0){
// 			cJSON_AddStringToObject(head_layer, "result", "s");
// 			cJSON_AddItemToObject(json, "data", data_array = cJSON_CreateArray());
// 			cJSON *data_layer;
...
static void analysis_request_qdqiyeqiehuan(void *arg)
{
	...
	else 
	{
		...
		{
			//result = mbi_etax_get_cookie_Interface_tmp(1,"1",related_type,mobile,"",sz_password,realationtype,uniqueIdentity,realationStatus,errinfo,&etax_cookie,&tpass_cookie,&dppt_cookie,svr_container,soft_cert_ip,soft_cert_port,login_method,area_code,temp_data);	
			result = mbi_etax_get_cookie_Interface(1,"1",related_type,mobile,"",sz_password, 1,uniqueIdentity,realationStatus,errinfo,&etax_cookie,&tpass_cookie,&dppt_cookie,svr_container,soft_cert_ip,soft_cert_port,login_method,area_code,temp_data);	
			if(result == 0){
				printf("dppt_cookie	= %s\r\n",dppt_cookie);
				char name_dppt[] = "dzfp-ssotoken";
				char domain_dppt[] = ".chinatax.gov.cn";
...
static void analysis_request_qd_get_cookie(void *arg)
{
	...
	else 
	{
		...
		result = mbi_etax_get_cookie_Interface(1,"1",related_type,mobile,"",sz_password,realationtype,uniqueIdentity,realationStatus,errinfo,&etax_cookie,&tpass_cookie,&dppt_cookie,svr_container,soft_cert_ip,soft_cert_port,login_method,area_code,temp_data);		
		if(result == 0){
			cJSON_AddStringToObject(head_layer, "result", "s");
			cJSON_AddItemToObject(json, "data", data_array = cJSON_CreateArray());
			cJSON *data_layer;
