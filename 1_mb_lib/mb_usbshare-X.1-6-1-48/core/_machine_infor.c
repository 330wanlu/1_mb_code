...
{   struct _machine_infor  *stream;
    ...
    //out("打开machine_infor_open模块\n");
	
    if(stream->state==0)
    {   //out("machine_infor_open第一次打开事件文件\n");
		...
		delete_invalid_file();
#endif
		_task_open("machine_info", process_check_task, stream, 1, NULL);
        stream->lock=_lock_open("machine_infor_open",NULL);
		//out("machine_infor_open创建设备信息文件\n");
	...
	stream->state++;	
    //out("打开machine_infor_open成功:j=%d\n",j);
	return j+1;	       
}
...
static void delete_log_file(struct _machine_infor *stream)
{
	...
	if (result <0)
	{
		//out("日志文件terminal.log不存在，无需判断删除\n");
		return;
	}
...
#endif
	{
		//out("日志文件terminal.log小于1M,无需删除\n");
		return;
	}
	out("需要上传历史terminal.log日志\n");
...
static void delete_tax_log_file(struct _machine_infor *stream)
{
	...
	if (result <0)
	{
		//out("日志文件terminal_tax.log不存在，无需判断删除\n");
		return;
	}
	memset(&pro, 0, sizeof(pro));
	stat(log_file, &pro);
	if (pro.st_size < 1024 * 1024)
	{
		//out("日志文件terminal_tax.log小于1M,无需删除\n");
		return;
	}
	memset(cmd, 0, sizeof(cmd));
...
{
	int wifi_support = 0;
	if (stream->hard_type == 3)//DF_MB_HARDWARE_A20_A33值为3的机型方支持wifi，如wifi不能正常启动视为不支持wifi
	{
...
static int get_mqtt_user_passwd(struct _machine_infor_fd *id, va_list args)
{
	...
	if (result < 0)
	{
		out("没有获取到配置MQTT服务器和端口配置,使用默认配置\n");
...
{   struct _switch_dev_id   	*mac_id;
    ...
    {   int fd;
        ...
        ty_close(fd);
        _queue_add("reset",reset_systerm,id->dev,10); 
    }
    return result;
}
...
{   int reason;
    ...
    ty_close(fd);
    _queue_add("power_off",off_systerm,id->dev,10); 
    return 0; 
}
...
{   int reason;
    ...
    ty_close(fd);
    _queue_add("reset_systerm",reset_systerm,id->dev,10); 
    return 0;   
}
...
int check_mem(char  *men_per)
{
	...
	for (i = 0; i < 10; i++)
	{
		...
		//out("Time %d mem used : %f \n", i + 1, g_mem_used);
		//mem_s[i] = g_mem_used;
		sum += g_mem_used;
	}
	avg = sum / 10;
	//out("获取到平均内存使用率百分比为：%4.2f \n",avg);
...
static int get_ter_information(struct _machine_infor_fd *id, va_list args)
{
	...
	sprintf(ter_info->product_code, "T001");
	sprintf(ter_info->product_name,"开票服务器");
	sprintf(ter_info->product_rename, "MBITKP");
	...
	//sprintf(ter_info->ter_version_detail, "%s", stream->note->ver);

	sprintf(ter_info->ter_type, "%s", stream->note->type);

	fun_deal_get_so_version(ter_info->tax_ver, ter_info->mbc_ver, ter_info->tax_so_load, ter_info->auth_file_load);
...
static int get_mysql_process(struct _machine_infor_fd *id, va_list args)
{
	...
	if (strlen(mysql_process->process_run) == 0)
	{
		strcpy(mysql_process->process_run,"1");
	}
	if (strlen(mysql_process->port) == 0)
	{
		strcpy(mysql_process->port, "3306");
	}
	if (strlen(mysql_process->load_name) == 0)
	{
		strcpy(mysql_process->load_name, "root");
	}
	if (strlen(mysql_process->load_passwd) == 0)
	{
		strcpy(mysql_process->load_passwd, "123456");
	}
	if (strlen(mysql_process->data_path) == 0)
	{
		strcpy(mysql_process->data_path, "/mnt/masstore/mysql");
	}
	return 0;
}
#endif
static int get_extend_space_process(struct _machine_infor_fd *id, va_list args)
{
	...
	if (strlen(extend_space_process->space_size) == 0)
	{
		strcpy(extend_space_process->space_size, "1");
	}
	if (strlen(extend_space_process->mount) == 0)
	{
		strcpy(extend_space_process->mount, "1");
	}
	if (strlen(extend_space_process->mount_dir) == 0)
	{
		strcpy(extend_space_process->mount_dir, "/mnt/masstore");
	}
	return 0;
}
...
static int get_ssh_process(struct _machine_infor_fd *id, va_list args)
{
	...
	if (strlen(ssh_process->process_run) == 0)
	{
		strcpy(ssh_process->process_run, "1");
	}
	if (strlen(ssh_process->port) == 0)
	{
		strcpy(ssh_process->port, "22");
	}
	if (strlen(ssh_process->load_name) == 0)
	{
		strcpy(ssh_process->load_name, "root");
	}
	if (strlen(ssh_process->load_passwd) == 0)
	{
		strcpy(ssh_process->load_passwd, "123456");
	}
	return 0;
}
...
static int get_ntpdate_process(struct _machine_infor_fd *id, va_list args)
{
	...
	if (strlen(ntpdate_process->process_run) == 0)
	{
		strcpy(ntpdate_process->process_run, "1");
	}
	if (strlen(ntpdate_process->server) == 0)
	{
		strcpy(ntpdate_process->server, "ntp1.aliyun.com");
	}
	if (strlen(ntpdate_process->time_now) == 0)
	{
		strcpy(ntpdate_process->time_now, "");
	}
	return 0;
}
...
static int get_tfcard_process(struct _machine_infor_fd *id, va_list args)
{
	...
	if (strlen(tfcard_process->insert) == 0)
	{
		strcpy(tfcard_process->insert, "0");
	}
	if (strlen(tfcard_process->mount) == 0)
	{
		strcpy(tfcard_process->mount, "0");
	}
	if (strlen(tfcard_process->status) == 0)
	{
		strcpy(tfcard_process->status, "0");
	}
	return 0;
}
...
static int start_process_mysql(struct _machine_infor	*stream)
{
	...
	{
		out("删除当前版本无效脚本/etc/mv_mysql_datadir.sh\n");
...
static int start_process_extend_space(struct _machine_infor	*stream)
{
	...
	if (access(DF_EXTEND_SPACE_SHELL, 0) != 0)
	{
		out("扩展区挂载脚本不存在,直接创建\n");
#ifndef MTK_OPENWRT
		out("新版本扩展区挂载脚本不存在,先删除massstore目录,直接创建\n");
		/*memset(oper_order, 0, sizeof(oper_order));
...
static int start_process_careader(struct _machine_infor	*stream)
{
	...
	if (result > 0)
	{
		out("/home/share/exe/ca_server 进程存在,需结束\n");
...
static int start_process_userload(struct _machine_infor	*stream)
{
	...
	{
		out("删除当前版本无效脚本/home/share/exe/MB2409115CX_U\n");
		system("rm /home/share/exe/MB2409115CX_U");
	}
	if (access("/home/share/exe/MB2409115CX_S", 0) == 0)
	{
		out("删除当前版本无效脚本/home/share/exe/MB2409115CX_S\n");
...
static int check_process_sqlite(struct _machine_infor	*stream)
{
	...
	//out("第一次启动是sqlite数据库不存在,需备份\n");
	if (access("/etc/mbdb.db", 0) != 0)
	{
		//out("备份目录中mbdb.db数据文件不存在\n");
		return 0;
	}
