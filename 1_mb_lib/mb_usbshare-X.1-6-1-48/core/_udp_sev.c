...
{   struct _udp_sev  *stream;
    ...
    {   //out("申请一个数据锁\n");
        ...
        /*stream->task_fd=_task_open("_udp_sev",_udp_sev_main_task,stream,DF_UDP_SEV_MAIN_TASK_TIMER,NULL);
        if(stream->task_fd<0)
        {   out("申请线程失败\n");
            close(stream->socketfd);
            _lock_close(stream->lock);
...
{   struct _udp_sev 	*stream;
	...
	if(result<=0)
	{	out("UDP服务接收出错result%d\n",result);
		return -1;	
	}
    out("读取udp数据完成 result = %d nFromLen =%d  client ip %s,client port = %d\n",result,nFromLen,inet_ntoa(saClient.sin_addr),ntohs(saClient.sin_port));
...
{   struct _udp_sev 	*stream;
	...
	if(result<0)
	{	out("UDP服务发送出错result%d\n",result);
		return -1;	
	}
    out("发送udp数据完成 result = %d nFromLen =%d\n",result,nFromLen);
    return result;   
}
