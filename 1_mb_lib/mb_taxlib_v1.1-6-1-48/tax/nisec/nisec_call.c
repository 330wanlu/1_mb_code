/*****************************************************************************
File name:   nisec_call.c
Description: 用于nisec模块中接口调用示例
Author:      Zako
Version:     1.0
Date:        2020.04
History:
...
int my_nisec_read_invoice(HDEV hDev, char *szQueryDateRange)
{
    ...
    for (i = 0; i < nTypeCount; i++) {  //专普票视为专票，电子机动车视为机动车
        ...
        int nRetBuffLen = 0;
        int nInvNum = GetInvBuff(hDev->hUSB, FPCX_COMMON_MONTH, &pRetBuff, &nRetBuffLen,
                                 abQueryDataRange, 8, true);
        NisecEntry(hDev->hUSB, NOP_CLOSE);
        if (0 == nInvNum) {
            _WriteLog(LL_WARN, "No invoice in this range of date, Inv-type:%d", bInvType);
            goto NextFPType;
        }
        if (nInvNum < 0)
            goto NextFPType;
        //   _WriteHexToDebugFile("nisec_usbraw.bin", pRetBuff, nRetBuffLen);
        nRet = NisecGetInvDetail(hDev, bInvType, pRetBuff, nInvNum, NULL);
    NextFPType:
        if (pRetBuff) {
            free(pRetBuff);
            pRetBuff = NULL;
...
int my_nisec_read_buyinvinfo_alldisplay(HDEV hDev)
{
    ...
    for (i = 0; i < nTypeCount; i++) {
        ...
        int nRet = GetStockInfo(hDev, stMIInf);
        if (nRet < 0)
            continue;
        printf("\n发票类型:\t\t\t%d\n当前发票代码(dqfpdm):\t\t%s\n当前发票号码(dqfphm)"
               ":\t\t%"
...
int my_nisec_read_taxinfo_alldisplay(HUSB hUSB)
{
    ...
    for (i = 0; i < sizeof(abInvoiceTypes); i++) {
        ...
        if (NisecSelectInvoiceTypeMini(hUSB, bInvType) < 0)
            continue;
        int nRet = GetMonitorInfo(hUSB, stMIInf);
        if (nRet < 0)
            continue;
        printf("发票类型:\t\t\t%d\n开票截止时间(kpjzsj):\t\t%s\n数据报送起始日期("
               "bsqsrq):\t%"
               ...
               stMIInf->zsljxe, stMIInf->fsljxe, stMIInf->zxbsrq, stMIInf->syrl, stMIInf->scjzrq,
               stMIInf->xdgnbs, stMIInf->lxkpsc, stMIInf->lxkpzs, stMIInf->lxzsljje,
               stMIInf->lxfsljje, stMIInf->lxkzxx);
    }
    free(stMIInf);
    if (NisecEntry(hUSB, NOP_CLOSE) < 0)
        return -2;
...
int my_nisec_read_id(HDEV hDev)
{
    ...
    char szTaxID[64] = "";
    char szNSRMC[256] = "";
    if (NisecGetBasicInfo(hDev, szSKPBH, szTaxID, szNSRMC) < 0) {
        _WriteLog(LL_FATAL, "Nisec get bh|taxid|mc failed");
        return -1;
    }
    printf("----------------------------------------------------------------\n"
           "NISEC SKP found, Device SN:%s NSRID:%s NSRName:%s\n"
           "----------------------------------------------------------------\n",
...
int my_nisec_make_invoice(HDEV hDev)
{
    ...
    if ((nRet = NisecMakeInvoiceMain(fpxx)) < 0) {
...
int my_nisec_read_upload_invoice(HDEV hDev)
{
    ...
    for (i = 0; i < sizeof(abInvoiceTypes); i++) {
        ...
        if ((nRet = QueryModifyInvoice(bInvType, hDev, szCurFPDM, szCurFPHM)) >= 0)
            break;
        nRet = -10;
        if (NisecEntry(hDev->hUSB, NOP_OPEN_FULL) < 0)
            break;
        ...
        int nRetBuffLen = 0;
        int nInvNum = GetInvBuff(hDev->hUSB, FPCX_COMMON_OFFLINE, &pRetBuff, &nRetBuffLen,
                                 abQueryDataRange, sizeof abQueryDataRange, true);
        NisecEntry(hDev->hUSB, NOP_CLOSE);
        if (0 == nInvNum) {
        ...
        if (nInvNum < 0) {
            _WriteLog(LL_FATAL, "GetInvBuff-type:%d nRet:%d\n", bInvType, nInvNum);
            goto NextFPType;
        }
        //   _WriteHexToDebugFile("nisec_usbraw.bin", pRetBuff, nRetBuffLen);
        //一张张处理
        nRet = NisecGetInvDetail(hDev, bInvType, pRetBuff, 1, UploadInvoice_CallBack);
        free(pRetBuff);
        break;
    ...
    if (nRet < 0)
        strcpy(szResultDes, "IO Error");
    else if (0 == nRet)
        strcpy(szResultDes, "upload failed");
    else  //> 0
        strcpy(szResultDes, "successful");
    _WriteLog(LL_INFO, "Nisec upload single invoice finish, fplx:%d fpdm:%s fphm:%s result:%s",
              bInvType, szCurFPDM, szCurFPHM, szResultDes);
    return nRet;
}
...
int my_nisec_upload_invoice_condition(HDEV hDev, uint8 bInvType, char *szFPDM, char *szFPHM)
{
    int nRet = -1;
    if (!CheckDeviceSupportInvType(hDev->abSupportInvtype, bInvType)) {
        _WriteLog(LL_FATAL, "Not support invtype:%d", bInvType);
    ...
    if (!fpxx)
        return -2;
    while (1) {
        if (NisecQueryInvInfo(hDev, bInvType, szFPDM, szFPHM, fpxx)) {
            _WriteLog(LL_FATAL, "Can not find this invoice %s %s", szFPDM, szFPHM);
            break;
        }
        if (!UploadInvoice_CallBack(fpxx))
            break;
        nRet = 0;
        break;
    }
...
//注意!!!这是单独接口,只能打开USB
// handle后单独调用.不能融入主流程，因为修改完之后需要重新连接设备
int my_nisec_change_certpassword(HDEV hDev, char *szOldPassword, char *szNewPassword)
{
    ...
    if (NisecChangeCertPassword(hDev->hUSB, hDev->szDeviceID, szOldPassword, szNewPassword) <= 0) {
...
int my_nisec_report_clear(HDEV hDev, uint8 bReportOrClear)
{
    ...
    for (i = 0; i < nTypeCount; i++) {
        ...
        _WriteLog(LL_INFO, "Begin %s, fplx:%d", bReportOrClear ? "RemoteReport" : "ClearCard",
                  bCommon_FPLX);
        if (bReportOrClear) {
            _WriteLog(LL_INFO, "Begin nisec reportsummary");
            if (NisecReporSummarytMain(hDev, bCommon_FPLX) < 0) {
                nRet = -2;
                break;
            }
        } else {
            _WriteLog(LL_INFO, "Begin nisec clearcard");
            if (NisecClearCardMain(hDev, bCommon_FPLX) < 0) {
                nRet = -2;
                break;
            }
        }
...
// https://bswj.jiangsu.chinatax.gov.cn:7001/preview.html?code=032001700311_00205951_20201017_F2I0H7C8
int my_nisec_get_pubserviceurl(HDEV hDev, uint8 bInvType, char *szFPDM, char *szFPHM)
{
    _WriteLog(LL_INFO, "Try my_nisec_get_pubserviceurl: FPDM:%s FPHM:%s", szFPDM, szFPHM);
    HFPXX fpxx = MallocFpxxLinkDev(hDev);
    ...
    while (1) {
        if ((nChildRet = NisecQueryInvInfo(hDev, bInvType, szFPDM, szFPHM, fpxx)) < 0) {
            nRet = -2;
            _WriteLog(LL_DEBUG, "QueryInvInfo failed, nRet:%d", nChildRet);
            break;
        }
...
{
    uint8 bFuncNo = 0;  // 0 == upload;1==query;2==cancel
    ...
    if (0 == bFuncNo) {
        ...
        if (DownloadTZDBH(fpxx->hDev, "20201201-20201230", "", "", szBuf, nBufSize) < 0) {
...
int my_nisec_switch_pubservice(HDEV hDev, bool bEnableOrDisable)
{
    ...
    if (NisecEntry(hDev->hUSB, NOP_OPEN_FULL) < 0)
        return -2;
    while (1) {
        if (NisecSelectInvoiceType(hDev, FPLX_COMMON_DZFP,
                                   SELECT_OPTION_TEMPLATE | SELECT_OPTION_MONITOR))
            break;
        nChildRet = SwitchPubservice(hDev, bEnableOrDisable, szSQXX);
        if (nChildRet < 0) {
        ...
        if (NisecUpdateCompanyInfo(hDev->hUSB, szSQXX) < 0) {
    ...
    NisecEntry(hDev->hUSB, NOP_CLOSE);
    _WriteLog(LL_INFO, "AisinoSwitchPubservice change status to :%s result:%s",
              bEnableOrDisable ? "enable" : "disable", nRet == 0 ? "successful" : "failed");
    return 0;
}
...
int nisec_get_basic_info(HDEV hDev, struct _plate_infos *plate_info)
{
	...
	//int nDataLen = 0;
	if (NisecGetBasicInfo(hDev, (char *)plate_info->plate_basic_info.plate_num, (char *)plate_info->plate_basic_info.ca_number, (char *)plate_info->plate_basic_info.ca_name) < 0) {
		_WriteLog(LL_FATAL, "Nisec get bh|taxid|mc failed");
		return -1;
	}
	//printf("----------------------------------------------------------------\n"
	...
	plate_info->plate_basic_info.plate_type = 2;
...
int nisec_get_state_info(HDEV hDev, struct _plate_infos *plate_info)
{
	...
	for (i = 0; i < sizeof(abInvoiceTypes); i++) {
		...
		if (!CheckDeviceSupportInvType(hDev->abSupportInvtype, bInvType)) {
			//_WriteLog(LL_FATAL, "Device not has this invoice-type, type:%d", bInvType);
			continue;
		}
		//_WriteLog(LL_INFO,"bInvType = %d\n", bInvType);
		memset(stMIInf, 0, sizeof(struct StMonitorInfo));
		if (NisecSelectInvoiceType(hDev, bInvType, SELECT_OPTION_TEMPLATE) < 0)
			continue;
		int nRet = GetMonitorInfo(hDev->hUSB, stMIInf);
		if (nRet < 0)
		{
			_WriteLog(LL_INFO,"GetMonitorInfo < 0 bInvType = %d\n", bInvType);
			continue;
		}
		/*_WriteLog(LL_INFO,"发票类型:\t\t\t%d\n开票截止时间(kpjzsj):\t\t%s\n数据报送起始日期("
		"bsqsrq):\t%"
		"s\n数据报送终止日期(bszzrq):\t%s\n单张发票开票金额限额(dzkpxe):\t%"
		"s\n正数累计限额?(zsljxe):\t\t%s\n负数累计限额?(fsljxe):\t\t%"
		"s\n最新报税日期(zxbsrq):\t\t%s\n剩余容量(syrl):\t\t\t%"
		...
		stMIInf->xdgnbs, stMIInf->lxkpsc, stMIInf->lxkpzs, stMIInf->lxzsljje,
		stMIInf->lxfsljje, stMIInf->lxkzxx);*/
		plate_info->plate_tax_info.inv_type[j] = abInvoiceTypes[i];
		j++;
		...
		/*_WriteLog(LL_INFO,"发票类型代码：%03d\n"
			...
			"单张发票开票金额限额:%s\n"
			"离线正数累计金额:%s\n",
			plate_info->plate_tax_info.inv_type[j - 1], plate_info->invoice_type_infos[i].monitor_info.bsqsrq, plate_info->invoice_type_infos[i].monitor_info.bszzrq, plate_info->invoice_type_infos[i].monitor_info.zxbsrq,
			plate_info->invoice_type_infos[i].monitor_info.kpjzsj, plate_info->invoice_type_infos[i].monitor_info.dzkpxe, plate_info->invoice_type_infos[i].monitor_info.lxzsljje);*/

	}
	plate_info->plate_tax_info.inv_type_num = j;

	//_WriteLog(LL_INFO,"支持的发票种类数为%d\n", plate_info->plate_tax_info.inv_type_num);
	free(stMIInf);
...
int nisec_read_inv_coil_number(HDEV hDev, struct _plate_infos *plate_info)
{
	...
	for (type_count = 0; type_count < sizeof(abInvoiceTypes); type_count++) {
		...
		//sprintf((char *)plate_info->invoice_type_infos[i].invoice_coils_info.dqfphm, "%s", stMIInf->dqfphm);
...
int nisec_get_state_info_and_coil_info(HDEV hDev, HUSB hUSB, struct _plate_infos *plate_info)
{
	...
	for (type_count = 0; type_count < sizeof(abInvoiceTypes); type_count++) {
		...
		if (NisecSelectInvoiceType(hDev, bInvType, SELECT_OPTION_TEMPLATE) < 0)
			continue;
		nRet = GetMonitorInfo(hUSB, stateInf);
		if (nRet < 0)
		{
			_WriteLog(LL_INFO,"GetMonitorInfo < 0 bInvType = %d\n", bInvType);
			continue;
		...
		//	"s\n发票剩余条目(zsyfs):\t\t%d\n返回发票条目(nGroupCount):\t%d\n"
		//	"-----------------------------------------------------------------\n",
		//	bInvType, coilInf->dqfpdm, coilInf->dqfphm, coilInf->zsyfs, coilInf->nGroupCount);
		plate_info->invoice_type_infos[mem_i].invoice_coils_info.state = 1;
		...
		if (coil_count > DF_MAX_INV_COILS)
			coil_count = DF_MAX_INV_COILS;
		plate_info->invoice_type_infos[mem_i].invoice_coils_info.wsyfpjsl = count;
		//_WriteLog(LL_INFO,"该发票种类的所有发票卷已读完\n");
		//printf("================================================================\n");
		if (NULL != stMIInf->pArray_StStockInfoGroup)
			free(stMIInf->pArray_StStockInfoGroup);
	}
...
static int check_fpxx_nisec(struct Fpxx* stp_fpxx, struct _plate_infos *plate_infos, char *errinfo)
{
	int i_spsmmclen = 0;
	...
	else 
	{
		if (strlen(stp_fpxx->gfsh) != 0 && (strlen(stp_fpxx->gfsh) != 15) && (strlen(stp_fpxx->gfsh) != 17) &&
			...
			printf("[-] TaxID's gfsh length must be [15,20]");
			sprintf(errinfo, "购方税号长度有误非0、15、17、18、20位");
			return DF_TAX_ERR_CODE_INV_DATA_ANALYSIS_ERR;
		}
	}*/
...
int nisec_make_invoice(HDEV hDev, char* inv_data, struct _plate_infos *plate_infos, struct _make_invoice_result *make_invoice_result)
{
	...
	int result = -1;
	ClearLastError(hDev->hUSB);
	result = LoadNisecDevInfo(hDev, hDev->hUSB, true);
	if (result < 0)
	{
	...
	if ((result = AnalyzeJsonBuff(hDev->bDeviceType,inv_data, stp_fpxx, (char *)make_invoice_result->errinfo)) != 0)
	{
		...
		result = DF_TAX_ERR_CODE_INV_DATA_ANALYSIS_ERR;
		goto FreeAndExit;
	}
	...
	if (NisecUpdateNodeInfo(stp_fpxx))
	{
	...
	result = check_fpxx_nisec(stp_fpxx, plate_infos, (char *)make_invoice_result->errinfo);
	if (result < 0)
	{
	...
	if ((result = NisecMakeInvoiceMain(stp_fpxx)) < 0) {
	...
	//_WriteLog(LL_INFO,"开票结果密文区信息内容：%s\n", make_invoice_result->mwq);
	result = 0;
...
int nesic_my_read_invoice_num(HDEV hDev, char *month, unsigned int *count, unsigned long *size)
{
	...
	for (i = 0; i < sizeof(abInvoiceTypes); i++)
	{
		...
		int nRetBuffLen = 0;
		int nInvNum = GetInvBuff(hDev->hUSB, 0, &pRetBuff, &nRetBuffLen, abQueryDataRange,
			sizeof abQueryDataRange, false);
	...
	*size = total_num * 3000;
	...
	//	uint8 bInvType = abInvoiceTypes[i];
	//	//if (QueryCurInvoTemplate(hDev->hUSB, bInvType, hDev->abInvTemplate) < 0)
	//	//	return -2;

	//	if (NisecEntry(hDev->hUSB, NOP_OPEN_FULL) < 0)
	//		return -1;
	//	if (NisecSelectInvoiceType(hDev, bInvType) < 0) 
	//	{
	//		if (NisecEntry(hDev->hUSB, NOP_CLOSE) < 0)
	//			return -2;
	//		_WriteLog(LL_INFO,"选择发票卷失败,%03d\n", bInvType);
	//		continue;
	//	}
	//	
	//	int nRetBuffLen = 0;
	...
	//	int nInvNum = GetInvBuff(hDev->hUSB,0, &pRetBuff, &nRetBuffLen, abQueryDataRange,
	//		sizeof abQueryDataRange,true);
	//	if (0 == nInvNum) {
	//		_WriteLog(LL_FATAL,
	//			"Get invoice buffer failed, error range or no invoice in this range");
	//		continue;
	//	}
	//	if (nInvNum < 0) {
	//		_WriteLog(LL_FATAL, "GetInvBuff-type:%d nRet:%d\n", bInvType, nInvNum);
	//		continue;
	//	}
	//	//_WriteLog(LL_INFO,"发票类型为%03d的发票数量为%d张\n", bInvType, nInvNum);
	//
	//	free(pRetBuff);

	//	*count += nInvNum;
	//	//_WriteLog(LL_INFO,"累积发票总数为%d\n", *count);
	//	int result;
	//	if ((result = NisecEntry(hDev->hUSB, NOP_CLOSE))< 0)
	//	{
	//		//_WriteLog(LL_INFO,"NisecEntry failed result = %d\n",result);
	//		//return -2;
	//	}
	//}
	//return *count;
...
int nisec_get_inv_details_mqtt(HDEV hDev, unsigned char *month, void *function, void *arg, char *errinfo, int old_new, unsigned short *start_stop, int sfxqqd)
{
	...
	nesic_my_read_invoice_num(hDev,(char *)month,&total_count, &total_size);
	...
	*start_stop = 1;
	...
	for (i = 0; i < sizeof(abInvoiceTypes); i++) 
	{
		...
		//_WriteLog(LL_INFO,"选择发票类型为%03d\n", bInvType);
		if (NisecSelectInvoiceType(hDev, bInvType, SELECT_OPTION_TEMPLATE) < 0)
			continue;
		uint8 *pRetBuff = NULL;
		int nRetBuffLen = 0;
		_WriteLog(LL_INFO,"时间段%s,发票类型为%03d\n", date_range, bInvType);
		int nInvNum = GetInvBuff(hDev->hUSB, 0, &pRetBuff, &nRetBuffLen, abQueryDataRange, sizeof abQueryDataRange, true);
		if (0 == nInvNum) 
		{
			_WriteLog(LL_FATAL,	"Get invoice buffer failed, error range or no invoice in this range");
			continue;
		}
		if (nInvNum < 0) 
		{
		...
		_WriteLog(LL_INFO,"发票数量为%d\n", nInvNum);
		//write_file("/home/share/exe/nisec_usbraw.bin", (char *)pRetBuff, nRetBuffLen);

		//NisecMultiInvoiceEntry_mqtt_hook(hDev, bInvType, pRetBuff, nRetBuffLen, nInvNum, total_count, com_count, function, arg, errinfo, old_new, start_stop, sfxqqd);
			...
			for (inv_count = 0; inv_count < nInvNum; inv_count++)
			{
				...
				int index_len;
				int nChildRet = NisecInvRaw2Fpxx(hDev, bInvType, pBlockBin, fpxx, &index_len);
				if (nChildRet < 0) 
				{
					FreeFpxx(fpxx);
				...
				if (old_new == 0)
				{
					result = fpxx_to_json_base_ykfpcx(fpxx, &encode_data);
				...
				//_WriteLog(LL_INFO,"处理%03d发票种类第 %d 张发票完成============================\n", bInvType, now_count);
...
static int nisec_fpdm_fphm_get_invs_json_base(HDEV hDev, char  *fpdm, char *fphm, HFPXX fpxx)
{
	int ret = -1;
	uint8 abInvoiceTypes[] = { 4, 7, 26 };
	int i = 0;
	for (i = 0; i < sizeof(abInvoiceTypes); i++)
	{
		...
		if ((ret = NisecQueryInvInfo(hDev, bInvType, fpdm, fphm, fpxx)) < 0)
		{
...
static int nisec_get_invoice_month_upload_hook(HDEV hDev, uint8 *month, void *function, void *arg, struct _upload_inv_num *upload_inv_num, unsigned short *start_stop, struct _inv_sum_data *now_month_sum)
{
	char errinfo[1024] = { 0 };
	int result = 0;
	char date_range[20] = { 0 };
	...
	result = nisec_read_tax_time(hDev, stand_time);
	if (result < 0)
	{
		_WriteLog(LL_INFO, "税控盘获取税盘时间失败\n");
		return DF_TAX_ERR_CODE_USB_IO_HANDLE_ERROR;
	}
	...
	*start_stop = 1;
	...
	for (i = 0; i < sizeof(abInvoiceTypes); i++)
	{
		...
		//_WriteLog(LL_INFO, "%s 获取%s月发票总数 = %d\n", hDev->szCompanyName, month, nInvNum);
		NisecEntry(hDev->hUSB, NOP_CLOSE);
	...
	if (hook_count == 0)
	{
		...
		nisec_query_invoice_month_all_data(hDev, (char *)month, &statistics, errinfo);
		...
		{
			logout(INFO, "TAXLIB", "发票同步", "%s税盘,发票读取月总数为0,汇总数据与发票明细数据对比不一致或对比出错,错误号：%d\r\n", hDev->szCompanyName, result);
	...
	if (*start_stop == 1)
	{
		...
		{
			logout(INFO, "TAXLIB", "发票同步", "%s税盘,汇总数据与发票明细数据对比不一致或对比出错,错误号：%d\r\n", hDev->szCompanyName, result);
		...
		//_WriteLog(LL_INFO,"当前发票正常读取完成,非中断结束,通知M服务当前月读取完成\n");
		if (statistics != NULL)
			free(statistics);
	}
	//_WriteLog(LL_INFO,"[%s]发票查询上传结束\n", hDev->szCompanyName);
	if (year_month_data != NULL)
		free(year_month_data);
	free(send_data);
	free(lxp_data);
...
static int nisec_get_invoice_single_upload_hook(HDEV hDev, void *function, void *arg, struct _upload_inv_num *upload_inv_num)
{
	...
	for (inv_count = 0; inv_count < sizeof(upload_inv_num->need_upload_dmhm) / sizeof(upload_inv_num->need_upload_dmhm[0]); inv_count++)
	{
		...
		if ((result = nisec_fpdm_fphm_get_invs_json_base(hDev, upload_inv_num->need_upload_dmhm[inv_count].fpdm, upload_inv_num->need_upload_dmhm[inv_count].fphm, fpxx)) < 0)
		{
...
int nisec_upload_m_server(HDEV hDev, unsigned short *start_stop, struct _upload_inv_num *upload_inv_num, void *function, void *arg, struct _inv_sum_data *now_month_sum)
{
	...
	if ((upload_inv_num->state == 0) && (upload_inv_num->need_fpdmhm_flag == 1))
	{
		...
		/////后续上传操作
		result = nisec_get_invoice_single_upload_hook(hDev, function, arg, upload_inv_num);
		return result;
	...
	*start_stop = 1;
	...
	for (; strcmp(summary_month, today_month) <= 0; mb_get_next_month(summary_month))
	{
		...
		{
			result = nisec_get_invoice_month_upload_hook(hDev, (uint8 *)year_month, function, arg, upload_inv_num, start_stop, now_month_sum);
			if (result < 0)
			{
		...
		{
			char *statistics = NULL;
			result = nisec_query_invoice_month_all_data(hDev, (char *)summary_month, &statistics, errinfo);
			if (result < 0)
			{
...
#else
static int nisec_get_invoice_month_upload_hook(HDEV hDev, char *month, void *function, void *arg, struct _upload_inv_num *upload_inv_num, unsigned short *start_stop, struct _inv_sum_data *now_month_sum)
{
	...
	*start_stop = 1;
	for (i = 0; i < sizeof(abInvoiceTypes); i++)
	{
		...
		int nRetBuffLen = 0;
		//_WriteLog(LL_INFO,"发票类型为%03d\n", bInvType);
		one_inv_type_num = GetInvBuff(hDev->hUSB, 0, &pInvoiceBuff, &nRetBuffLen, abQueryDataRange, sizeof abQueryDataRange, true);
		if (one_inv_type_num <= 0)
		{
			_WriteLog(LL_INFO, "%s 获取%s月的%03d类型发票,无发票或查询失败 = %d\n", hDev->szCompanyName, month, bInvType, one_inv_type_num);
			if (pInvoiceBuff != NULL)
			{
				free(pInvoiceBuff);
				pInvoiceBuff = NULL;
			}
		...
		_WriteLog(LL_INFO, "%s 获取%s月的%03d类型发票,总数为 = %d\n", hDev->szCompanyName, month, bInvType, one_inv_type_num);
		NisecEntry(hDev->hUSB, NOP_CLOSE);
...
int nisec_upload_m_server(HDEV hDev, unsigned short *start_stop, struct _upload_inv_num *upload_inv_num, void *function, void *arg, struct _inv_sum_data *now_month_sum)
{
	...
	*start_stop = 1;
	...
	for (; strcmp(invoice_month, today_month) <= 0; mb_get_next_month(invoice_month))
	{
		...
		result = nisec_get_invoice_month_upload_hook(hDev, invoice_month, function, arg, upload_inv_num, start_stop, now_month_sum);
		if (result < 0)
		{
			_WriteLog(LL_INFO, "读取发票数据并回调上传过程中出现异常,停止正在查询%s月发票详情动作\n", (char *)invoice_month);
			return result;
...
int nisec_read_current_inv_code(HDEV hDev, uint8_t bNeedType, char *szInvCurrentNum, char *szInvTypeCode)
{
	...
	if (NisecSelectInvoiceType(hDev, bNeedType, SELECT_OPTION_TEMPLATE) < 0) {
		return -3;
	}
...
int nisec_waste_invoice(HDEV hDev, uint8 bInvType, char *szFPDM, char *szFPHM, struct _cancel_invoice_result *cancel_invoice_result)
{
	...
	while (1) {
		...
		if ((nChildRet = NisecWasteInvoice(hDev, bInvType, false, szFPDM, szFPHM,fpxx->kpr,1,&cancel_invoice_result->need_restart)) < 0)
		{
...
int nisec_cancel_null_invoice_to_plate(HDEV hDev, struct _cancel_invoice_result *cancel_invoice_result)
{
	...
	if ((result = NisecWasteInvoice(hDev, cancel_invoice_result->fplxdm, true, (char *)cancel_invoice_result->fpdm, (char *)cancel_invoice_result->fphm, (char *)cancel_invoice_result->zfr, cancel_invoice_result->zfzs,&cancel_invoice_result->need_restart)) < 0)
	{
		sprintf((char *)cancel_invoice_result->errinfo, "底层作废失败,%s", hDev->hUSB->szLastErrorDescription);
		//sprintf((char *)cancel_invoice_result->errinfo, "底层操作作废失败");
		return DF_TAX_ERR_CODE_PLATE_OPER_FAILED;
...
int nisec_is_exsist_offline_inv(HDEV hDev, char *fpdm, char *fphm, char *kpsj, int *dzsyh)
{
	...
	uint8 abQueryDataRange[8] = { 0 };
	uint8 month_now[20] = { 0 };
	uint8 last_month_range[50] = { 0 };
	uint8 now_month_range[50] = { 0 };
	uint8 date_range[50] = { 0 };
	ClearLastError(hDev->hUSB);
	memcpy(month_now,hDev->szDeviceTime,6);
	turn_month_to_range((char *)month_now, (char *)now_month_range);
	//_WriteLog(LL_INFO, "离线票查询范围本月：%s", now_month_range);

	last_month_asc((char *)month_now);	
	turn_month_to_range((char *)month_now, (char *)last_month_range);
	//_WriteLog(LL_INFO, "离线票查询范围上月：%s", last_month_range);

	memcpy(date_range, last_month_range, 8);
	memcpy(date_range + 8, now_month_range + 9, 8);
	//_WriteLog(LL_INFO, "离线票查询范围：%s", date_range);
	asc2bcd(date_range, abQueryDataRange, strlen((char *)date_range));
	...
	//int nUploadOK = 0;
	uint8 bInvType = 0;
	//_WriteLog(LL_INFO,"开始获取税控盘离线票张数\n");
	if (NisecEntry(hDev->hUSB, NOP_OPEN_FULL) < 0)
		return -1;
	for (i = 0; i < sizeof(abInvoiceTypes); i++) 
	{
		...
		int nRetBuffLen = 0;
		int nInvNum = GetInvBuff(hDev->hUSB, 2, &pRetBuff, &nRetBuffLen, abQueryDataRange,
			sizeof abQueryDataRange, false);
		//_WriteLog(LL_FATAL, "%03d票种读取离线张数完成,离线张数为%d\n", bInvType, nInvNum);
		...
		nChildRet = NisecLogicIO(hDev->hUSB, cb, 16, sizeof(cb), NULL, 0);
		if (nChildRet < 15)
			continue;
...
int nisec_inv_upload_server(HDEV hDev, struct _offline_upload_result *offline_upload_result)
{
	...
	for (i = 0; i < sizeof(abInvoiceTypes); i++) 
	{
		...
		while (1) {
			//如果签名为空，需要补签名，完了才能上传
			//if (!strlen(fpxx->sign) && NisecFpxxAppendSignIO(fpxx, false))
			...
			if (NisecInvoiceUploadAndDownload(fpxx, szDZSYH)) {
				if (strlen(hDev->hUSB->errinfo) < sizeof(offline_upload_result->errinfo))
					strcpy((char *)offline_upload_result->errinfo, hDev->hUSB->errinfo);
				_WriteLog(LL_WARN, "Nisec upload invoice,netio failed, step1,errinfo [%s]", offline_upload_result->errinfo);
		...
		_WriteLog(LL_DEBUG, "Nisec upload invoice %s, fplx:%d fpdm:%s fphm:%s",	nRet ? "successful" : "failed", fpxx->fplx, fpxx->fpdm, fpxx->fphm);
		...
		FreeFpxx(fpxx);
		//nRet = NisecGetInvDetailUpload(hDev, bInvType, pRetBuff, 1, UploadInvoice_CallBack, offline_upload_result);
		//_WriteLog(LL_FATAL, "NisecGetInvDetail nRet:%d\n", nRet);
		free(pRetBuff);
		pRetBuff = NULL;
		break;
	NextFPType:
	...
	else  //== 1
	{
		...
		HFPXX stp_fpxx = MallocFpxxLinkDev(hDev);
		int nChildRet = NisecQueryInvInfo(hDev, offline_upload_result->fplxdm, (char *)offline_upload_result->fpdm, (char *)offline_upload_result->fphm, stp_fpxx);
		if (nChildRet < 0) 
		{
			FreeFpxx(stp_fpxx);
			_WriteLog(LL_WARN, "盘号:%s,发票上传后查询发票失败, fpdm:%s fphm:%s", hDev->szDeviceID,offline_upload_result->fpdm, offline_upload_result->fphm);
			return -3;
		}
	...
	_WriteLog(LL_INFO, "nisec_inv_upload_server Nisec upload single invoice finish,【%s】 fplx:%d fpdm:%s fphm:%s result:%s", hDev->szCompanyName, bInvType, offline_upload_result->fpdm, offline_upload_result->fphm, szResultDes);
	return nRet;
}
...
int nisec_fpdm_fphm_update_invs(HDEV hDev, char  *fpdm, char *fphm, char **inv_json)
{
	...
	else
	{
		...
		while (1) {
			...
			_WriteLog(LL_FATAL, "Update invoice's sign success");
			if (NisecInvoiceUploadAndDownload(fpxx, szDZSYH)) {
				_WriteLog(LL_WARN, "Nisec upload invoice,netio failed, step1,errinfo [%s]", fpxx->hDev->hUSB->szLastErrorDescription);
				break;
			}
			_WriteLog(LL_WARN, "Nisec upload invoice,NisecInvoiceUploadAndDownload, step1 success");
			if (NisecUpdateInvUploadFlag(fpxx, szDZSYH) < 0) {
...
int nisec_check_server_address(HDEV hDev, char **splxxx,char *errinfo)
{
	...
	char *tmp = NisecPreCompanyXml(hDev);
	if (tmp != NULL)
	{
		char *tmp_base;
		...
		Base64_Encode(tmp, strlen(tmp), tmp_base);
		free(tmp);
		*splxxx = tmp_base;
	}
...
int nisec_verify_cert_passwd(HDEV hDev, int *left_num)
{
	...
	while (1) {
		...
		if ((nChildRet = NisecLogicIORaw(hDev->hUSB, cb, 4, sizeof(cb), NULL, 0)) != 8)
		{
			//_WriteLog(LL_INFO, "Nisec_Crypt_Password NisecLogicIORaw failed nChildRet = %d", nChildRet);
		...
		sprintf(szBuf, "33%s", hDev->szDeviceID);
		if (Build16ByteCryptKeyAnd4ByteVCode(szBuf, DEF_CERT_PASSWORD_DEFAULT, abPreHashBegin8,
			abStep2_16BytesCMD, abStep2_16bytesParament,
			abCryptKey, abUSBCmdVCode) < 0)
		{
			//_WriteLog(LL_INFO, "Nisec_Crypt_Password Build16ByteCryptKeyAnd4ByteVCode failed");
			nRet = DF_TAX_ERR_CODE_USB_IO_HANDLE_ERROR;
			break;
		}
		...
		if ((nChildRet = NisecLogicIORaw(hDev->hUSB, cb, 16, sizeof(cb), abCryptKey, 0x10)) < 0) {
			...
			_WriteLog(
				...
				-nRet);
			char errinfo[1024] = { 0 };
			sprintf(errinfo, "证书口令有误，可能原因为未修改默认证书口令12345678,剩余尝试次数%d", -nRet);
			*left_num = -nRet;
...
int nisec_change_certpassword(HDEV hDev, char *szOldPassword, char *szNewPassword, char *errinfo)//注意!!!这是单独接口,只能打开USB handle后单独调用.不能融入主流程，因为修改完之后需要重新连接设备
//int my_change_certpassword(HUSB hUsb, char *szOldPassword, char *szNewPassword)
{
	...
	nChildRet = NisecChangeCertPassword(hDev->hUSB, hDev->szDeviceID, szOldPassword, szNewPassword);
	if (nChildRet <= 0)
	{
		sprintf(errinfo, "密码修改失败");
		return -5;
	}
...
int nisec_copy_report_data(HDEV hDev, uint8 inv_type, char *errinfo)
{
	...
	if (FPLX_COMMON_ZYFP != inv_type &&	FPLX_COMMON_PTFP != inv_type&&	FPLX_COMMON_DZFP != inv_type)
	{
	...
	if ((result = NisecReporSummarytMain(hDev, inv_type)) < 0) {
...
int nisec_report_clear(HDEV hDev, uint8 inv_type, char *errinfo)
{
	...
	ClearLastError(hDev->hUSB);//优化[USB] 09c000,设备未开启错误，后期统一修改
	if ((result = NisecClearCardMain(hDev, inv_type)) < 0) {
		strcpy(errinfo, hDev->hUSB->errinfo);//网络出错存储在errinfo
...
int nisec_upload_check_redinvform(HDEV hDev, uint8 inv_type, char* inv_data, struct _plate_infos *plate_infos, struct _askfor_tzdbh_result *askfor_tzdbh_result)
{
	...
	else
	{
		...
		if ((result = NisecQueryInvInfo(hDev, FPLX_COMMON_ZYFP, (char *)askfor_tzdbh_result->lzfpdm, (char *)askfor_tzdbh_result->lzfphm, fpxx)) < 0)
		{
			FreeFpxx(fpxx);
			_WriteLog(LL_INFO, "发票获取失败,ret = %d\n", result);
...
int nisec_redinv_tzdbh_cancel(HDEV hDev, char *redNum, char *errinfo)
{
	...
	nChildRet = LoadNisecDevInfo(hDev, hDev->hUSB, true);
	if (nChildRet < 0)
	{
		free(szBuf);
		return DF_TAX_ERR_CODE_USB_IO_HANDLE_ERROR;
...
int nisec_connect_pubservice(HDEV hDev, uint8 inv_type, char *errinfo)
{
	...
	while (1) {
		...
		nChildRet = SwitchPubservice(hDev, true, szSQXX);
		if (nChildRet < 0) {
			strcpy(errinfo, hDev->hUSB->errinfo);
			_WriteLog(LL_FATAL, "AisinoSwitchPubservice failed");
			break;
		...
		if (NisecEntry(hDev->hUSB, NOP_OPEN_FULL) < 0)
			break;
		if (NisecSelectInvoiceType(hDev, FPLX_COMMON_DZFP, SELECT_OPTION_TEMPLATE))
			break;
		if (NisecUpdateCompanyInfo(hDev->hUSB, szSQXX) < 0) {
	...
	_WriteLog(LL_INFO, "AisinoSwitchPubservice change status to :%s result:%s",
		true ? "enable" : "disable", nRet == 0 ? "successful" : "failed");
...
int nisec_disconnect_pubservice(HDEV hDev, uint8 inv_type, char *errinfo)
{
	...
	while (1) {
		...
		nChildRet = SwitchPubservice(hDev, false, szSQXX);
		if (nChildRet < 0) {
			strcpy(errinfo, hDev->hUSB->errinfo);
			_WriteLog(LL_FATAL, "AisinoSwitchPubservice failed");
			break;
	...
	_WriteLog(LL_INFO, "AisinoSwitchPubservice change status to :%s result:%s",
		false ? "enable" : "disable", nRet == 0 ? "successful" : "failed");
...
int nisec_query_net_invoice_coil(HDEV hDev, char *date_range, char *inv_data, char *errinfo)
{
	...
	result = NisecNetInvoiceQueryReadyEasy(&hi,hDev, coil_data);
	if (result < 0)
	{
		strcpy(errinfo, hDev->hUSB->szLastErrorDescription);
		HTTPClose(&hi);
...
int nisec_query_net_invoice_coil_download_unlock(HDEV hDev, char *date_range, uint8 fllxdm, char *fpdm, char *fpqshm, int fpzs, char *errinfo)
{
	...
	for (;;)
	{
		...
		//_WriteLog(LL_FATAL, "网络发票种类:%03d,网络发票代码：%s,网络发票号码：%s,发票发票张数：%d", net_fpzl, net_fpdm, net_fphm, net_fpzs);
		//_WriteLog(LL_FATAL, "下载发票种类:%03d,下载发票代码：%s,下载发票号码：%s,下载发票张数：%d", fllxdm, fpdm, fpqshm, fpzs);
	...
	result = NisecNetInvoiceDownloadWriteEasy(&hi, hDev, dowm_data);
	if (result < 0)
	{
		strcpy(errinfo, hDev->hUSB->szLastErrorDescription);
		if (strlen(errinfo) == 0)
...
int nisec_query_invoice_month_all_data(HDEV hDev, char *month, char **data_json, char *errinfo)
{
	...
	ClearLastError(hDev->hUSB);
	//如果是当前月则到今天截止
	result = LoadNisecDevInfo(hDev, hDev->hUSB, false);
	if (result < 0)
	{
		return DF_TAX_ERR_CODE_USB_IO_HANDLE_ERROR;
	}	
	...
	{  //专普票视为专票，电子机动车视为机动车
		...
		memset(&statistics, 0, sizeof(struct StMonthStatistics));
		if (GetMonthStatistics(hDev->hUSB, start_date, end_date, &statistics) < 0)
		{
			cJSON_AddStringToObject(dir3, "data", "");
			continue;
		}
		...
		cJSON_AddStringToObject(root, "qckcfs", statistics.qckcfs);//期初库存
		cJSON_AddStringToObject(root, "lgfpfs", statistics.lgfpfs);//领购发票份数 
		cJSON_AddStringToObject(root, "thfpfs", statistics.thfpfs);//退回发票份数 

		int zsfpfs = atoi(statistics.zsfpfs) + atoi(statistics.zffpfs) + atoi(statistics.kffpfs);
		memset(tmp_s, 0, sizeof(tmp_s));
		sprintf(tmp_s, "%d", zsfpfs);
		cJSON_AddStringToObject(root, "zsfpfs", tmp_s);//正数发票份数 

		int zffpfs = atoi(statistics.zffpfs) + atoi(statistics.kffpfs);
		memset(tmp_s, 0, sizeof(tmp_s));
		sprintf(tmp_s, "%d", zffpfs);
		cJSON_AddStringToObject(root, "zffpfs", tmp_s);//正废发票份数 

		int fsfpfs = atoi(statistics.fsfpfs) + atoi(statistics.fffpfs);
		memset(tmp_s, 0, sizeof(tmp_s));
		sprintf(tmp_s, "%d", fsfpfs);
		cJSON_AddStringToObject(root, "fsfpfs", tmp_s);//负数发票份数 

		cJSON_AddStringToObject(root, "fffpfs", statistics.fffpfs);//负废发票份数 
		cJSON_AddStringToObject(root, "kffpfs", statistics.kffpfs);//空废发票份数 
		cJSON_AddStringToObject(root, "qmkcfs", statistics.qmkcfs);//期末库存份数 
		cJSON_AddStringToObject(root, "zsfpljje", statistics.zsfpljje);//正数发票累计金额 
		cJSON_AddStringToObject(root, "zsfpljse", statistics.zsfpljse);//正数发票累计税额 
		cJSON_AddStringToObject(root, "zffpljje", statistics.zffpljje);//正废发票累计金额 
		cJSON_AddStringToObject(root, "zffpljse", statistics.zffpljse);//正废发票累计税额 
		cJSON_AddStringToObject(root, "fsfpljje", statistics.fsfpljje);//负数发票累计金额 
		cJSON_AddStringToObject(root, "fsfpljse", statistics.fsfpljse);//负数发票累计税额 
		cJSON_AddStringToObject(root, "fffpljje", statistics.fffpljje);//负废发票累计金额 
		cJSON_AddStringToObject(root, "fffpljse", statistics.fffpljse);//负废发票累计税额 

		long double sjxsje = strtold(statistics.zsfpljje, NULL) - strtold(statistics.fsfpljje, NULL);
		memset(tmp_s, 0, sizeof(tmp_s));
		sprintf(tmp_s, "%4.2Lf", sjxsje);
		//PriceRound(tmp_s, 2, tmp_s);
		cJSON_AddStringToObject(root, "sjxsje", tmp_s);//实际销售金额 
		long double sjxsse = strtold(statistics.zsfpljse, NULL) - strtold(statistics.fsfpljse, NULL);
		memset(tmp_s, 0, sizeof(tmp_s));
		...
		char *base_buf;
		json_buf = cJSON_Print(root);	
		//_WriteLog(LL_INFO, "发票类型%03d", bInvType);
		//_WriteLog(LL_INFO, "%s", json_buf);
		base_buf = malloc(strlen(json_buf) * 2);
		Base64_Encode(json_buf, strlen(json_buf), base_buf);
		free(json_buf);
...
int nisec_get_cert(HDEV hDev, char *cert_data, char *errinfo)
{
	...
	if (NisecEnumCertsMem(hDev->hUSB, pCerts) < 2)
	{
		free(pCerts);
		return -1;
	}
...
int nisec_update_summary_data(HDEV hDev, struct _plate_infos *plate_infos)
{
	...
	result = nisec_query_invoice_month_all_data(hDev, (char *)month, &statistics, errinfo);
	if (result < 0)
	{
