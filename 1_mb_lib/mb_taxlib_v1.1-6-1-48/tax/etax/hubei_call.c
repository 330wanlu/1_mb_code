...
int etax_hubei_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hubei_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hubei_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hubei_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_hubei_fuction = 
{
    .etax_login = etax_hubei_login,
    .etax_user_query = etax_hubei_user_query,
    .etax_enterpriselist = etax_hubei_enterpriselist,
    .etax_relationchange = etax_hubei_relationchange
};

//自定义header
HEADER_STRUCT header_array_hubei[] = {
    {"Cookie", "oauth2_referer=szzhzz.hubei.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
int get_tpass_addr_hubei(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_hubei_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    if(!atoi(login_parameter->sys_type))  //默认税
    {
        ...
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://etax.hubei.chinatax.gov.cn");
        memset(hEtax->etax.modelTaAddr.szInitRequestPage,0x00,sizeof(hEtax->etax.modelTaAddr.szInitRequestPage));
        strcpy(hEtax->etax.modelTaAddr.szInitRequestPage,"/portal/");
    }
    else                                  //票
    {
        hEtax->etax.user_args = (void *)(&header_array_hubei);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.hubei.chinatax.gov.cn:8443");
        memset(hEtax->etax.modelTaAddr.szInitRequestPage,0x00,sizeof(hEtax->etax.modelTaAddr.szInitRequestPage));
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            strcpy(pValue,"https://tpass.hubei.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.hubei.chinatax.gov.cn/portal/login-web/api/third/sso/login/redirect&client_id=y852fe37d8e24d46ae8923fcd5yee6ac&response_type=code&state=test");
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            sprintf(login_parameter->lpcode,"https://etax.hubei.chinatax.gov.cn/portal/login-web/api/third/sso/login/redirect?code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
...
int etax_hubei_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //sprintf(relationchange_parametetr->lpcode,"https://etax.hubei.chinatax.gov.cn/web/dzswj/kxyh/loginyw.html?code=%s&state=",objItem->valuestring);
        
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            sprintf(relationchange_parametetr->lpcode,"https://etax.hubei.chinatax.gov.cn/portal/login-web/api/third/sso/login/redirect?code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
