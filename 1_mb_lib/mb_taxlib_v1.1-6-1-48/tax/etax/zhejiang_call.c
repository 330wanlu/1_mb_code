...
int etax_zhejiang_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_zhejiang_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_zhejiang_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_zhejiang_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_zhejiang_fuction = 
{
    .etax_login = etax_zhejiang_login,
    .etax_user_query = etax_zhejiang_user_query,
    .etax_enterpriselist = etax_zhejiang_enterpriselist,
    .etax_relationchange = etax_zhejiang_relationchange
};

//自定义header
HEADER_STRUCT header_array_zhejiang[] = {
    {"Cookie", "oauth2_referer=szzhzz.zhejiang.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_zhejiang(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_zhejiang_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_zhejiang);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.zhejiang.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            strcpy(pValue,"https://tpass.zhejiang.chinatax.gov.cn:8443/login?redirect_uri=https://etax.zhejiang.chinatax.gov.cn/zjgfdzswj/main/kx/skip.html?service=https://etax.zhejiang.chinatax.gov.cn/zjgfdzswj/main/home/wdxx/index.html&client_id=xdf6a5cd4cx74bfca46x3bbfcxa4c4xf&response_type=code&state=test");
            //strcpy(pValue,"");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_zhejiang(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            ...
            sprintf(login_parameter->lpcode,"https://etax.zhejiang.chinatax.gov.cn/zjgfdzswj/main/kx/skip.html?client_id=xdf6a5cd4cx74bfca46x3bbfcxa4c4xf&code=%s&service=https://etax.zhejiang.chinatax.gov.cn/zjgfdzswj/main/home/wdxx/index.html&state=test",objItem->valuestring);
...
int etax_zhejiang_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_zhejiang(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_zhejiang_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_zhejiang(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_zhejiang(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.zhejiang.chinatax.gov.cn/zjgfdzswj/main/kx/skip.html?client_id=xdf6a5cd4cx74bfca46x3bbfcxa4c4xf&code=%s&service=https://etax.zhejiang.chinatax.gov.cn/zjgfdzswj/main/home/wdxx/index.html&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
