...
int etax_gansu_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_gansu_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_gansu_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_gansu_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_gansu_fuction = 
{
    .etax_login = etax_gansu_login,
    .etax_user_query = etax_gansu_user_query,
    .etax_enterpriselist = etax_gansu_enterpriselist,
    .etax_relationchange = etax_gansu_relationchange
};

//自定义header
HEADER_STRUCT header_array_gansu[] = {
    {"Cookie", "oauth2_referer=szzhzz.gansu.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_gansu(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_gansu_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_gansu);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.gansu.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            //https://tpass.gansu.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.gansu.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=https://etax.gansu.chinatax.gov.cn/xxmh/html/index_login.html&client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&response_type=code&state=test
            //https://tpass.gansu.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.gansu.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&client_id=w5w89a3acaew4mw5b59mm85782ckdmm2&response_type=code&state=test
            strcpy(pValue,"https://tpass.gansu.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.gansu.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&client_id=w5w89a3acaew4mw5b59mm85782ckdmm2&response_type=code&state=test");
            //strcpy(pValue,"");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_gansu(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        else if(memcmp(login_parameter->login_method,"02",2) == 0)  //数字证书
        {
            //获取税盘证书与税局地址进行签名验证
            if(!atoi(login_parameter->sys_type)){
                if ((nChildRet = TPassLoginAuthHelloAndAuthgansu(&hEtax->tpass, uriTpassFull))) {
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            ...
            //sprintf(login_parameter->lpcode,"https://etax.gansu.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=18169169c19c11edb15240a6b75aad54&code=%s&service=http://etax.gansu.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
            //https://etax.gansu.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&code=%s&service=https://etax.gansu.chinatax.gov.cn/xxmh/html/index_login.html&state=test
            sprintf(login_parameter->lpcode,"https://etax.gansu.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&code=%s&state=test",objItem->valuestring);
...
int etax_gansu_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_gansu(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_21_Msg(&hEtax->tpass, uriTpassFull,user_query_parameter->svr_container,user_query_parameter->soft_cert_ip,user_query_parameter->soft_cert_port,errinfo_uft8_21)))
            {
                sprintf((char *)errinfo_uft8, "%s",errinfo_uft8_21);
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
...
int etax_gansu_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_21_Msg(&hEtax->tpass, uriTpassFull,enterpriselist_parameter->svr_container,enterpriselist_parameter->soft_cert_ip,enterpriselist_parameter->soft_cert_port,errinfo_uft8_21)))
            {
                sprintf((char *)errinfo_uft8, "%s",errinfo_uft8_21);
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
...
int etax_gansu_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_gansu(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_gansu(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        {
            pValue = calloc(1,BS_BLOCK*2);  //b62deMdW365e4dfba3W63bab7dW77be4
            strcpy(pValue,"https://tpass.gansu.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.gansu.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=https://etax.gansu.chinatax.gov.cn/xxmh/html/index_login.html&client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&response_type=code&state=test");
            printf("lbc-debug pValue = %s\n",pValue);
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_gansu(&hEtax->tpass, uriTpassFull,relationchange_parametetr->svr_container,relationchange_parametetr->soft_cert_ip,relationchange_parametetr->soft_cert_port,errinfo_uft8_21)))
            {
                sprintf((char *)errinfo_uft8, "%s",errinfo_uft8_21);
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.gansu.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&code=%s&service=https://etax.gansu.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
