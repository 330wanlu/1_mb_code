...
int etax_guangxi_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    if(!atoi(login_parameter->sys_type))  //默认税
    {
        ...
        // strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://etax.guangxi.chinatax.gov.cn:9723");
        // memset(hEtax->etax.modelTaAddr.szInitRequestPage,0x00,sizeof(hEtax->etax.modelTaAddr.szInitRequestPage));
        // strcpy(hEtax->etax.modelTaAddr.szInitRequestPage,"/web/dzswj/ythclient/mh.html?flag=logout");
    }
    else                 //票
    {
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            pValue = calloc(1,BS_BLOCK*2);  //b62deMdW365e4dfba3W63bab7dW77be4
                        // https://tpass.guangxi.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.guangxi.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            //https://etax.guangxi.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test
...
int etax_guangxi_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //sprintf(relationchange_parametetr->lpcode,"https://etax.guangxi.chinatax.gov.cn/web/dzswj/kxyh/loginyw.html?code=%s&state=",objItem->valuestring);
