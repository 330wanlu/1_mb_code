...
int etax_guizhou_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_guizhou_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_guizhou_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_guizhou_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_guizhou_fuction = 
{
    .etax_login = etax_guizhou_login,
    .etax_user_query = etax_guizhou_user_query,
    .etax_enterpriselist = etax_guizhou_enterpriselist,
    .etax_relationchange = etax_guizhou_relationchange
};

//自定义header
HEADER_STRUCT header_array_guizhou[] = {
    {"Cookie", "oauth2_referer=szzhzz.guizhou.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_guizhou(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_guizhou_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_guizhou);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.guizhou.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            //https://tpass.guizhou.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.guizhou.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=https://etax.guizhou.chinatax.gov.cn/xxmh/html/index_login.html&client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&response_type=code&state=test
            strcpy(pValue,"https://tpass.guizhou.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.guizhou.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=https://etax.guizhou.chinatax.gov.cn/xxmh/html/index_login.html&client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&response_type=code&state=test");
            //strcpy(pValue,"");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_guizhou(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_guizhou(&hEtax->tpass, uriTpassFull,login_parameter->svr_container,login_parameter->soft_cert_ip,login_parameter->soft_cert_port,errinfo_uft8_21)))
            {
        ...
        else if(memcmp(login_parameter->login_method,"02",2) == 0)  //数字证书
        {
            //获取税盘证书与税局地址进行签名验证
            if(!atoi(login_parameter->sys_type)){
                if ((nChildRet = TPassLoginAuthHelloAndAuthGuizhou(&hEtax->tpass, uriTpassFull))) {
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            ...
            //sprintf(login_parameter->lpcode,"https://etax.guizhou.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=18169169c19c11edb15240a6b75aad54&code=%s&service=http://etax.guizhou.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
            //https://etax.guizhou.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&code=%s&service=https://etax.guizhou.chinatax.gov.cn/xxmh/html/index_login.html&state=test
            sprintf(login_parameter->lpcode,"https://etax.guizhou.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&code=%s&service=https://etax.guizhou.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
...
int etax_guizhou_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_guizhou(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_guizhou_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_guizhou(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_guizhou(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_guizhou(&hEtax->tpass, uriTpassFull,relationchange_parametetr->svr_container,relationchange_parametetr->soft_cert_ip,relationchange_parametetr->soft_cert_port,errinfo_uft8_21)))
            {
                sprintf((char *)errinfo_uft8, "%s",errinfo_uft8_21);
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.guizhou.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=f3dHd4H42d994f3H8b56a3577bHa9fb4&code=%s&service=https://etax.guizhou.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
