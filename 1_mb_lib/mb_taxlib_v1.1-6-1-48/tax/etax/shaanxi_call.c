...
int etax_shaanxi_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_shaanxi_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_shaanxi_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_shaanxi_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_shaanxi_fuction = 
{
    .etax_login = etax_shaanxi_login,
    .etax_user_query = etax_shaanxi_user_query,
    .etax_enterpriselist = etax_shaanxi_enterpriselist,
    .etax_relationchange = etax_shaanxi_relationchange
};

//自定义header
HEADER_STRUCT header_array_shaanxi[] = {
    {"Cookie", "oauth2_referer=szzhzz.shaanxi.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_shaanxi(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_shaanxi_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_shaanxi);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.shaanxi.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            //strcpy(pValue,"https://tpass.shaanxi.chinatax.gov.cn:8443/sso/login?service=https://etax.shaanxi.chinatax.gov.cn/xxmh/html/index_login.html?cdId=ae246ea402184b748a383446d67137d6&response_type=code&state=test");
            //https://tpass.shaanxi.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.shaanxi.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=https://etax.shaanxi.chinatax.gov.cn/xxmh/html/index_login.html&client_id=k8n367k7fn884nedb9bfnnff8729a4nf&response_type=code&state=test
            strcpy(pValue,"https://tpass.shaanxi.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.shaanxi.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=https://etax.shaanxi.chinatax.gov.cn/xxmh/html/index_login.html&client_id=k8n367k7fn884nedb9bfnnff8729a4nf&response_type=code&state=test");
            //strcpy(pValue,"");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_shaanxi(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_21_Msg_shaanxi(&hEtax->tpass, uriTpassFull,login_parameter->svr_container,login_parameter->soft_cert_ip,login_parameter->soft_cert_port,errinfo_uft8_21)))
            {
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            ...
            //https://etax.fujian.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=18169169c19c11edb15240a6b75aad54&code=%s&service=http://etax.fujian.chinatax.gov.cn/xxmh/html/index_login.html&state=test
            sprintf(login_parameter->lpcode,"https://etax.shaanxi.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=k8n367k7fn884nedb9bfnnff8729a4nf&code=%s&service=https://etax.shaanxi.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
...
int etax_shaanxi_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_shaanxi(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_shaanxi_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_shaanxi(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_shaanxi(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        {
            pValue = calloc(1,BS_BLOCK*2);  //b62deMdW365e4dfba3W63bab7dW77be4
            strcpy(pValue,"https://tpass.shaanxi.chinatax.gov.cn:8443/login?redirect_uri=https://etax.shaanxi.chinatax.gov.cn/zjgfdzswj/main/kx/skip.html?service=https://etax.shaanxi.chinatax.gov.cn/zjgfdzswj/main/home/wdxx/index.html&client_id=xdf6a5cd4cx74bfca46x3bbfcxa4c4xf&response_type=code&state=test");
            printf("lbc-debug pValue = %s\n",pValue);
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_21_Msg_shaanxi(&hEtax->tpass, uriTpassFull,relationchange_parametetr->svr_container,relationchange_parametetr->soft_cert_ip,relationchange_parametetr->soft_cert_port,errinfo_uft8_21)))
            {
                sprintf((char *)errinfo_uft8, "%s",errinfo_uft8_21);
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.shaanxi.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=k8n367k7fn884nedb9bfnnff8729a4nf&code=%s&service=https://etax.shaanxi.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
