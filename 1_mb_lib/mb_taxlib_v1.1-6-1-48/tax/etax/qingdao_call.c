...
int etax_qingdao_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_qingdao_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_qingdao_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_qingdao_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_qingdao_fuction = 
{
    .etax_login = etax_qingdao_login,
    .etax_user_query = etax_qingdao_user_query,
    .etax_enterpriselist = etax_qingdao_enterpriselist,
    .etax_relationchange = etax_qingdao_relationchange
};

//自定义header
HEADER_STRUCT header_array_qingdao[] = {
    {"Cookie", "oauth2_referer=szzhzz.qingdao.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_qingdao(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_qingdao_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_qingdao);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.qingdao.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            strcpy(pValue,"https://tpass.qingdao.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https%3A%2F%2Fetax.qingdao.chinatax.gov.cn%3A7553%2Fsso%2Flogin%3Fservice%3Dhttps%3A%2F%2Fetax.qingdao.chinatax.gov.cn%2Fportal%2F%26qtxtlx%3Dkxlogin&client_id=c6dc3eJecf7a445495Jdf4fb6ab38ec5&response_type=code");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_qingdao(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            //https://etax.qingdao.chinatax.gov.cn:7553/sso/login?service=https://etax.qingdao.chinatax.gov.cn/portal/&qtxtlx=kxlogin&client_id=c6dc3eJecf7a445495Jdf4fb6ab38ec5&code=
            sprintf(login_parameter->lpcode,"https://etax.qingdao.chinatax.gov.cn:7553/sso/login?service=https://etax.qingdao.chinatax.gov.cn/portal/&qtxtlx=kxlogin&client_id=c6dc3eJecf7a445495Jdf4fb6ab38ec5&code=%s",objItem->valuestring);
        }
        else                //票
        {
...
int etax_qingdao_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_qingdao(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_qingdao_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_qingdao(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_qingdao(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.qingdao.chinatax.gov.cn:7553/sso/login?service=https://etax.qingdao.chinatax.gov.cn/portal/&qtxtlx=kxlogin&client_id=c6dc3eJecf7a445495Jdf4fb6ab38ec5&code=%s",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
