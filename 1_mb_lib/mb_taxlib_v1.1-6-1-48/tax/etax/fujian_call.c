...
int etax_fujian_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_fujian_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_fujian_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_fujian_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_fujian_fuction = 
{
    .etax_login = etax_fujian_login,
    .etax_user_query = etax_fujian_user_query,
    .etax_enterpriselist = etax_fujian_enterpriselist,
    .etax_relationchange = etax_fujian_relationchange
};

//自定义header
HEADER_STRUCT header_array_fujian[] = {
    {"Cookie", "oauth2_referer=szzhzz.fujian.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_fujian(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_fujian_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_fujian);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.fujian.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            strcpy(pValue,"https://tpass.fujian.chinatax.gov.cn:8443/login?redirect_uri=https://etax.fujian.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=http://etax.fujian.chinatax.gov.cn/xxmh/html/index_login.html&client_id=18169169c19c11edb15240a6b75aad54&response_type=code&state=test");
            //strcpy(pValue,"");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_fujian(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            ...
            //sprintf(login_parameter->lpcode,"https://etax.fujian.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=18169169c19c11edb15240a6b75aad54&code=%s&service=http://etax.fujian.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
            sprintf(login_parameter->lpcode,"https://etax.fujian.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=18169169c19c11edb15240a6b75aad54&code=%s&service=http://etax.fujian.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
...
int etax_fujian_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_fujian(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_fujian_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_fujian(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_fujian(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.fujian.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=18169169c19c11edb15240a6b75aad54&code=%s&service=http://etax.fujian.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
