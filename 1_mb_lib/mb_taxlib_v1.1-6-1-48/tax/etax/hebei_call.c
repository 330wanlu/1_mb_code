...
int etax_hebei_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hebei_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hebei_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hebei_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_hebei_fuction = 
{
    .etax_login = etax_hebei_login,
    .etax_user_query = etax_hebei_user_query,
    .etax_enterpriselist = etax_hebei_enterpriselist,
    .etax_relationchange = etax_hebei_relationchange
};

//自定义header
HEADER_STRUCT header_array_hebei[] = {
    {"Cookie", "oauth2_referer=szzhzz.hebei.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_hebei(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_hebei_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_hebei);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.hebei.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            //https://etax.hebei.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&response_type=code&state=test&code=A440D06B87B2470992D4D1AE514C6819
            strcpy(pValue,"https://tpass.hebei.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/interfaceHideLogout?redirect_uri=https://etax.hebei.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&client_id=r4aebra24rd54u5395r447acb7d9r829&response_type=code&state=test");
            //strcpy(pValue,"https://tpass.hebei.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?response_type=code&client_id=r4aebra24rd54u5395r447acb7d9r829");
            //strcpy(pValue,"");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_hebei(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            ...
            //sprintf(login_parameter->lpcode,"https://etax.hebei.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?client_id=18169169c19c11edb15240a6b75aad54&code=%s&service=http://etax.hebei.chinatax.gov.cn/xxmh/html/index_login.html&state=test",objItem->valuestring);
            sprintf(login_parameter->lpcode,"https://etax.hebei.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&response_type=code&state=test&code=%s",objItem->valuestring);
        ...
        else                //票
        {
            ...
            {
                //sprintf(login_parameter->lpcode,"https://etax.hebei.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&response_type=code&state=test&code=%s",objItem->valuestring);
                sprintf(login_parameter->lpcode,"https://etax.hebei.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&code=%s&state=test",objItem->valuestring);
            }
            else
            {
...
int etax_hebei_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_hebei(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_hebei_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        tem_type = 1;
        relationchange_parametetr->sys_type = "1";
    }
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_hebei(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_hebei(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        {
            pValue = calloc(1,BS_BLOCK*2);  //b62deMdW365e4dfba3W63bab7dW77be4
            strcpy(pValue,"https://tpass.hebei.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.hebei.chinatax.gov.cn/KxsfrzAction.do?activity=login&client_id=s4ce78569sbs47w3b8bw38D56a3D5cc2&response_type=code&state=test");
            printf("lbc-debug pValue = %s\n",pValue);
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.hebei.chinatax.gov.cn/KxsfrzAction.do?activity=login&client_id=s4ce78569sbs47w3b8bw38D56a3D5cc2&code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(tem_type == 1)
            {
                //sprintf(relationchange_parametetr->lpcode,"https://etax.hebei.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&response_type=code&state=test&code=%s",objItem->valuestring);
                sprintf(relationchange_parametetr->lpcode,"https://etax.hebei.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&code=%s&state=test",objItem->valuestring);
