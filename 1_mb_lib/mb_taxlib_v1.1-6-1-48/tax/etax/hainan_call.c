...
int etax_hainan_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hainan_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hainan_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hainan_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_hainan_fuction = 
{
    .etax_login = etax_hainan_login,
    .etax_user_query = etax_hainan_user_query,
    .etax_enterpriselist = etax_hainan_enterpriselist,
    .etax_relationchange = etax_hainan_relationchange
};

//自定义header
HEADER_STRUCT header_array_hainan[] = {
    {"Cookie", "oauth2_referer=szzhzz.hainan.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_hainan(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_hainan_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_hainan);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.hainan.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            //https://tpass.hainan.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.hainan.chinatax.gov.cn/zjgfdzswj/main/kx/skip.html?service=https://etax.hainan.chinatax.gov.cn/zjgfdzswj/main/home/wybs/index.html&client_id=jUbj6a85a7a44azb8j3U8de9abzeb57a&response_type=code&state=test
            strcpy(pValue,"https://tpass.hainan.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.hainan.chinatax.gov.cn/zjgfdzswj/main/kx/skip.html?service=https://etax.hainan.chinatax.gov.cn/zjgfdzswj/main/home/wybs/index.html&client_id=jUbj6a85a7a44azb8j3U8de9abzeb57a&response_type=code&state=test");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_hainan(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            ...
            sprintf(login_parameter->lpcode,"https://etax.hainan.chinatax.gov.cn/zjgfdzswj/main/kx/skip.html?service=https://etax.hainan.chinatax.gov.cn/zjgfdzswj/main/home/wybs/index.html&client_id=jUbj6a85a7a44azb8j3U8de9abzeb57a&code=%s&state=test",objItem->valuestring);
...
int etax_hainan_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_hainan(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_hainan_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_hainan(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_hainan(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.hainan.chinatax.gov.cn/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666&code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
