...
int etax_hunan_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hunan_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hunan_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_hunan_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_hunan_fuction = 
{
    .etax_login = etax_hunan_login,
    .etax_user_query = etax_hunan_user_query,
    .etax_enterpriselist = etax_hunan_enterpriselist,
    .etax_relationchange = etax_hunan_relationchange
};

//自定义header
HEADER_STRUCT header_array_hunan[] = {
    {"Cookie", "oauth2_referer=szzhzz.hunan.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_hunan(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_hunan_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    if(!atoi(login_parameter->sys_type))  //默认税
    {
        ...
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://etax.hunan.chinatax.gov.cn/");
        memset(hEtax->etax.modelTaAddr.szInitRequestPage,0x00,sizeof(hEtax->etax.modelTaAddr.szInitRequestPage));
        strcpy(hEtax->etax.modelTaAddr.szInitRequestPage,"/wsbs/toLogin.do");
    }
    else                 //票
    {
        hEtax->etax.user_args = (void *)(&header_array_hunan);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.hunan.chinatax.gov.cn:8443");
        memset(hEtax->etax.modelTaAddr.szInitRequestPage,0x00,sizeof(hEtax->etax.modelTaAddr.szInitRequestPage));
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            pValue = calloc(1,BS_BLOCK*2);  //b62deMdW365e4dfba3W63bab7dW77be4
                        // https://tpass.hunan.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.hunan.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test
            strcpy(pValue,"https://tpass.hunan.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.hunan.chinatax.gov.cn/wsbs/toLogin.do&client_id=uau7c23c98e347ab8452c2u486bbf53e&response_type=code&state=test");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_hunan(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            //https://etax.hunan.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test
            sprintf(login_parameter->lpcode,"https://etax.hunan.chinatax.gov.cn/wsbs/toLogin.do?code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
...
int etax_hunan_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        else
        {
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_hunan(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        //sprintf(relationchange_parametetr->lpcode,"https://etax.hunan.chinatax.gov.cn/web/dzswj/kxyh/loginyw.html?code=%s&state=",objItem->valuestring);
        
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            sprintf(relationchange_parametetr->lpcode,"https://etax.hunan.chinatax.gov.cn/wsbs/toLogin.do?code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
