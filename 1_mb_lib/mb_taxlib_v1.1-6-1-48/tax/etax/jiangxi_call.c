...
int etax_jiangxi_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_jiangxi_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_jiangxi_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_jiangxi_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_jiangxi_fuction = 
{
    .etax_login = etax_jiangxi_login,
    .etax_user_query = etax_jiangxi_user_query,
    .etax_enterpriselist = etax_jiangxi_enterpriselist,
    .etax_relationchange = etax_jiangxi_relationchange
};

//自定义header
HEADER_STRUCT header_array_jiangxi[] = {
    {"Cookie", "oauth2_referer=szzhzz.jiangxi.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_jiangxi(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_jiangxi_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_jiangxi);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.jiangxi.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            //https://tpass.jiangxi.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.jiangxi.chinatax.gov.cn/sso/login?service=https://etax.jiangxi.chinatax.gov.cn/xxmh/html/index_login.html&v=2&client_id=f91863cc09c75f6b881b8f9953035b6f&response_type=code&state=test
            strcpy(pValue,"https://tpass.jiangxi.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.jiangxi.chinatax.gov.cn/etax/jsp/do_tpass_login_initData.jsp&response_type=code&state=test&client_id=s699fxqa48q647bda4x6bxafcxscqe62");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_jiangxi(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            //https://etax.jiangxi.chinatax.gov.cn/sso/login?client_id=f91863cc09c75f6b881b8f9953035b6f&code=%s&service=https://etax.jiangxi.chinatax.gov.cn/xxmh/html/index_login.html&state=test&v=2
            //https://etax.jiangxi.chinatax.gov.cn/sso/login?client_id=f91863cc09c75f6b881b8f9953035b6f&code=7B842D4B46274918BED8A5732B9DBF11&service=https://etax.jiangxi.chinatax.gov.cn/xxmh/html/index_login.html&state=test&v=2
            sprintf(login_parameter->lpcode,"https://etax.jiangxi.chinatax.gov.cn/etax/jsp/do_tpass_login_initData.jsp?code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
...
int etax_jiangxi_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_jiangxi(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_jiangxi_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_jiangxi(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_jiangxi(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        {
            pValue = calloc(1,BS_BLOCK*2);  //b62deMdW365e4dfba3W63bab7dW77be4
            //strcpy(pValue,"https://tpass.jiangxi.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https%3A%2F%2Fetax.jiangxi.chinatax.gov.cn%3A7553%2Fsso%2Flogin%3Fservice%3Dhttps%3A%2F%2Fetax.jiangxi.chinatax.gov.cn%2Fportal%2F%26qtxtlx%3Dkxlogin&client_id=c6dc3eJecf7a445495Jdf4fb6ab38ec5&response_type=code");
            //strcpy(pValue,"https://tpass.jiangxi.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.jiangxi.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test");
            strcpy(pValue,"https://tpass.jiangxi.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.jiangxi.chinatax.gov.cn/etax/jsp/do_tpass_login_initData.jsp&response_type=code&state=test&client_id=s699fxqa48q647bda4x6bxafcxscqe62");
            printf("lbc-debug pValue = %s\n",pValue);
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            //sprintf(relationchange_parametetr->lpcode,"https://etax.jiangxi.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?code=%s",objItem->valuestring);
            sprintf(relationchange_parametetr->lpcode,"https://etax.jiangxi.chinatax.gov.cn/etax/jsp/do_tpass_login_initData.jsp?code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
