...
int etax_beijing_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_beijing_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_beijing_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_beijing_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_beijing_fuction = 
{
    .etax_login = etax_beijing_login,
    .etax_user_query = etax_beijing_user_query,
    .etax_enterpriselist = etax_beijing_enterpriselist,
    .etax_relationchange = etax_beijing_relationchange
};

//自定义header
HEADER_STRUCT header_array_beijing[] = {
    {"Cookie", "oauth2_referer=szzhzz.beijing.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_beijing(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_beijing_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_beijing);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.beijing.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            pValue = calloc(1,BS_BLOCK*2);  //b62deMdW365e4dfba3W63bab7dW77be4
                        // https://tpass.beijing.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.beijing.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test
            strcpy(pValue,"https://tpass.beijing.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.beijing.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_beijing(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        /*if(hEtax->tpass.user_args)  //zwl 0625
        {
            printf("lbc-debug");
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            //https://etax.beijing.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test
            sprintf(login_parameter->lpcode,"https://etax.beijing.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?code=%s",objItem->valuestring);
        }
        else                //票
        {
...
int etax_beijing_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_beijing(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
...
int etax_beijing_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_beijing(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_beijing(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        {
            pValue = calloc(1,BS_BLOCK*2);  //b62deMdW365e4dfba3W63bab7dW77be4
            //strcpy(pValue,"https://tpass.beijing.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https%3A%2F%2Fetax.beijing.chinatax.gov.cn%3A7553%2Fsso%2Flogin%3Fservice%3Dhttps%3A%2F%2Fetax.beijing.chinatax.gov.cn%2Fportal%2F%26qtxtlx%3Dkxlogin&client_id=c6dc3eJecf7a445495Jdf4fb6ab38ec5&response_type=code");
            strcpy(pValue,"https://tpass.beijing.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?redirect_uri=https://etax.beijing.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do&client_id=dcR7baR2cd6f428R8b7f9dfRRfc6bRd6&response_type=code&state=test");
            printf("lbc-debug pValue = %s\n",pValue);
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.beijing.chinatax.gov.cn/kxsfrz-cjpt-web/tpass/tpassLogin.do?code=%s",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
