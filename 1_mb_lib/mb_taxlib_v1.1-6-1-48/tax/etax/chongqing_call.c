...
int etax_chongqing_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_chongqing_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_chongqing_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_chongqing_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_chongqing_fuction = 
{
    .etax_login = etax_chongqing_login,
    .etax_user_query = etax_chongqing_user_query,
    .etax_enterpriselist = etax_chongqing_enterpriselist,
    .etax_relationchange = etax_chongqing_relationchange
};

//自定义header
HEADER_STRUCT header_array_chongqing[] = {
    {"Cookie", "oauth2_referer=szzhzz.chongqing.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
//获取tpass地址
int get_tpass_addr_chongqing(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_chongqing_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_chongqing);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.chongqing.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            //strcpy(pValue,"https://tpass.chongqing.chinatax.gov.cn:8443/#/login?client_id=CfcG3KCCGeG44e7faC374Cb5bK3GC3d4&response_type=code&state=test&redirect_uri=https%3A%2F%2Fetax.chongqing.chinatax.gov.cn%2Fsword%3Fctrl%3DKxLoginCtrl_initNsrxx");
            strcpy(pValue,"https://tpass.chongqing.chinatax.gov.cn:8443/api/v1.0/auth/oauth2/login?client_id=CfcG3KCCGeG44e7faC374Cb5bK3GC3d4&response_type=code&state=test&redirect_uri=https%3A%2F%2Fetax.chongqing.chinatax.gov.cn%2Fsword%3Fctrl%3DKxLoginCtrl_initNsrxx");
            ...
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_chongqing(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_chongqing(&hEtax->tpass, uriTpassFull,login_parameter->svr_container,login_parameter->soft_cert_ip,login_parameter->soft_cert_port)))
            {
        ...
        else if(memcmp(login_parameter->login_method,"02",2) == 0)  //数字证书
        {
            ...
            if(!atoi(login_parameter->sys_type)) 
            {
                if ((nChildRet = TPassLoginAuthHelloAndAuthChongQing(&hEtax->tpass, uriTpassFull))) {
        ...
        {
            //
            //sprintf(login_parameter->lpcode,"https://etax.chongqing.chinatax.gov.cn:7553/sso/login?service=https://etax.chongqing.chinatax.gov.cn/portal/&qtxtlx=kxlogin&client_id=c6dc3eJecf7a445495Jdf4fb6ab38ec5&code=%s",objItem->valuestring);
            sprintf(login_parameter->lpcode,"https://etax.chongqing.chinatax.gov.cn/sword?ctrl=KxLoginCtrl_initNsrxx&client_id=CfcG3KCCGeG44e7faC374Cb5bK3GC3d4&code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
...
int etax_chongqing_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        //获取跳转tpass地址
        //获取跳转tpass地址
        if(RET_SUCCESS != get_tpass_addr_chongqing(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        }
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_chongqing(&hEtax->tpass, uriTpassFull,user_query_parameter->svr_container,user_query_parameter->soft_cert_ip,user_query_parameter->soft_cert_port)))
            {
                sprintf((char *)errinfo_uft8, "102税盘已离线或是登陆参数有误");
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
...
int etax_chongqing_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_chongqing(&hEtax->tpass, uriTpassFull,enterpriselist_parameter->svr_container,enterpriselist_parameter->soft_cert_ip,enterpriselist_parameter->soft_cert_port)))
            {
                sprintf((char *)errinfo_uft8, "102税盘已离线或是登陆参数有误");
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
        ...
        printf("enterpriselist_parameter->outenterpriselist = %d\r\n",sizeof(enterpriselist_parameter->outenterpriselist));
        //结束
        nRet = RET_SUCCESS;
    }while (false);
...
int etax_chongqing_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        /*//获取跳转tpass地址
        //if(RET_SUCCESS != get_tpass_addr_chongqing(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
        if(RET_SUCCESS != get_tpass_addr_chongqing(szTPassConfigJson,&pValue,"1"))
        {
             sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
             break;
        ...
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_chongqing(&hEtax->tpass, uriTpassFull,relationchange_parametetr->svr_container,relationchange_parametetr->soft_cert_ip,relationchange_parametetr->soft_cert_port)))
            {
                sprintf((char *)errinfo_uft8, "102税盘已离线或是登陆参数有误");
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            ...
            sprintf(relationchange_parametetr->lpcode,"https://etax.chongqing.chinatax.gov.cn/sword?ctrl=KxLoginCtrl_initNsrxx&client_id=CfcG3KCCGeG44e7faC374Cb5bK3GC3d4&code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
