/*****************************************************************************
File name:   shanxi_call.c
Description: 用于山西省调用etax接口
Author:      Wang
Version:     1.0
Date:        2023.02
History:
...
int etax_shanxi_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_shanxi_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_shanxi_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);
int etax_shanxi_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8);

ETAX_FUCTION etax_shanxi_fuction = 
{
    .etax_login = etax_shanxi_login,
    .etax_user_query = etax_shanxi_user_query,
    .etax_enterpriselist = etax_shanxi_enterpriselist,
    .etax_relationchange = etax_shanxi_relationchange
};

//自定义header
HEADER_STRUCT header_array_shanxi[] = {
    {"Cookie", "oauth2_referer=szzhzz.shanxi.chinatax.gov.cn; x_host_key=188715edd92-73398ffbd187ed688673e9dcd9a219287ac81e5c"},
...
int get_tpass_addr_shanxi(char *src_data,char **tpass_addr,char *sys_type)
{
...
int etax_shanxi_login(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    {
        hEtax->etax.user_args = (void *)(&header_array_shanxi);
        memset(hEtax->etax.modelTaAddr.szTaxAuthorityURL,0x00,sizeof(hEtax->etax.modelTaAddr.szTaxAuthorityURL));
        strcpy(hEtax->etax.modelTaAddr.szTaxAuthorityURL,"https://dppt.shanxi.chinatax.gov.cn:8443");
    ...
    do{
        ...
        if(!atoi(login_parameter->sys_type)) 
        {
            ...
            strcpy(tpassurl,"https://tpass.shanxi.chinatax.gov.cn:8443/#/login?redirect_uri=https://etax.shanxi.chinatax.gov.cn/tpass/sso/loginSubmit&client_id=d74f6Xacf6434c36acbepe4e6fpbcf8f&response_type=code&state=test");
        
            if (!(uriTpassFull = evhttp_uri_parse(tpassurl))){
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址转换失败"); 
                break;
            }

            if (!FillUploadAddrModelDynamic(TAADDR_ETAX_TPASS, hDev->szRegCode, "https://tpass.shanxi.chinatax.gov.cn:8443", "/",&hEtax->tpass.modelTaAddr)) {
        ...
        else
        {
            ...
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_shanxi(szTPassConfigJson,&pValue,login_parameter->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        //(CtxBridge)hEtax->tpass.ctxApp->szRandom16BytesKey
        ...
        if(memcmp(login_parameter->login_method,"21",2) == 0)  //软证书    "172.30.92.124",18001  "192.168.0.247",18005   //login_parameter->soft_cert_ip,login_parameter->soft_cert_port
        {
            //TPassLoginAuthHelloAndAuth_soft_cert_shanxi(&hEtax->tpass, uriTpassFull,login_parameter->svr_container,login_parameter->soft_cert_ip,login_parameter->soft_cert_port);
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_shanxi(&hEtax->tpass, uriTpassFull,login_parameter->svr_container,login_parameter->soft_cert_ip,login_parameter->soft_cert_port,errinfo_uft8_21)))
            {
        ...
        {
            if(!atoi(login_parameter->sys_type)){
                if ((nChildRet = TPassLoginAuthHelloAndAuthShanXi(&hEtax->tpass, uriTpassFull))) {
        ...
        //步骤5 根据筛选出来的身份进行登陆
        if(!atoi(login_parameter->sys_type)){
            if (nChildRet = TPassLoginPasswordLoginDoShanXi(&hEtax->tpass, szUserId, login_parameter->sz_password, szBuf,sizeof(szBuf))){
        ...
        if(!atoi(login_parameter->sys_type))  //默认税
        {
            sprintf(login_parameter->lpcode,"https://etax.shanxi.chinatax.gov.cn/tpass/sso/loginSubmit?code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
        ...
        //strcpy(login_parameter->lptoken, tem_key); 
...
int etax_shanxi_user_query(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        if(memcmp(user_query_parameter->login_method,"21",2) == 0)  //软证书    "172.30.92.124",18001  "192.168.0.247",18002   //login_parameter->soft_cert_ip,login_parameter->soft_cert_port
        {
            //TPassLoginAuthHelloAndAuth_soft_cert_shanxi(&hEtax->tpass, uriTpassFull,user_query_parameter->svr_container,user_query_parameter->soft_cert_ip,user_query_parameter->soft_cert_port);
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_shanxi(&hEtax->tpass, uriTpassFull,user_query_parameter->svr_container,user_query_parameter->soft_cert_ip,user_query_parameter->soft_cert_port,errinfo_uft8_21)))
            {
                sprintf((char *)errinfo_uft8, "%s",errinfo_uft8_21);
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
        ...
        else if(memcmp(user_query_parameter->login_method,"02",2) == 0)
        {
            if (nChildRet = TPassLoginAuthHelloAndAuthShanXi(&hEtax->tpass, uriTpassFull)) {
                sprintf((char *)errinfo_uft8, "406签名验签服务器响应请求错误:%s",(const char *)evbuffer_pullup(hEtax->tpass.bufHttpRep, -1));
                break;
...
int etax_shanxi_enterpriselist(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        if(memcmp(enterpriselist_parameter->login_method,"21",2) == 0)  //软证书    "172.30.92.124",18001  "192.168.0.247",18002   //login_parameter->soft_cert_ip,login_parameter->soft_cert_port
        {
            //TPassLoginAuthHelloAndAuth_soft_cert_shanxi(&hEtax->tpass, uriTpassFull,enterpriselist_parameter->svr_container,enterpriselist_parameter->soft_cert_ip,enterpriselist_parameter->soft_cert_port);
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_shanxi(&hEtax->tpass, uriTpassFull,enterpriselist_parameter->svr_container,enterpriselist_parameter->soft_cert_ip,enterpriselist_parameter->soft_cert_port,errinfo_uft8_21)))
            {
                sprintf((char *)errinfo_uft8, "%s",errinfo_uft8_21);
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
        ...
        else if(memcmp(enterpriselist_parameter->login_method,"02",2) == 0)
        {
            if ((nChildRet = TPassLoginAuthHelloAndAuthShanXi(&hEtax->tpass, uriTpassFull))) {
                sprintf((char *)errinfo_uft8, "406签名验签服务器响应请求错误:%s",(const char *)evbuffer_pullup(hEtax->tpass.bufHttpRep, -1));
                break;
        ...
        nCount = cJSON_GetArraySize(objItem);
        printf("6666666666666666666666666666666666666666666         nCount = [%d]\r\n",nCount);
        for (i = 0; i < nCount; i++) {
            objItemChild = cJSON_GetArrayItem(objItem, i);
        ...
        /*for (i = 0; i < nCount; i++) {  // 测试
            ...
            if(strlen(enterpriselist_parameter->idcard)){
                if (!(objItemChild2 = cJSON_GetObjectItem(objItemChild, "id_card")))
                    continue;
                if (strcmp(objItemChild2->valuestring, enterpriselist_parameter->idcard)){
                    printf("sssssssssssss1sssssssssss  objItemChild2->valuestring = %s  enterpriselist_parameter->idcard = %s\r\n",objItemChild2->valuestring,enterpriselist_parameter->idcard);
                    continue;}
            }
            ...
            if(strlen(enterpriselist_parameter->mobile)){
                if (!(objItemChild2 = cJSON_GetObjectItem(objItemChild, "mobile")))
                    continue;
                if (strcmp(objItemChild2->valuestring, enterpriselist_parameter->mobile)){
                    printf("ssssssssssss2ssssssssss   objItemChild2->valuestring = %s  enterpriselist_parameter->mobile = %s\r\n",objItemChild2->valuestring,enterpriselist_parameter->mobile);
                    continue;}
            }

            if (!(objItemChild2 = cJSON_GetObjectItem(objItemChild, "relatedType")))
                continue;
            if (strcmp(objItemChild2->valuestring, enterpriselist_parameter->related_type)){
                printf("sssssssssssss3sssssssss  objItemChild2->valuestring = %s  enterpriselist_parameter->related_type = %s\r\n",objItemChild2->valuestring, enterpriselist_parameter->related_type);
            //if (strcmp(objItemChild2->valuestring, "03"))
                continue;}
                
            if (!(objItemChild2 = cJSON_GetObjectItem(objItemChild, "user_id"))){
                //printf("ssssssssssssssssssssss  enterpriselist_parameter->user_id = %s\r\n",enterpriselist_parameter->user_id);
                continue;}
            strcpy(szUserId, objItemChild2->valuestring);
...
int etax_shanxi_relationchange(HETAX hEtax, HDEV hDev,void *args,char *errinfo_uft8)
{
    ...
    do{
        ...
        else
        {
            szTPassConfigJson = (char *)hEtax->etax.user_args;
            //获取跳转tpass地址
            if(RET_SUCCESS != get_tpass_addr_shanxi(szTPassConfigJson,&pValue,relationchange_parametetr->sys_type))
            {
                sprintf((char *)errinfo_uft8, "406获取跳转地址错误,地址解析失败");
                break;
            }
        ...
        if(memcmp(relationchange_parametetr->login_method,"21",2) == 0)  //软证书    "172.30.92.124",18001  "192.168.0.247",18002   //login_parameter->soft_cert_ip,login_parameter->soft_cert_port
        {
            //TPassLoginAuthHelloAndAuth_soft_cert_shanxi(&hEtax->tpass, uriTpassFull,relationchange_parametetr->svr_container,relationchange_parametetr->soft_cert_ip,relationchange_parametetr->soft_cert_port);
            if ((nChildRet = TPassLoginAuthHelloAndAuth_soft_cert_shanxi(&hEtax->tpass, uriTpassFull,relationchange_parametetr->svr_container,relationchange_parametetr->soft_cert_ip,relationchange_parametetr->soft_cert_port,errinfo_uft8_21)))
            {
                sprintf((char *)errinfo_uft8, "%s",errinfo_uft8_21);
                printf("lbc-debug nChildRet=%d        nRet = %d     \n",nChildRet,nRet);
                break;
        ...
        //步骤5 根据筛选出来的身份进行登陆
        if(!atoi(relationchange_parametetr->sys_type)){
            if (nChildRet = TPassLoginPasswordLoginDoShanXi(&hEtax->tpass, szUserId, relationchange_parametetr->sz_password, szBuf,sizeof(szBuf))){
                if (!(root = cJSON_Parse(szBuf)))
                    break;
                if (!(objItem = cJSON_GetObjectItem(root, "msg")))
                    break;
        ...
        //步骤6 企业切换
        printf("             relationchange_parametetr->uniqueIdentity   = [%s]     relationchange_parametetr->realationStatus =[%s]   \r\n",relationchange_parametetr->uniqueIdentity,relationchange_parametetr->realationStatus);
        ...
        if(!atoi(relationchange_parametetr->sys_type))  //默认税
        {
            sprintf(relationchange_parametetr->lpcode,"https://etax.shanxi.chinatax.gov.cn/tpass/sso/loginSubmit?code=%s&state=test",objItem->valuestring);
        }
        else                //票
        {
            if(0 > get_final_url(pValue,objItem->valuestring,final_url))
