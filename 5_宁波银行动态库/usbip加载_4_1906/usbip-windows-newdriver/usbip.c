...
// static int query_interface_8009(SOCKET sockfd, char * busid, struct usb_interface * uinf0,int trans_type,int  dev_port)
// {
// 	int ret;
// 	char sendbuf[50] = {0};
// 	char head[50] = {0};
...
// static int attach_device(char * host,char * port ,char *random,int trans_type,int dev_port,int *sockfd)
// {
// 	//SOCKET sockfd;
// 	int rhport,result = -1;
...
// 		ret = apply_for_port_use(*sockfd,dev_port,host,port,random);//公网转发申请端口使用
// 		if(ret != 0)
// 		{
// 			close_sock(*sockfd);
...
// 		ret = query_interface_8009(*sockfd, bus_id, &uinf, trans_type,dev_port);
// 		if(ret <0)
// 		{
// 			if(ret == -6)//未授权
...
// 	result = usbip_vbus_detach_device(devfd,rhport);
...
static int attach_device(char * host,char * port ,char *random,int trans_type,int dev_port,int *sockfd,char *oper_id,int *virtual_port)
{
	...
	{
		WriteLogFile("query_import_device 1\n");
	...
	WriteLogFile("query_import_device 2\n");
	rhport = query_import_device(*sockfd, bus_id, &uinf, &devfd);
	if (rhport < 0) {
		WriteLogFile("err query");
		close_sock(*sockfd);
...
// int attach_usbip(char * host, int serv_port, int dev_port, int trans_type,char *user,int *sockfd)
// {
// 	int ret = 0;
...
// 	ret = attach_device(host, ser_port_str, user, trans_type, dev_port,sockfd);
