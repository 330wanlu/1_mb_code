...
DWORD WINAPI Deal_Thread(LPVOID lpParam)
{
	...
	//result = attach_usbip(info->server_ip, info->server_port, info->dev_port, info->trans_type, info->user,&info->sockfd);//阻塞调用
...
EXPORT_DLL int init_dll_interface()
{
	...
	// WriteLogFile("init________________1________________");
	// if (1 == dll_inint_flag)
	// {
	// 	WriteLogFile("已初始化完成");
	// 	return 0;
	// }
	// WriteLogFile("init________________2________________");
	...
	{
		WriteLogFile("usbip already start!!!!!!!!!!!!!!");
	}
	return 0;
}
...
{
	WriteLogFile("!!!!!!!!!!!lbc get_init_flag = %d",dll_inint_flag);
	return dll_inint_flag;
}
...
static int open_port(cJSON *root,result_CallBack callback)
{
	...
	for (point = 0; point < 20; point++)
		...
		WriteLogFile("___________get server_port %s\n",item->valuestring);
		...
		memcpy(line_info[point].ter_id, item->valuestring, strlen(item->valuestring));
		WriteLogFile("___________get ter_id %s\n",item->valuestring);
		item = cJSON_GetObjectItem(root, "user");
		if (item == NULL)  //为NULL，默认为 ”anyone“
		{
			// WriteLogFile("get user error!");
			...
			memcpy(line_info[point].user, item->valuestring, strlen(item->valuestring));
		WriteLogFile("___________get user: %s\n",item->valuestring);
		...
		WriteLogFile("___________get oper_id: %s\n",line_info[point].oper_id);
		item = cJSON_GetObjectItem(root, "trans_type");
		if (item == NULL)  //没有trans_type 时，默认为0
		{
			WriteLogFile("get trans_type error!");
		...
		{
			WriteLogFile("55555555555555555\r\n");
			if(item->type==cJSON_String)
				line_info[point].trans_type = atoi(item->valuestring);
			else if(item->type==cJSON_Number)
				line_info[point].trans_type = item->valueint;
		...
		WriteLogFile("___________get trans_type: %d\n",line_info[point].trans_type);
		for(i = 0; i < 20; i++)
		{
			if(line_info[i].use_flag == 1 &&line_info[i].thread_flag == 1 && point != i)//当前链路正在被使用
			{
...
static int forced_close_port(cJSON *root)
{
	...
	WriteLogFile("_____3_____forced_close_port:server_ip:%s,server_port = %d,usb_port:%d!",server_ip,server_port,usb_port);
	...
	//WriteLogFile("________________________g_buf = %s\r\n",g_buf);
	...
	WriteLogFile("----------------------------------forced_close_port-------------------------------------\r\n");
	free(g_buf);
	cJSON_Delete(json);
	Sleep(500);
	shutdown(tmp_socket,SD_BOTH);
...
//	result_CallBack callback;
	...
	{
		WriteLogFile("	______1______usbip _____ close_port \n");
	...
	{
		WriteLogFile("	______2______usbip _____ forced_close_port \n");
