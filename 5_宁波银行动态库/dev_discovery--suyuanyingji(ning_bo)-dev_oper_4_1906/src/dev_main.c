...
int apply_port(unsigned char *inbuff)
{
	...
	if(result != 1)
	{
		WriteLogFile("apply_port,端口不存在，port = %d\n",port);
		cJSON_Delete(root);
		return -1;
	}
	...
	for(i = 0;i<128;++i)
	{
		if(0 == memcmp(cabinet[i].ca_TerminalID,ter_id,strlen(ter_id)))  //找到对应的机器id
		{
			WriteLogFile("enetr apply_port,找到对应id = %s",cabinet[i].ca_TerminalID);
			break;
		}
	}
	...
	//Sleep(20*1000);
	//hDLL=LoadLibrary("ModelDll\\ModelApiDll\\usbip.dll");//加载动态链接库MyDll.dll文件；
...
// 	WriteLogFile("______________________________________________enetr 强 relese_port");
...
int judge_port_exit(char *terid,int port_data)
{
	...
	*(strrchr(set_path,'\\')+1) = 0;
	...
	for(i = 0;i<128;++i)
	{
		if(0 == memcmp(cabinet[i].ca_TerminalID,ter_id,strlen(ter_id)))  //找到对应的机器id
		{
			WriteLogFile("enetr get_info_with_terID,找到对应id = %s",cabinet[i].ca_TerminalID);
			break;
		}
	}
	if(i == 128)
	{
		...
		if(NULL != ter_info)
			free(ter_info);
		WriteLogFile("get_info_with_terID 未找到对应id");
		return -1;
	}
	result = read_setting(file_setting, 128);
	if(result < 0)
	...
	hDLL=LoadLibrary(set_path);//加载动态链接库MyDll.dll文件；
...
BOOL thrans_task_init(void)
{
	...
	iMode = 1;
	ioctlsocket(server_fd, FIONBIO, (u_long FAR*)&iMode); //设置为非阻塞模式
...
EXPORT_DLL int clean_dll_interface()
{
	...
	FreeLibrary(usbip_hDLL);//卸载MyDll.dll文件；
...
int get_ter_all_info(char *json_in, void *hook,char *output)
{
	...
	WriteLogFile("dev_info 获取机柜信息准备_______!");
	...
	if (item != NULL) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (obj != NULL && obj->type == cJSON_Object) {	// 判断数字内的元素是不是obj类型
				...
				val = cJSON_GetObjectItem(obj, "server_ip");		// 获得obj里的值
...
// 					val = cJSON_GetObjectItem(obj, "usb_port");		// 获得obj里的值
...
// 						val = cJSON_GetObjectItem(obj, "usb_vid");		// 获得obj里的值
...
// 	// obj = cJSON_GetArrayItem(item, 0);		// 获取的数组里的obj
