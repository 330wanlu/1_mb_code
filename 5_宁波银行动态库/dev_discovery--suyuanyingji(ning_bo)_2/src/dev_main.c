...
EXPORT_DLL int init_dll_interface(int time)
{
	...
	{
		WriteLogFile("dll_info already start!!!!!!!!!!");
	}
	// /*终端信息表初始化*/
	// if (!terminal_init()){
	// 	return 0;
...
{
//	int i;
	WriteLogFile("enter clean_dll_interface");
	if (init_flag != 1)
		return DLL_ERROR_SYSTEM;
	...
	WriteLogFile("exit clean_dll_interface");
	start_flag = 0;
	return ret;
...
int get_ter_all_info(char *json_in, void *hook,char *output)
{
	...
	WriteLogFile("_______server_ip:%s\r\n",item->valuestring);
	memcpy(server_ip, item->valuestring, strlen(item->valuestring));
	...
	WriteLogFile("_______server_port:%s\r\n",item->valuestring);
	server_port = atoi(item->valuestring);
...
int get_net_delay(char *json_in, void *hook, char *output)
{
	...
	WriteLogFile("dev_info 		2	intranet_time");
...
/*						######################################						*/
	...
	get_upgrade_net_intranet(server_ip, server_port,intranet_ip, external_ip,output,ter_id,notice_type);
	// get_usbshare_net_delay_time_test(server_ip, server_port,intranet_ip, external_ip);
...
EXPORT_DLL int  send_cmd_deal(char * json_in, result_CallBack callback,char *output)
{
	...
	cJSON*item = NULL;
//	int i;
	//WriteLogFile("enter dev_info send_cmd_deal");
	int result = 0;
	if (init_flag != 1)
	{
	...
	{
		WriteLogFile("lbc dev_info 接收到的指令为:%s\n", item->valuestring);
		int i;
