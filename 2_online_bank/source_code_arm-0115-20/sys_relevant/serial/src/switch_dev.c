...
int switch_dev_add(int hard_type,char *ter_type)
{
    ...
    //switch_dev_out("增加\"/dev/switch\"文件至文件系统\n");
...
{   struct _switch_dev	*stream;
    ...
    //switch_dev_out("打开switch_dev_open模块\n");
    if(stream->state==0)
    {   //switch_dev_out("首次使用switch_dev_open\n");
        stream->seria=ty_open(DF_SERIA_DIR,0,0,NULL);
        if(stream->seria<0)
        {   switch_dev_out("打开串口[%s]错误\n",DF_SERIA_DIR);
            return DF_ERR_PORT_CORE_SWITCH_SERIA;
        }
        //switch_dev_out("设置串口波特率[%d]\n",DF_SERIA_BPS);
        ty_ctl(stream->seria,DF_SERIA_CM_BPS,DF_SERIA_BPS);
        stream->lock=_lock_open("switch_dev.c",NULL);
        if(stream->lock<0)
        {   switch_dev_out("申请本页锁失败\n",stream->lock);
            return stream->lock;
        }

        switch_dev_out("发送上电命令给设备\n");
        result=dev_power_up(stream);
        if(result<0)
        {   
			log_out(INFO, "system", "init", "与单片机设备联机失败\n");
			sleep(2);
			result = dev_power_up(stream);
			if (result < 0)
			{
				log_out(INFO, "system", "init", "与单片机设备第二次联机失败\n");
		...
		{
			switch_dev_out("操作风扇失败,默认认为机柜不支持风扇控制\n");
		...
		if (result< 0 || r_order < 0)
		{
			switch_dev_out("操作蓝牙失败,默认认为机柜不支持蓝牙控制\n");
		...
		if (result< 0 || r_order < 0)
		{
			switch_dev_out("操作端口节电功能失败,默认认为机柜不支持端口节电功能\n");
			stream->dev_support.support_power = 0;
			log_out(INFO, "system", "init", "当前机柜为非节点状态\n");
		}
		else
		{
			stream->dev_support.support_power = 1;
			log_out(INFO, "system", "init", "当前机柜非节点状态\n");
		}

		//强制非节点版本
		//wang 02 03 壹沓合兆建行盾掉盾发现由于节点逻辑导致 这里强制将节点逻辑去除
		//stream->dev_support.support_power = 0;
...
{   struct _switch_dev	*stream;
    ...
    int i;
    stream=data;
	//switch_dev_out("进入switch_dev_ctl函数\n");
	if((stream==NULL))	
		return DF_ERR_PORT_CORE_FD_NULL;
...
//	switch_dev_out("获取下电端口状态完成---result:%d\n", result);
	if (r_order<0)
		return result;
	if (result < 0)
		return result;
...
static int dev_get_ad_status(struct _switch_dev_fd   *id, va_list args)
{
	...
	// if (id->dev->dev_support.support_power != 1)
	// {
	// 	switch_dev_out("id->dev->dev_support.support_power != 1\n");
	// 	return -1;
	// }
	switch_dev_out("获取ad\n");
...
static int dev_get_hard_version(struct _switch_dev	*stream)
{
	...
	switch_dev_out("发布日期：%s\n", infor.r_date);
	switch_dev_out("发布软件版本：%s\n", infor.soft);
	switch_dev_out("发布硬件版本：%s\n", infor.hard);

	if (strcmp((const char *)infor.soft, "PM9.01") >= 0)
	{
		switch_dev_out("单片机版本支持LCD屏显\n");
...
{   uint8 *dev_id;
	...
	//switch_dev_out("单片机应答打开%d端口成功,耗时：%lldms\n", port, diff);
    return result;   
}
...
{   uint8 *dev_id;
    ...
    int8 order;
    int result;
   // switch_dev_out("关闭端口命令\n");
    dev_id=va_arg(args,uint8 *);
    port=va_arg(args,int );    
    memset(buf,0,sizeof(buf));
    bcd2asc(dev_id,buf,6);
    buf[12]=(port/256);buf[13]=(port&0xff);
   // switch_dev_out("从id=[%s]的[%d]关闭\n",buf,port);
	//switch_dev_out("通知单片机关闭%d端口\n",port);
	//long long start = get_time_msec();
    result=oper_stm32_dev(id->dev,DF_PC_ORDER_USB_MA_CLOSE,buf,NULL,sizeof(buf),&order);  
	//switch_dev_out("关闭端口命令完成\n");
	if (result < 0)
	{
		switch_dev_out("单片机应答关闭%d端口失败\n", port);
		return result;
	}
	if (order < 0)
	{
		switch_dev_out("单片机应答关闭%d端口命令字错误,命令字：%02x\n", port,order);
		return DF_ERR_PORT_CORE_TY_USB_STM32_CLOSE;
	}
	//long long end = get_time_msec();
	//long long diff = (long long)end - (long long)start;
	//switch_dev_out("单片机应答关闭%d端口成功,耗时：%lldms\n", port, diff);
    return result; 
}
...
{   struct _switch_dev_id  *mac_id;
    ...
    int result;
    int8 r_oder;
    //switch_dev_out("获取设备机器编号\n");
    memset(r_buf,0,sizeof(r_buf));
    result=oper_stm32_dev(id->dev,DF_PC_ORDER_READ_DEV_ID,NULL,r_buf,0,&r_oder);
    if(result<0)
	{	switch_dev_out("oper_stm32_dev result = %d\n",result);
        return result;
	}
    if(r_oder<0)
	{	switch_dev_out("oper_stm32_dev r_oder = %d\n",r_oder);
        return DF_ERR_PORT_CORE_TY_USB_STM32_DEV;
	}
    //switch_dev_out("获取后:[%s]\n",r_buf);
    mac_id=va_arg(args,struct _switch_dev_id  *); 
...
{   switch_dev_out("重新启动\n");
    //log_file_close();
	switch_dev_out("使用reoot重启命令\n");
#ifndef PRODUCE_PROGRAM
	_mqtt_client_pub("mb_equ_ter_will", id->dev->ter_id);//生产环境未发货无法连接MQTT
#endif
	system("sync");
	#ifdef MTK_OPENWRT
	sleep(1);
...
{   uint8 buf[1024];
    ...
    int8 order;
    memset(buf,0,sizeof(buf));
    //switch_dev_out("获取心跳时间\n");
    result=oper_stm32_dev(id->dev,DF_PC_ORDER_HREAT_TIMER,NULL,buf,0,&order);
    //switch_dev_out("获取心跳时间成功,result=%d\n",result);
    if(result<0)
        return result;
    if(order<0)
        return DF_ERR_PORT_CORE_TY_USB_STM32_DEV;
    result=buf[0];
    //switch_dev_out("心跳时间为:%d秒\n",result);
    return result;
}
...
{	int port;
	...
	if (result < 0)
	{
		log_out(INFO, "SYSTEM", "单片机", "操作单片机，%d端口灯亮红灯失败\r\n", port);
	}
	return result;
}
...
{	int port;
	...
	if (result < 0)
	{
		log_out(INFO, "SYSTEM", "单片机", "操作单片机，%d端口灯亮绿灯失败\r\n", port);
	}
	return result;
}
...
{	int port;
	...
	if (result < 0)
	{
		log_out(INFO, "SYSTEM", "单片机", "操作单片机，%d端口灯熄灭失败\r\n", port);
	}
	return result;
}
...
static int dev_oper_red_on_gren_twi(struct _switch_dev_fd *id, va_list args)
{
	...
	struct _switch_dev *stream;
	stream = id->dev;
	//switch_dev_out("指示端口%d交替闪烁,红灯常1.9秒,绿灯亮0.1秒\n",port);
	if (stream->dev_support.support_lcd != 1)
	{
		buf[0] = (port / 256); buf[1] = (port & 0xff);
		buf[2] = 0x01;
...
static int dev_oper_red_on_gren_on(struct _switch_dev_fd *id, va_list args)
{
	...
	struct _switch_dev *stream;
	stream = id->dev;
	//switch_dev_out("指示端口%d交替闪烁,红灯常0.3秒,绿灯亮0.3秒\n", port);
	if (stream->dev_support.support_lcd != 1)
	{
		buf[0] = (port / 256); buf[1] = (port & 0xff);
		buf[2] = 0x01;
	...
	buf[2] = 0x06;//1号灯状态 0灭 1亮 2闪
	buf[3] = 0x06;//2号灯状态 0灭 1亮 2闪
	buf[4] = 0x00;//1号灯熄灭时长 x*100ms
	buf[5] = 0x00;//1号灯点亮时长 x*100ms
	buf[6] = 0x00;//2号灯熄灭时长 x*100ms
	buf[7] = 0x00;//2号灯点亮时长 x*100ms
...
static int dev_oper_get_key_value(struct _switch_dev_fd *id, va_list args)
{
	...
	{
		log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第一次失败\r\n");
		s_buf[0] = 0x00; s_buf[1] = 0x00; s_buf[2] = 0x02; s_buf[3] = 0x00;
		result = oper_stm32_dev(stream, DF_PC_ORDER_CTL_READ_KEY, s_buf, (unsigned char *)buf, 4, NULL);
		...
		if (memcmp(compare_buf, buf, 512) == 0)
		{
			...
			log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第二次失败\r\n");
		}
	}
	...
	if (memcmp(compare_buf, buf + 512, 512) == 0)
	{
		...
		log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第三次失败\r\n");
		s_buf[0] = 0x02; s_buf[1] = 0x00; s_buf[2] = 0x02; s_buf[3] = 0x00;
		result = oper_stm32_dev(stream, DF_PC_ORDER_CTL_READ_KEY, s_buf, (unsigned char *)buf + 512, 4, NULL);
		if (memcmp(compare_buf, buf + 512, 512) == 0)
		{

			log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第四次失败\r\n");
		}
	}
	...
	{
		log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第五次失败\r\n");
		s_buf[0] = 0x04; s_buf[1] = 0x00; s_buf[2] = 0x02; s_buf[3] = 0x00;
		result = oper_stm32_dev(stream, DF_PC_ORDER_CTL_READ_KEY, s_buf, (unsigned char *)buf + 512 + 512, 4, NULL);
		if (memcmp(compare_buf, buf + 512 + 512, 512) == 0)
		{
			log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第六次失败\r\n");
		}
	}
	...
	{
		log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第七次失败\r\n");
		s_buf[0] = 0x06; s_buf[1] = 0x00; s_buf[2] = 0x02; s_buf[3] = 0x00;
		result = oper_stm32_dev(stream, DF_PC_ORDER_CTL_READ_KEY, s_buf, (unsigned char *)buf + 512 + 512 + 512, 4, NULL);
		...
		if (memcmp(compare_buf, buf + 512 + 512 + 512, 512) == 0)
		{
			log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第八次失败\r\n");
		}
	}
	...
	if (memcmp(compare_buf, buf, 512) == 0)
	{
		log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第九次失败\r\n");
		s_buf[0] = 0x08; s_buf[1] = 0x00; s_buf[2] = 0x02; s_buf[3] = 0x00;
		result = oper_stm32_dev(stream, DF_PC_ORDER_CTL_READ_KEY, s_buf, (unsigned char *)buf + 512 + 512 + 512 + 512, 4, NULL);
	}
	...
	{
		log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第十次失败\r\n");
		s_buf[0] = 0x0a; s_buf[1] = 0x00; s_buf[2] = 0x02; s_buf[3] = 0x00;
		result = oper_stm32_dev(stream, DF_PC_ORDER_CTL_READ_KEY, s_buf, (unsigned char *)buf + 512 + 512 + 512 + 512 + 512, 4, NULL);
	}
	...
	{
		log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第十一次失败\r\n");
		s_buf[0] = 0x0c; s_buf[1] = 0x00; s_buf[2] = 0x02; s_buf[3] = 0x00;
		result = oper_stm32_dev(stream, DF_PC_ORDER_CTL_READ_KEY, s_buf, (unsigned char *)buf + 512 + 512 + 512 + 512 + 512 + 512, 4, NULL);
	}
	...
	{
		log_out(INFO, "SWITCH_DEV", "获取单片机密钥", "获取单片机密钥失败第十二次失败\r\n");
		s_buf[0] = 0x0e; s_buf[1] = 0x00; s_buf[2] = 0x02; s_buf[3] = 0x00;
		result = oper_stm32_dev(stream, DF_PC_ORDER_CTL_READ_KEY, s_buf, (unsigned char *)buf + 512 + 512 + 512 + 512 + 512 + 512 + 512, 4, NULL);
	}
...
{	uint8 buf[10];
	int result = 0;
	logout(INFO, "switch", "mac_restart", "开始重启\r\n");
	memset(buf,0,sizeof(buf));
	buf[0]=0x00;
	result = oper_stm32_dev(stream,DF_PC_ORDER_CTL_MAC_POWER,buf,NULL,1,NULL);
	// usleep(100000);
	sleep(3);
	logout(INFO, "switch", "mac_restart", "下电完成,result = %d\n\r\n",result);
	buf[0]=0x01;
	result = oper_stm32_dev(stream,DF_PC_ORDER_CTL_MAC_POWER,buf,NULL,1,NULL);
	logout(INFO, "switch", "mac_restart", "上电完成,result = %d\n\r\n",result);
	//switch_dev_out(INFO, "system", "init", "复位mac芯片成功，延时10秒\n");
	sleep(6);//要兼容dhcp此处需延时5秒
	//sleep(2);
	return 0;
}
#endif
...
/*--------------------------------------发送和接受命令--------------------------------------------*/
static int oper_stm32_dev(struct _switch_dev	*stream,uint8 order,uint8 *in_buf,uint8 *switch_dev_out_buf,
                        int in_len,int8 *r_order)
{
    ...
    uint16 crc,crc_val;
    int timer;
    switch_dev_out("发送命令给STM32,并且等待返回,该部分单任务运行,命令字为:%02x\n",order);
    ...
    {   result=ty_ctl(stream->seria,DF_SERIA_CM_GET);
		...
		if (datalen - 2 < 0)
		{
			log_out(INFO, "SYSTEM", "单片机", "crc_8005 测试 datalen 长度错误\r\n");
			_lock_un(stream->lock);
			return -1;
		}			
        ...
        if(switch_dev_out_buf==NULL)
		{	_lock_un(stream->lock);
            return 0;
		}	
		memcpy(switch_dev_out_buf,buf+5,datalen-DF_PC_ORDER_HEAD_LEN);
        _lock_un(stream->lock);
	
		//switch_dev_out("datalen = %d\n",datalen);
...
static int pack_s_buf(uint8 order,uint8 *in_buf,uint8 *switch_dev_out_buf,int in_len)
{   uint16 len;
    uint16 crc;
    len=in_len+DF_PC_ORDER_HEAD_LEN;
	switch_dev_out_buf[0]='T';switch_dev_out_buf[1]='Y';
	switch_dev_out_buf[2]=((len>>8)&0xff);switch_dev_out_buf[3]=(len&0xff);
	switch_dev_out_buf[4]=order;
	memcpy(switch_dev_out_buf+5,in_buf,in_len);
	crc=crc_8005(switch_dev_out_buf,len-2,0);
	switch_dev_out_buf[len-2]=((crc>>8)&0xff);
	switch_dev_out_buf[len-1]=(crc&0xff);
    return (int )len;
}


///////////////////////////仅发送命令////////////////////////////////////////////////////////
//static int send_oper_stm32_dev(struct _switch_dev	*stream,uint8 order,uint8 *in_buf,uint8 *switch_dev_out_buf,int in_len,int8 *r_order)
//{   uint8 buf[2048];
//    int len,i,j,datalen;
//    int result;
//    uint16 crc,crc_val;
//    int timer;
//    //switch_dev_out("发送命令给STM32,不等待返回\n");
//    len=pack_s_buf(order,in_buf,buf,in_len);
//    //_lock_set(stream->lock);
//    ty_ctl(stream->seria,DF_SERIA_CM_CLS);
//    result=ty_write(stream->seria,buf,len);
