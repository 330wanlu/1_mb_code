...
static int _socket_client_open(struct ty_file	*file)
{
	...
	{	socket_client_out("[_socket_client_open] DF_ERR_PORT_CORE_SLAVE_SPACE\n");
		return DF_ERR_PORT_CORE_SLAVE_SPACE;
	}
	id=&stream->fd[j];
	memset(id,0,sizeof(struct _socket_client_fd));
	//socket_client_out("[_socket_client_open]申请一个数据锁\n");
    id->lock=_lock_open("_socket_client.c",NULL);
    if(id->lock<0)
    {   socket_client_out("[_socket_client_open]申请锁失败\n");
        return id->lock;
    }
    //socket_client_out("[_socket_client_open]申请一个新的socket客户端\n");
	id->socketfd=socket(AF_INET,SOCK_STREAM,0);
	//socket_client_out("\n\n================================================\n[_socket_client_open]socketfd = %d\n\n\n",id->socketfd);
	if(id->socketfd<0)
		return id->socketfd;
	//socket_client_out("[_socket_client_open]socket fd = %d\n",id->socketfd);
	//socket_client_out("[_socket_client_open]客户端:socketfd=%d\n",id->socketfd);
	...
	stream->state++;
	socket_client_out("[_socket_client_open]打开socket客户端成功 return = %d \n",j+1);
	return j+1;
}
...
/*		if(id->connect!=0)
		...
		if(ip_len<0)
		{
			socket_client_out("解析IP失败\n");
			return -1;
		}
		socket_client_out("解析IP成功 IP为：%s\n",id->name);
		...
		socket_client_out("连接服务器:%s:%d\n",id->name,id->port);
		if((result=connect(id->socketfd,(struct sockaddr *)&add,sizeof(add)))==-1)
		{	socket_client_out("连接服务器失败\n");
			return -1;
		}		
		flags = fcntl(id->socketfd, F_GETFL, 0); //设置成非阻塞模式
		fcntl(id->socketfd, F_SETFL, flags | O_NONBLOCK);
		id->connect=1;
		id->task=_scheduled_task_open("socket_client",_socket_client_task,id,200,NULL);
		socket_client_out("_scheduled_task_open   id->task = %d\n",id->task);
		id->poll = 1;
		socket_client_out("连接服务器成功\n");
		return 1;	
	}
	else if(cm==DF_SOCKET_CLIENT_CM_CLOSE_TASK)
	{
		socket_client_out("即将关闭扫描线程  id->task = %d\n",id->task);
		_scheduled_task_close(id->task);
		socket_client_out("成功关闭扫描线程\n");
		id->poll=0;
		return 1;
	}
...
static int _socket_client_write(void *pro_data,int fd,uint8 *buf,int buf_len)
{
	...
	//socket_client_out("write fd = %d\n",id->socketfd);
	//socket_client_out("write buf_len = %d\n",buf_len);
	//for(i=0;i<buf_len;i++)
	//{
	//	printf("%02x ",pbuf[i]);
	//}
...
static int _socket_client_close(void *pro_data,int fd)
{
	...
	if(id->state==0)
		return DF_ERR_PORT_CORE_SLAVE_DEV;
	socket_client_out("客户端关闭线路:id->socketfd=%d\n",id->socketfd);
	
	//sleep(1);
	if(id->poll!=0)
	{	socket_client_out("【_socket_client_close】有任务没有关闭，需先关闭任务\n");
		_scheduled_task_close(id->task);
		id->poll=0;
		id->task=0;
	} 
...
static void _socket_client_task(void *arg,int o_timer)
{
	...
	if(result==0)
	{	socket_client_out("线路出现问题result=%d socketfd=%d\n",result,id->socketfd);
		id->connect=0;
		id->task=0;
		_lock_set(id->lock);
		if(id->err!=NULL)
	...
	{	_lock_set(id->lock);
		//socket_client_out("接受到数据\n");
		if(id->hook!=NULL)
		{	if(id->mem_fd->ty_fd==0)
			{	//socket_client_out("_socket_client_task test1\n");
				if(id->hook!=NULL)
				{	//socket_client_out("\n\n================================================\nsocketfd = %d\n\n\n",id->socketfd);
			...
			{	//socket_client_out("_socket_client_task test2\n");
...
//    {   //socket_client_out("read data by read i = %d\n",i);
//		result=read(sock,buf+i,buf_len-i);
//        if(result<=0)
//		{	//socket_client_out("read data time socket_client_out or err \n\n\n\n");
//            return result;
//		}
//    }
//    return i;
...
{	
	...
	//socket_client_out("解析IP成功 IP为：%s\n",name);
	if((socketfd=socket(AF_INET,SOCK_STREAM,0)) == -1)		//AF_INET:ipv4 SOCK_STREAM:tcp 
	{
		socket_client_out("创建socket失败\n");
		return -1;
	}
	...
	//new add by whl
	const int val = 1;
	setsockopt(socketfd, IPPROTO_TCP, TCP_NODELAY, &val, sizeof(val));	//TCP_NODELAY:无tcp延时
...
int socket_http_netPost(char *hostname,int port,char *request,char *content_type,char *parmdata,int time_out,char **returndata,int *returndata_len)
{
	...
	socket_client_out("enter socket_http_netPost\n");
	ip_len = get_DNS_ip((char *)hostname, (char *)name);
	if(ip_len<0)
	{	socket_client_out("解析IP失败\n");
		return -1;
	...
	//socket_client_out("进入获取数据函数\n");
	result = deal_http_data(socketfd,returndata);
	//socket_client_out("退出接收处理函数\n");
	*returndata_len = result;

	//socket_client_out("接收到数据：%s\n",returndata);
...
static int deal_http_data(int sockfd,char **outbuf)
{
	...
	//socket_client_out("获取第一行数据\n");
	memset(buf,0,sizeof(buf));
	numchars = get_line(sockfd, buf, sizeof(buf));
	//socket_client_out("获取到的第一行数据为：%s\n",buf);
	memset(buf,0,sizeof(buf));
	numchars = get_line(sockfd, buf, sizeof(buf)); 
	//socket_client_out("获取到的第二行数据为：%s\n",buf);
    while ((numchars > 0) && strcmp("\n", buf))  
    {  
        if (strncasecmp(buf, "Content-Length:",15) == 0)  
		{	memset(len_buf,0,sizeof(len_buf));
			memcpy(len_buf,&buf[15],20);
			//socket_client_out("len_buf = %s",len_buf);
			cls_no_asc(len_buf,strlen(len_buf)); 
            content_length = atoi(len_buf);  
			//socket_client_out("Content-Length = %d\n",content_length);
		}
		memset(buf,0,sizeof(buf));
        numchars = get_line(sockfd, buf, sizeof(buf));  
		//socket_client_out("获取到的第%d行数据为：%s",num,buf);
		//num +=1;
    ...
    /*没有找到 content_length */  
	//socket_client_out("退出获取头循环\n");
    if (content_length <=0) {  
        socket_client_out("错误请求,接收到的长度不对content_length = %d\n",content_length); 
        return -1;  
    }  
	 /*接收 POST 过来的数据*/  
	//if(content_length>102400)
	//{
	//	socket_client_out("超过缓存大小,无法接收\n");
	//	return -1;
	//}
	...
	//socket_client_out("HTTP data :%s\n",tmp);
	*outbuf = tmp;
	//memcpy(outbuf,tmp,sizeof(tmp));
	//socket_client_out("TRAN return data :%s\n",*outbuf);
...
{  
	...
	//socket_client_out("接收到的一行数据：");
	//for(j=0;j<i;j++)
	//{
	//	printf("%02x ",(unsigned char)buf[j]);
	//}
	//printf("\n");
	//socket_client_out("获取到的数据：%s\n",buf);
    /*返回 buf 数组大小*/  
    return(i);  
}  