...
static int _socket_sev_open(struct ty_file	*file)
{
    ...
    {   //socket_sev_out("申请一个数据锁\n");
       ...
       // socket_sev_out("设置tcp没有延时\n");
        ...
        {	socket_sev_out("监听打开失败\n");
            close(stream->socketfd);
            _lock_close(stream->lock);
            return DF_ERR_PORT_CORE_SOCKET_LISTER;
        }
        stream->task_fd=_scheduled_task_open("_socket_sev",_socket_sev_main_task,stream,DF_SOCKET_SEV_MAIN_TASK_TIMER,NULL);
        if(stream->task_fd<0)
        {   socket_sev_out("申请线程失败\n");
            close(stream->socketfd);
            _lock_close(stream->lock);
    ...
    if(ty_pd==0)
    {  
		//socket_sev_out("本文件需要添加守护线程\n");
        //char name[DF_FILE_NAME_MAX_LEN];
        //memset(name,0,sizeof(name));
        //sprintf(name,"socket_sev/sever/%d",stream->port);
        //ty_pd_add_file(name);
    }
    socket_sev_out("打开socket服务成功\n");
    return 1;
}
...
static int _socket_sev_ctl(void *data,int fd,int cm,va_list args)
{
	...
	else if(cm==DF_SOCKET_SEV_CM_LINE_CLOSE)
	{
		int sockfd;
		int i;
		//socket_sev_out("应用层服务端强制关闭socket链路\n");
		sockfd=va_arg(args,int);
		//socket_sev_out("关闭一条socket链路\n");
		_lock_set(stream->lock);
...
static void _socket_sev_main_task(void *arg,int o_timer)
{
    ...
    for(;;)
    {
        ...
        if(_delay_task_add("0",_task_line,&stream->line[i],0)<0)
        {   socket_sev_out("创建队列失败\n");
            close_line(&stream->line[i]);
        }
        continue;
    }
...
//    line=arg;
//    stream=line->stream;
//    //socket_sev_out("维护线路的正常运行\n");
//    if(stream->hook==NULL)
//    {   //socket_sev_out("无应用程序,应关闭链路\n");
//        shutdown(line->socketfd,SHUT_RDWR);
//        close_line(line);
//        return ;    
//    }
//    if(stream->hook(line->socketfd,line->ip,line->port,stream->arg)<0)
//    {   //socket_sev_out("应用发生错误,关闭该线路\n");
//        shutdown(line->socketfd,SHUT_RDWR);
//        close_line(line);
//        return ;
//    }
//    //if(stream->close_type==1)
//    //{   socket_sev_out("要求shutdown关闭该链路\n");
//    //    shutdown(line->socketfd,SHUT_RDWR);
//    //}
//	result=read(line->socketfd,byte,1);
//	if(result == 0)
//	{
//		//socket_sev_out("客户端已关闭链路，要求删除该链路\n");
//		close_line(line); 
//		return ;
//	}
//	//socket_sev_out("客户端未能正常关闭socket，要求close该链路\n");
//	shutdown(line->socketfd,SHUT_RDWR);
//	close_line(line);  
//	return ;
//}
...
static void _task_line(void *arg)
{
    ...
    line=arg;
    stream=line->stream;
    socket_sev_out("维护线路的正常运行,stream->close_type=%d\n",stream->close_type);
    if(stream->hook==NULL)
    {   socket_sev_out("无应用程序,应关闭链路\n");
        shutdown(line->socketfd,SHUT_RDWR);
        close_line(line);
    ...
    if(stream->hook(line->socketfd,line->ip,line->port,stream->arg)<0)
    {   socket_sev_out("应用发生错误,全部关闭线路\n");
        shutdown(line->socketfd,SHUT_RDWR);
        close_line(line);
        return ;
    }
    if(stream->close_type==1)
    {   socket_sev_out("要求全部关闭该链路\n");
		result=read(line->socketfd,&byte,1);
		if(result == 0)
		{	socket_sev_out("客户端已关闭链路，要求删除该链路\n");
			shutdown(line->socketfd,SHUT_RDWR);
		...
		socket_sev_out("查询到客户端超时没有关闭连接\n");
    }
	else if(stream->close_type==0)
	{	socket_sev_out("3240模式下关闭\n");
		shutdown(line->socketfd,SHUT_RDWR);
		close_line(line);   
		return ;
	}
