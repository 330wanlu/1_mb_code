...
int get_net(char *eth,struct _net_define  *net)
{
	...
	//int type;
    if(net==NULL)
    {   linux_ip_out("输入指针不能为空\n");
        return -1;
    }
    ip =  get_ip(eth);
	if(ip < 0)
	{
		linux_ip_out("获取ip地址失败\n");
	}
	net->ip = ip;
	//linux_ip_out("IP=%08x\n",net->ip);
	mask = get_mask(eth);
...
int find_now_wifi_ssid(char *ssid)
{
	...
	if (p_e == NULL)
	{
		linux_ip_out("当前连接的wifi SSID规则有误\n");
		return - 2;
	}
	memcpy(ssid, p_s, p_e - p_s);
	//linux_ip_out("当前连接的wifi SSID:%s\n", ssid);
...
int set_net(char* eth_wlan, struct _net_define	*net)
{
	...
	if (strcmp(eth_wlan,"wlan0") == 0)
	{
		...
		{
			linux_ip_out("找到可用wifi DNS 配置文件\n");
			copy_file_cmp(reslov_path, "/etc/resolv.conf");
		}
		system("ifconfig wlan0 up");
	...
	else
	{
		...
		if (ip_type == 2)
		{
			...
			if (result < 0)
			{
				...
				linux_ip_out("无需在文件中重新设置网络参数\n");
				//return 0;
			}
			//linux_ip_out("设置固定IP操作\n");
			linux_ip_out("设置当前网络参数\n");
			linux_ip_out("ip = [%08x] mask = [%08x] gate = [%08x]\n", net->ip, net->mask, net->gateway[0]);
			chmod_file(DF_LINUX_NETWORK_FILE, "777");
			system("/etc/init.d/auto_config_network");
		...
		{
			linux_ip_out("//////eth0 配置模式之DHCP IP\n");
			int mac_use = 0;
			char if_data[1024] = { 0 };
			char mac_address[20] = { 0 };
			char cmd[1024] = { 0 };
			//判断MAC地址是否存在
			if (access("/etc/init.d/mac_address", 0) == 0)
			{
				result = read_file("/etc/init.d/mac_address", mac_address, sizeof(mac_address));
			...
			if (mac_use == 0)
			{
				linux_ip_out("//////eth0 配置模式之DHCP IP ,配置地址文件不存在或mac地址异常\n");
				...
				if (memcmp(ter_id, "068", 3) != 0)//第一次，没有机器编号的时候
				{
					...
					write_file("/etc/init.d/mac_address", mac_address, strlen(mac_address));
			...
			{	
				linux_ip_out("//////eth0 配置模式之DHCP IP ,配置地址文件正常\n");
				char ter_id[13] = { 0 };
				get_configure_file("/etc/mqtt_tmp.conf", "mqtt_user = ", ter_id);
				...
				{
					linux_ip_out("//////eth0 配置模式之DHCP IP ,配置地址文件正常，且当前无机器编号,ter_id=%s,老的mac=%s\n",ter_id,mac_address);
 ...
 int get_dns_file_servername(char *dns)
{
 ...
 int write_mac2network(char *w_mac)
 {
	 ...
	 linux_ip_out("write_correct_mac2file buf=[%s]\n", buf);
...
int set_net_1(char *eth,struct _net_define  *net)
{
	...
	if((net->ip!=0)&&(net->mask!=0))
	{
		...
		linux_ip_out("设置当前网络的IP=[%s]\n",ip_asc);
		ip2ip_asc(net->mask,mask_asc);
		linux_ip_out("设置当前网络的mask_asc=[%s]\n",mask_asc);
...
int get_DNS_ip(char *hostname,char *hostip)
{
	...
	//linux_ip_out("hostnamebuf %s\n", hostnamebuf);
	if (memcmp(hostnamebuf, "127.0.0.1", len) == 0)
	{
		linux_ip_out("本机IP不用解析\n");
	...
	//	linux_ip_out("解析IP地址失败sigsetjmp\n");
	//	return -1;
	//}
	//alarm(2); /* 设置超时时间 */
	...
	{	
		...
		//linux_ip_out("host name	:%s\n",h->h_name);
		//linux_ip_out("ip length	:%d\n",h->h_length);
		//linux_ip_out("type	:%d\n",h->h_addrtype);
		//linux_ip_out("ip	:%s\n",inet_ntoa(in));
	}
...
//    fd=socket(AF_INET,SOCK_DGRAM,0);
//    if(fd<0)
//    {   linux_ip_out("设置子网掩码失败,socket打开失败\n");
//        return -1;
//    }
//    memcpy(ifr.ifr_name,eth,strlen(eth));
//    addr=(struct sockaddr_in *)&(ifr.ifr_addr);
//    memset(ip_asc,0,sizeof(ip_asc));
//    ip2ip_asc(ip,ip_asc);
//    linux_ip_out("设置的子网掩码=[%s]\n",ip_asc);
//    addr->sin_family = AF_INET;
//    inet_aton(ip_asc,&(addr->sin_addr));
//    linux_ip_out("addr->sin_addr = %08x\n",addr->sin_addr);
//    req =ioctl(fd, SIOCSIFNETMASK,&ifr);
//    if(req<0)
//    {   close(fd);
//        linux_ip_out("设置子网掩码失败req = %d\n",req);
//        return -1;
//    }
//    close(fd);
//    linux_ip_out("子网掩码成功\n");
...
//        linux_ip_out("找到有效的路由信息\n");
//        ip=((way&0xff)<<24)+(((way>>8)&0xff)<<16)+(((way>>16)&0xff)<<8)+(((way>>24)&0xff)<<0);
//        linux_ip_out("gateway=0x%08x,ip=0x%08x\n",gateway,ip);
//        if(ip==gateway)
//        {   linux_ip_out("找到相同的路由信息\n");
//            return 0;
//        }     
//    }
//    linux_ip_out("没有找到路由信息\n");
//    return -2;
//}
...
//    linux_ip_out("设置的网关=[%s]\n",gateway_asc);
//    addr->sin_family = AF_INET;
//	addr->sin_port = 0;
//    if(inet_aton(gateway_asc,&(addr->sin_addr))<0)
//	{
//		linux_ip_out("inet_aton error\n");
//	}
...
int get_socket_net_state(char *eth)
{
	...
	if (err == 0) 
	{
// 		linux_ip_out("edata.data = %02x\n",edata.data);
// 		linux_ip_out(stdout, "Link detected: %s\n",edata.data ? "yes":"no");
		close(fd);
		return edata.data;		
	} 
	else if (errno != EOPNOTSUPP) 
	{
        linux_ip_out("Cannot get link status，err = %d\n");
		close(fd);
		return -1;
    }
	close(fd);
...
static int pack_dns(struct _net_define  *net,char *o_buf)
{
	...
	sprintf(o_buf + strlen(o_buf), "nameserver 218.2.2.2\n");
...
//	linux_ip_out("pack_eth netmask = %d\n",netmask);
...
{	char *ps,*end_ps;	
	...
	{	linux_ip_out("with linux_ip_out:\"\n");
		return -2;
	}
	i=end_ps-ps;
	linux_ip_out("i = %d\n",i);
	if(i>100)
	{
		linux_ip_out("不符合mac地址规范\n");
		return -3;
	}
	memset(macaddr,0,sizeof(macaddr));
	memcpy(macaddr,ps,i);
	linux_ip_out("mac=%s\n",macaddr);
	linux_ip_out("找到MAC地址");
	ps = end_ps+2;//指向\n\n之后
	...
	linux_ip_out("find the first ifconfig eth0\n");
	...
	linux_ip_out("find the second ifconfig eth0\n");
	...
	if ((ps == NULL) || (ps>end))
	{
		linux_ip_out("without3:[%s]\n", "ifconfig eth0 ");
		return -1;
	}
	linux_ip_out("find the third ifconfig eth0\n");
	...
	i =  end_ps-ps;
	if(i>4)
	{	linux_ip_out("不符合子网掩码地址设置规范\n");
		return -3;
	}
	memset(mask,0,sizeof(buf));
	memcpy(mask,ps,end_ps-ps);
	netmask = atoi(mask);
	linux_ip_out("netmask = %d\n",netmask);
	i = dton(netmask);
	net->mask = strap_32(i);
	//net->mask = dton(netmask);
	linux_ip_out("net->mask=0x%08X\n",net->mask);
	linux_ip_out("找到子网掩码\n");
	...
	linux_ip_out("net->gateway=0x%08X\n",net->gateway[0]);
	linux_ip_out("找到网关\n");
	return 0;
}
...
{	struct ifreq ifreq;
	...
	if(sock<0)
	{
		linux_ip_out("打开socket失败\n");
		return -1;
	}
	strcpy(ifreq.ifr_name,"eth0");
	if(ioctl(sock,SIOCGIFHWADDR,&ifreq)<0)
	{
		linux_ip_out("获取本机MAC失败\n");
		close(sock);
		return -2;
	}
	linux_ip_out("获取本机MAC成功\n");
	linux_ip_out("%02X:%02X:%02X:%02X:%02X:%02X\n",	(unsigned char)ifreq.ifr_hwaddr.sa_data[0],
...
{	struct _net_define  o_net;
	...
	linux_ip_out("set_net_file \n");
	result = get_a20_net_file(eth, &o_net, mac);
	if (result == 0)
	{
		linux_ip_out("已找到本机网络设置\n");
		//if(memcmp(net->ip,&o_net.ip,sizeof(uint32))==0)
		//{	linux_ip_out("IP已经与设置结果一样了\n");
		//	return 0;
		//}
		linux_ip_out("原来IP=%08x,要设置的IP=%08x\n",o_net.ip,net->ip);
		linux_ip_out("原来DNS=%08x,要设置的DNS=%08x\n",o_net.DNS,net->DNS);
		linux_ip_out("原来netmask=%08x,要设置的netmask=%08x\n",o_net.mask,net->mask);
		linux_ip_out("原来gateway=%08x,要设置的gateway=%08x\n",o_net.gateway[0],net->gateway[0]);
	...
	//	return -1;
	//}
	linux_ip_out("set_net_file gateway = %08x\n",net->gateway[0]);
	linux_ip_out("set_net_file ip = %08x\n",net->ip);
	linux_ip_out("set_net_file netmask = %08x\n",net->mask);
	linux_ip_out("set_net_file NDS = %08x\n",net->DNS);
	linux_ip_out("set_net_file MAC ADDRESS = %s \n",mac);
	get_mac_addr(benjimac);
	if(net->type==1)//如果是DHCP,保持文件中内容不变
	{
		if(memcmp(mac,benjimac,10)==0)
		{
			linux_ip_out("要设置的MAC地址与本机一致\n");
			return 0;
		}
		net->ip=o_net.ip;
		net->mask=o_net.mask;
...
static int get_dns(char *name,char *str,char *end,struct _net_define  *net)
{
	...
	if((ps==NULL)||(ps>end))
	{	linux_ip_out("1without:[%s]\n",name);
		return -1;
	}
	//linux_ip_out("find the first nameserver\n");
	ps+=strlen(name);//指向第一个nameserver 结尾
	str = ps;
	ps=strstr(str,name);//指向第二个nameserver 开头
	if((ps==NULL)||(ps>end))
	{	linux_ip_out("2without:[%s]\n",name);
		return -1;
	}
	//linux_ip_out("find the second nameserver\n");
	ps+=strlen(name);//指向第二个nameserver 结尾
	str = ps;
	ps=strstr(str,name);//指向第三个nameserver 开头
	if((ps==NULL)||(ps>end))
	{	linux_ip_out("3without:[%s]\n",name);
		return -1;
	}
	//linux_ip_out("find the third nameserver\n");
	ps+=strlen(name);//指向第三个nameserver 结尾
	str = ps;
	
	end_ps=strstr(str,"\n");
	if((end_ps==NULL)||(end_ps>end))
	{	linux_ip_out("with linux_ip_out:enter \n");
		return -2;
	}
	i =  end_ps-ps;
	//linux_ip_out("i= %d \n",i);
	if(i>sizeof(buf))
	{	linux_ip_out("不符合DNS地址设置规范\n");
		return -3;
	}
	memset(buf,0,sizeof(buf));
	memcpy(buf,ps,end_ps-ps);	
