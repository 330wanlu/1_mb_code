...
{	struct _register  *stream;
	...
	//register_out("添加socket client文件成功\n");
	return result;
}
...
int data_3des_base(uint8 *in_data, uint8 *out_data)
{
	...
	//register_out("[data_des3_base:]buf_len = %d\n",buf_len);
	memcpy(buf, in_data, buf_len);
	buyu = buf_len % 8;
	//register_out("[data_des3_base:]buyu = %d\n",buyu);
	if (buyu > 0)
	{
		buf_len = buf_len + 8 - buyu;
	}
	//register_out("读到%d个字节及补齐\n",buf_len);
	//for(i=0;i<buf_len;i++)
	//{
	//	register_out("%02X ",buf[i]);
	//}
	//register_out("\n");
	...
	//register_out("base64datalen=%d\n",base_len);
	//register_out("base64data:   %s\n",base_buf);
...
static int _register_terminal_to_server(struct _register_fd *id, va_list args)
{
	...
	register_out("11111111  http_token_register_dev_to_server  parm.s_ip=%s,parm.s_port=%d\n",parm.s_ip,parm.s_port);
	...
	register_out("注册查询组包数据：%s\n", g_buf);

	register_out("2222222  http_token_register_dev_to_server  parm.s_ip=%s,parm.s_port=%d\n",parm.s_ip,parm.s_port);
	...
	register_out("exit http_token_register_dev_to_server\n");
	return result;		
}
static int _get_mqtt_server_info_from_local(struct _register_fd *id, va_list args)
{
	...
	//register_out("通过加密算法获取出密码\n");
	data_3des_base((uint8 *)user, (uint8 *)passwd);
	//register_out("计算出的密码为%s\n", passwd);
...
static int analysis_deal_respone_data(char *data, char *server_addr, int *port)
{
	...
	register_out("获取到的MQTT服务器端口号为%s\n", mqtt_port);
	...
	//register_out("获取到的ca.crt证书文件为%s\n", ca_cert_base);
	...
	//register_out("获取到的client.crt证书文件为%s\n", client_cert_base);
	...
	//register_out("获取到的client.key证书文件为%s\n", client_key_base);

	char tmp[4000];
	memset(tmp, 0, sizeof(tmp));
	if (get_file_base64("/etc/ca.crt", tmp, sizeof(tmp))<0)
	{
		register_out("获取文件失败:[%s]\n", "/etc/ca.crt");
	...
	else
	{
		...
		{
			register_out("client.cert文件不一致需要更新\n");
			save_cert_file(client_cert_base, "/etc/client.crt");
		}
	}
	...
	else
	{
		...
		{
			register_out("client.key文件不一致需要更新\n");
			save_cert_file(client_key_base, "/etc/client.key");
		}
	}
	cJSON_Delete(root);
...
static int save_cert_file(char *base_file, char *path)
{
	...
	register_out("base64解码\n");
	memset(in_buf, 0, sizeof(in_buf));
	memcpy(in_buf, base_file, base_len);
	//register_out("要base64的数据长度为%d\n", base_len);
	...
	//register_out("base64后的数据长度为%d\n", buf_len);
	register_out("BASE64后数据：%s\n",buf);
	//for (i = 0; i<buf_len; i++)
	//{
	//	register_out("%02X ", buf[i]);
	//}
