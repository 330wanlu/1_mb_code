...
{   struct _get_net_state *stream;
    ...
    //get_net_state_out("增加\"/dev/get_net_state\"文件至文件系统\n");
...
static int get_net_state_open(struct ty_file	*file)
{
   ...
   // get_net_state_out("打开get_net_state模块\n");
    if(stream->state==0)
    {   //get_net_state_out("首次使用get_net_state_open\n");
        ...
        {   get_net_state_out("打开switch设备失败\n");
		...
		if(stream->task<0)
		{
			get_net_state_out("开启查询网络状态线程失败\n");
			return stream->task;
		}
		// get_net_state_out("get_net_state模块初始化成功\n");
    }
	...
	stream->state++;	
    //get_net_state_out("打开get_net_state成功:j=%d\n",j);
	return j+1;	 
}
...
static int check_net_internet(struct _get_net_state *stream)
{
	...
	{
		//get_net_state_out("解析www.baidu.com域名失败\n");
	...
	//{
	//	get_net_state_out("解析www.njmbxx.com ftp端口失败\n");
	//	sprintf(net_status.internet, "0");
	//	goto END;
	//}
...
{	
	...
	if ((eth0_ok < 0 && wlan0_ok <0) || (eth0_ok < 0 && wlan0_ok == 0) || (eth0_ok < 0 && wlan0_ok ==1))
	{
		if (wlan0_ok < 0)
		{
			...
			{	//不支持wifi,灯全灭
				get_net_state_out("1不支持wifi,灯全灭\n");
	...
	else if (eth0_ok == 0 && wlan0_ok < 0)
	{
		if (stream->wifi_state != DF_SWITCH_CM_PORT_RED_OFF_GREN_OFF)
		{	//不支持wifi,灯全灭
			get_net_state_out("2不支持wifi,灯全灭\n");
			ty_ctl(stream->switch_dev, DF_SWITCH_CM_PORT_RED_OFF_GREN_OFF, DF_WIFI_LED_SAME_USBPORT);
			stream->wifi_state = DF_SWITCH_CM_PORT_RED_OFF_GREN_OFF;
		}
		...
		{
			get_net_state_out("获取到的网络状态为离线0\n");
	...
	else if (eth0_ok == 0 && wlan0_ok == 0)
	{
		if (stream->eth_off_count <= 5)//离线时间大于5个周期
		{
			...
			{	//支持wifi,未连接
				get_net_state_out("1支持wifi,未连接亮红灯\n");
		...
		if (stream->eth_off_count > 5)//离线时间大于5个周期
		{
			...
			{	//支持wifi,正在连接
				//get_net_state_out("1支持wifi,正在连接绿灯闪\n");
			...
			{
				get_net_state_out("get_configured_wifi_list wifi搜索可用列表失败\n");
			}
			else
			{			
				get_net_state_out("有线网络连接异常切换至wifi模式\n");
				...
				else
				{
					...
					{
						get_net_state_out("当前wifi不可用,删除连接\n");
	...
	else if (eth0_ok == 0 && wlan0_ok ==1)
	{
		...
		if ((strlen(stream->now_ssid) != 0) && (strlen(stream->connect_ssid) != 0))
		{
			...
			{
				get_net_state_out("当前连接的wifi与要设置的wifi不一致,需要更换\n");
				_mqtt_client_disconnect();
		...
		{	//支持wifi,已连接
			get_net_state_out("1支持wifi,连接成功,亮绿灯\n");
		...
		if (atoi(stream->net_status.internet) == 0)
		{
			...
			{
				//get_net_state_out("获取到的网络状态为在线1\n");
	...
	else if ((eth0_ok == 1 && wlan0_ok < 0) || (eth0_ok == 1 && wlan0_ok == 0))
	{
		if (wlan0_ok < 0)
		{
			if (stream->wifi_state != DF_SWITCH_CM_PORT_RED_OFF_GREN_OFF)
			{	//不支持wifi,灯全灭
				get_net_state_out("3不支持wifi,灯全灭\n");
				ty_ctl(stream->switch_dev, DF_SWITCH_CM_PORT_RED_OFF_GREN_OFF, DF_WIFI_LED_SAME_USBPORT);
				stream->wifi_state = DF_SWITCH_CM_PORT_RED_OFF_GREN_OFF;
			}
		}
		else
		{
			if (stream->wifi_state != DF_SWITCH_CM_PORT_RED_ON_GREN_OFF)
			{	//支持wifi,未连接
				get_net_state_out("2支持wifi,未连接亮红灯\n");
				ty_ctl(stream->switch_dev, DF_SWITCH_CM_PORT_RED_ON_GREN_OFF, DF_WIFI_LED_SAME_USBPORT);
				stream->wifi_state = DF_SWITCH_CM_PORT_RED_ON_GREN_OFF;
			}
		}
	...
	{
		get_net_state_out("有线网络正常情况需关闭无线网络服务\n");
		system("killall wpa_supplicant");
...
static int get_internet_infomation(struct _get_net_state *stream)
{
	...
	//get_net_state_out("向服务器请求公网信息完成\n");
	if (result < 0)
	{
		if (returndata != NULL)
		{
			//get_net_state_out("释放http接收数据内存\n");
			free(returndata);
			returndata = NULL;
			//get_net_state_out("释放内存成功,指向空\n");
		}
		get_net_state_out("获取公网IP信息失败\n");
		return -1;
	}
	else
	{
		...
		{
			get_net_state_out("获取公网信息得到的数据长度存在异常\n");
			free(returndata);
			returndata = NULL;
			return -2;
		}
		//get_net_state_out("将公网信息的UTF8数据转换成GBK数据\n");
		utf82gbk((uint8 *)returndata, (uint8 *)inter_info);
		free(returndata);
		returndata = NULL;
		//get_net_state_out("获取公网IP信息成功%s\n", inter_info);
		result = analysis_json_internet_info(stream, inter_info);
		if (result < 0)
		{
			get_net_state_out("解析公网JOSN数据失败,result = %d\n",result);
...
static int analysis_json_internet_info(struct _get_net_state *stream,char *inbuf)
{
	...
	{
		get_net_state_out("获取到的公网IP长度错误,ip %s\n", internet_ip);
		cJSON_Delete(root);
		return -5;
	}
	...
	// 	get_net_state_out("获取精度信息 failed\n");
	// 	cJSON_Delete(root);
	// 	return -6;
	// }
	// if (strlen(longitude) < 7)
	// {
	// 	get_net_state_out("获取精度信息长度错误,ip %s\n", longitude);
	// 	cJSON_Delete(root);
	// 	return -7;
	// }
	// ///////////////////////////////////////////////////////////////////
	...
	// 	get_net_state_out("获取纬度信息 failed\n");
	// 	cJSON_Delete(root);
	// 	return -8;
	// }
	// if (strlen(latitude) < 7)
	// {
	// 	get_net_state_out("获取纬度信息长度错误,ip %s\n", latitude);
	// 	cJSON_Delete(root);
	// 	return -9;
	// }
	...
	if (get_json_value_can_not_null(Jsp_Tmp, area_info, 0, 200) < 0) {
		get_net_state_out("获取区域信息 failed\n");
		cJSON_Delete(root);
		return -10;
	}
	if (strlen(area_info) < 7)
	{
		get_net_state_out("获取区域信息长度错误,ip %s\n", area_info);
		cJSON_Delete(root);
		return -11;
	}

	// logout(INFO, "SYSTEM", "网络检测", "公网IP为%s,经度信息为%s,纬度信息为%s,区域信息为%s\r\n", internet_ip, longitude, latitude, area_info);
...
static int set_wifi_used_status(struct _get_net_state_fd *id, va_list args)
{
	...
	get_net_state_out("设置wifi切换,要设置的wifi为:%s,当前连接的wifi为：%s\n", stream->now_ssid, stream->connect_ssid);
	return 0;
}