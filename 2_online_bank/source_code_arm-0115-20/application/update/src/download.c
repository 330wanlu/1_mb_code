...
int connect_server_m(http_t_m *info)
{
    ...
    if(-1 == set_socket_option_m(sockfd))
    {
        goto failed;
    }
 
    if ((addr = dns_m(info->host_name)) == -1)
    {
...
int parse_http_header_m(http_t_m *info)
{
    ...
    while(fgets(info->buffer, RECV_BUF, info->in))
    {
        ...
        if(p = strncasestr_m(info->buffer, "Content-length"))
        {
            p = strchr(p, ':');
            p += 2;     // 跳过冒号和后面的空格
            info->len = atoi(p);
        ...
        else if(p = strncasestr_m(info->buffer, "Transfer-Encoding"))
        {
            if(strncasestr_m(info->buffer, "chunked"))
            {
                info->chunked_flag = 1;
            }
            else
        ...
        else if(p = strncasestr_m(info->buffer, "Location"))
        {
            p = strchr(p, ':');
            p += 2;     // 跳过冒号和后面的空格
...
int read_data_m(http_t_m *info, int len)
{
    ...
    while(total_len)
    {
        ...
        total_len -= rtn_len;
        lprintf(MSG_DEBUG, "read len: %d\n", rtn_len);
        if(-1 == save_data_m(info, info->buffer, rtn_len))
        {
            return -1;
        }
...
int recv_chunked_response_m(http_t_m *info)
{
    ...
    do{
        ...
        if(-1 == read_data_m(info, part_len))
            return -1;
...
int recv_response_m(http_t_m *info)
{
    ...
    if(info->chunked_flag)
        return recv_chunked_response_m(info);
 
    if(-1 == read_data_m(info, total_len))
        return -1;
...
int http_download_m(char *url, char *save_path)
{
    ...
    //初始化结构体
    info = malloc(sizeof(http_t_m));
    if(!info)
    {
        lprintf(MSG_ERROR, "malloc failed\n");
        return -1;
    }
    memset(info, 0x0, sizeof(http_t_m));
    info->sock = -1;
    info->save_path = save_path;
 
    // 解析url
    if(-1 == parser_URL_m(url, info))
        goto failed;
 
    // 连接到server
    if(-1 == connect_server_m(info))
        goto failed;
 
    // 发送http请求报文
    if(-1 == send_request_m(info))
        goto failed;
    ...
    switch(info->status_code)
    {
        ...
        case HTTP_REDIRECT:
            ...
            clean_up_m(info);
            return http_download_m(tmp, save_path);
    ...
    clean_up_m(info);
    printf("___________________________________recv %d bytes\n", info->recv_data_len);
    return info->recv_data_len;
failed:
    clean_up_m(info);
    return -1;
}
