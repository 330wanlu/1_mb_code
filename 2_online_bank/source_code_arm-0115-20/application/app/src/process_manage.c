...
int main_start_init()
{
	...
	// if (detect_process("/usr/local/mysql/bin/mysqld_safe", "tmp11.txt") > 0)
	// {
	// 	process_manage_out("%s进程存在,需结束\n", "/usr/local/mysql/bin/mysqld_safe");
	// 	system("killall /usr/local/mysql/bin/mysqld_safe");
	// }

	// if (detect_process("/usr/local/mysql/libexec/mysqld", "tmp11.txt") > 0)
	// {
	// 	process_manage_out("%s进程存在,需结束\n", "/usr/local/mysql/libexec/mysqld");
	// 	system("killall /usr/local/mysql/libexec/mysqld");
	// }
	return 0;
}
...
int detect_process_path(char * process_name,char *path)
{
	...
	//process_manage_out("order=[%s]\n",order);
	system(order);

	FILE* fp = fopen(path, "r");
	if (fp == NULL)
	{
		process_manage_out("文件打开失败,文件名：%s\n", path);
	...
	while (fgets((char *)lineBuff, 256, fp) != NULL)
	{
		...
		//process_manage_out("%s\n", lineBuff);
		num += 1;
	}
	fclose(fp);
	if (num >= 1)
	{	//process_manage_out("检测到进程(%s)存在，无需再次启动,%s\n",process_name,buff);
		sprintf(order, "rm %s", path);
		system(order);
		return num;
	}
	//logout(INFO, "system", "进程管理", "检测到进程(%s)不存在,需再次启动,num = %d\r\n",process_name,atoi(buff));
	//process_manage_out("检测到进程未启动\n");
	sprintf(order, "rm %s", path);
	system(order);
	return -1;
}
...
{    
	...
	//process_manage_out("检测到进程(%s)不存在,需再次启动,num = %d\n",process_name,num);
	//process_manage_out("检测到进程未启动\n");
	sprintf(order, "rm %s", tmp_file);
	system(order);
	return -1;
...
int check_and_close_process(char * process_name)
{
	...
	//close(stream);
	//process_manage_out("buf = %s\n\n\n",buff);
	FILE* fp  = fopen(tmp_file,"r");  
	if(fp == NULL)  
	{	process_manage_out("文件打开失败\n");
		memset(order, 0, sizeof(order));
		sprintf(order, "rm %s", tmp_file);
		system(order);
		sync();
	...
	//process_manage_out("配置文件总大小为：%d字节\n",configLen);
	...
	i +=1;
	process_manage_out("读到第%d行的数据为：%s\n",i,lineBuff);
	pidnum = atoi(lineBuff);
	process_manage_out("[%s] process PID num = %d\n",process_name,pidnum);
	pID=(pid_t)pidnum;
