...
struct usbi_transfer {
	...
	int transferred;
	uint32 stream_id;
	uint8 state_flags;   /* Protected by usbi_transfer->lock */
	uint8 timeout_flags; /* Protected by the flying_stransfers_lock */
...
struct usbi_os_backend {
	...
	int (*get_config_descriptor)(struct libusb_device *device,
		uint8 config_index, unsigned char *buffer, size_t len,
		int *host_endian);
	...
	int (*get_config_descriptor_by_value)(struct libusb_device *device,
		uint8 bConfigurationValue, unsigned char **buffer,
		int *host_endian);
	...
	int (*alloc_streams)(struct libusb_device_handle *dev_handle,
		uint32 num_streams, unsigned char *endpoints, int num_endpoints);
