...
-   Other :
***********************************************************************/
int get_usbshare_status_new(char *server_ip, int server_port,char *intranet_ip ,char *external_ip, char *out_ter_info)
{
	SOCKET s_client;
	int result,i = 0;
	cJSON *root = NULL;
...
// void measure_latency( char *host, int port) 
// {
...
//     WriteLogFile("连接到 %s:%d 的延迟为 %f 秒\n", host, port, latency);

//     // 关闭套接字
//     close(sockfd);
// }

// int get_usbshare_net_delay_time(char *server_ip, int server_port,char *intranet_ip ,char *external_ip, char *out_ter_info)
// {
// 	SOCKET s_client;
// 	int result,i = 0;
// 	cJSON *root = NULL;
...
// 	WriteLogFile("_____zwl______dev_info get_usbshare_status_new server_ip=%s,server_port=%d", server_ip,server_port);
...
//
// int measure_latency(char *host, int port)
// {
//     WSADATA wsaData;
...
//     int iResult = getaddrinfo(host, "3240", &hints, &result);
//     if (iResult != 0) {
...
//         WriteLogFile("Error at socket(): %ld\n", WSAGetLastError());
//         freeaddrinfo(result);
//         WSACleanup();
//         return 1;
//     }
...
//     if (ConnectSocket == INVALID_SOCKET) {
//         WriteLogFile("Unable to connect to server!\n");
//         freeaddrinfo(result);
//         WSACleanup();
//         return 1;
//     }
...
//     // 发送和接收数据以测量延时
//     char sendbuf[512] = "Ping";
//     char recvbuf[512];
//     int recvbuflen = 512;
//     iResult = send(ConnectSocket, sendbuf, (int)strlen(sendbuf), 0);
//     if (iResult == SOCKET_ERROR) {
//         WriteLogFile("send failed with error: %d\n", WSAGetLastError());
//         closesocket(ConnectSocket);
//         WSACleanup();
//         return 1;
//     }
...
//     if (iResult > 0) {
//         WriteLogFile("Network latency: %d ms\n", GetTickCount()); // Just an example, not a precise measurement
//     } else if (iResult == 0) {
//         WriteLogFile("Connection closed\n");
//     } else {
//         WriteLogFile("recv failed with error: %d\n", WSAGetLastError());
//     }
...
//int get_usbshare_net_delay_time_test(char *server_ip, int server_port,char *intranet_ip ,char *external_ip, char *out_ter_info)
...
// //
// int measure_latency_test(char *host, int port)
// {
...
int get_upgrade_net_intranet(char *server_ip, int server_port,char *intranet_ip ,char *external_ip, char *out_ter_info, char *ter_id, int notice_type)
{
	SOCKET s_client;
	int result,i = 0;
	cJSON *root = NULL;
