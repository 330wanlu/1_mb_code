...
static ssize_t usbip_xmit(int sockfd, void *buff, size_t bufflen, int sending)
{
	...
	if (!bufflen)
		return 0;
	dbg_file("do %d: len:%d\n", sending, bufflen);
	WriteLogFile("--------------------usbip_xmit-----------------\r\n");

	do {
		WriteLogFile("--------------------%d\r\n",i);
		if (sending)
		{
			WriteLogFile("--------------------sending\r\n");
		...
		if (nbytes <= 0)
		{
			...
			ret = GetLastError();
			WriteLogFile("return -1!  ret = %d",ret);
			return -1;
		}
...
SOCKET tcp_connect(char *hostname, char *service)
{
	...
	WriteLogFile("usbipDLL socket connect ip :%s  port:%d", hostname, atoi(service));
    ...
    // sprintf(tmp_server,"%d",service);
	memcpy(tmp_server,service,strlen(service));
	/* get all possible addresses */
	WriteLogFile("hostname=%s,tmp_server=%s",hostname,tmp_server);
	ret = getaddrinfo(hostname, tmp_server, &hints, &res0);
	if (ret) {
		WriteLogFile("getaddrinfo error!");
	...
	for (res = res0; res; res = res->ai_next) {
		...
		// iMode = 1;
		// ioctlsocket(clifd, FIONBIO, (u_long FAR*)&iMode); //����Ϊ������ģʽ
	...
	WriteLogFile("no destination to connect to");
	return INVALID_SOCKET;
}
