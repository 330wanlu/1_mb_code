...
int usbip_vbus_detach_device(HANDLE fd, int port)
{
	...
	ret = DeviceIoControl(fd, IOCTL_USBVBUS_UNPLUG_HARDWARE,
				&unplug, sizeof(unplug), out, 1024, &unused, NULL);
	WriteLogFile("111 usbip_vbus_detach_device,ret = %d,unplug.addr = %d",ret,unplug.addr);

	// unplug.addr = port;
	// ret = DeviceIoControl(fd, IOCTL_USBVBUS_UNPLUG_HARDWARE,
	// 			&unplug, sizeof(unplug), out, 1024, &unused, NULL);
	// WriteLogFile("222 usbip_vbus_detach_device,ret = %d,port = %d",ret,unplug.addr);
	if(ret)
		return 0;
	return -1;
}
