 ...
 BOOL terminal_setting_init(void)
{
	...
	*(strrchr(set_path,'\\')+1) = 0;
	...
	c_DestPath = strcat(set_path, "ModelApiDll\\Client.set");
	...
	if (access(c_DestPath , 0)){
		...
		for (i = 1; i <= all_terminal_setting_table.count; i++){
			...
			WriteLogFile("usbipDLL read ip :%s --port:%d--dataport:%d --remoteip:%s--remote_port:%d--remote_dataport:%d\n", all_terminal_setting_table.terminal_setting[i - 1].terminal_ip, \
