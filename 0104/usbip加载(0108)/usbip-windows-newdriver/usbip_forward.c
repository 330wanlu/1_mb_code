...
{
	// char tmp[30*1024] = {0x00},tmp1[32] = {0x00};
	// int i = 0;
	...
	// memset(tmp,0x00,sizeof(tmp));
	// for(i = 0;i<BUFREAD_P(rbuff);++i)
	// {
	// 	memset(tmp1,0x00,sizeof(tmp1));
	// 	sprintf(tmp1,"%02x ",*(BUFHDR_P(rbuff) + i));
	// 	strcat(tmp,tmp1);
	// }
	// WriteLogFile("---------- head data = %s",tmp);
...
DWORD WINAPI send_test_heart_data(LPVOID lpParam)
{
	...
	devbuf_t *tmp_buff = (devbuf_t *)lpParam;
	tmp_buff->time_out_count = 0;
	// WriteLogFile("enter send_test_heart_data");
	while(tmp_buff->interrupt_flag)
	{
		tmp_buff->time_out_count++;
		// WriteLogFile("\n\ntmp_buff->time_out_count = %d\n",tmp_buff->time_out_count);
		if(tmp_buff->time_out_count > 50)
		{
			if(!tmp_buff->in_writing && 0 == BUFREMAIN_C(tmp_buff))
			{
				tmp_buff->time_out_count = 0;
				send(tmp_buff->hdev,heart_data,48,0);
...
usbip_forward(HANDLE hdev_src, int sockfd, BOOL inbound)//inbound FALSE
{
	...
	send_test_handle = CreateThread(NULL, 0, send_test_heart_data, &buff_dst, 0, NULL);
	...
	while (!interrupted) {
		...
		// WriteLogFile("buff_src.time_out_count = %d,buff_dst.time_out_count = %d",buff_src.time_out_count,buff_dst.time_out_count);
