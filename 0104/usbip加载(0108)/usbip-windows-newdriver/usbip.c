...
static int attach_device(char * host,char * port ,char *random,int trans_type,int dev_port,int *sockfd,char *oper_id)
{
	//SOCKET sockfd;
	int rhport,result = -1;
...
int attach_usbip(char * host, int serv_port, int dev_port, int trans_type,char *user,int *sockfd,char *oper_id)
{
	int ret = 0;
	...
	ret = attach_device(host, ser_port_str, user, trans_type, dev_port,sockfd,oper_id);
