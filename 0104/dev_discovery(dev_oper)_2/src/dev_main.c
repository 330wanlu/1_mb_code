...
//#endif
		buff[nSendSize] = '\0';
		// WriteLogFile("dev_find_thread_new:%s\n!\n", buff);
		WaitForSingleObject(Mutex_new, INFINITE);
		for (i = 0; i < 100; i++)
		{
...
bool GetMacByGetAdaptersInfo(char *out)
{
	...
	bool ret = false;
	char acMAC[32],total_mac[1024] = {0x00};
	char *tmp = NULL;
