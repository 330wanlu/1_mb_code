...
//	int result;
	...
	if (root == NULL)
	{
		WriteLogFile("parse_json_result 111!\n");
		return -1;
	}
	...
	if (head == NULL)
	{
		WriteLogFile("parse_json_result 222!\n");
		return DLL_ERR_NEW_SHARE_CMD_OPER;
	}
	item = cJSON_GetObjectItem(head, "result");
	if (item == NULL)
	{
		WriteLogFile("parse_json_result 333!\n");
		return DLL_ERR_NEW_SHARE_CMD_OPER;
	}
	if (memcmp(item->valuestring, "s", 1) == 0)
	{
		WriteLogFile("parse_json_result 444!\n");
		return 0;
	}
	else
	{
		if(memcmp(item->valuestring, "f01", 3) == 0)
		{
			WriteLogFile("parse_json_result 444!\n");
			return DLL_ERR_NEW_SHARE_SET_IP_FAILED;
		}
...
re_read:
	...
	WriteLogFile("dev_info 接收到的完整包为:%s!", recvbody);
	cJSON_Delete(rt);
	free(noformat);
	free(recvbody);
...
re_read:
	...
	if (result_2 < 0)
	{
		...
		WriteLogFile("dev_info set_usbshare_ip_new recv head error!:result_2 :%d ip:%s port:%d last_err:%d\n", result_2, server_ip, server_port, GetLastError());
		Sleep(1000);
		re_read++;
		if (re_read == 30)
		{
