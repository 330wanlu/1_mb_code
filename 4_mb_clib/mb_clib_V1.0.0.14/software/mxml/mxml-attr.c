                   ...
                   const char *name)  /* I - Name of attribute */
{
    ...
    for (i = node->value.element.num_attrs, attr = node->value.element.attrs; i > 0; i--, attr++) {
        ...
        if (!strcmp(attr->name, name)) {
#ifdef DEBUG
            _WriteLog(LL_DEBUG, "    Returning \"%s\"!\n", attr->value);
#endif /* DEBUG */
            return (attr->value);
        }
    }
...
mxmlElementGetAttrCount(mxml_node_t *node) /* I - Node */
{
                         ...
                         ...)                /* I - Additional arguments as needed */
{
...
#ifdef DEBUG
    fprintf(stderr, "mxmlElementSetAttrf(node=%p, name=\"%s\", format=\"%s\", ...)\n", node,
            name ? name : "(null)", format ? format : "(null)");
#endif /* DEBUG */
    ...
    if (!value)
        mxml_error("Unable to allocate memory for attribute '%s' in element %s!", name,
