...
#ifdef DEBUG
    _WriteLog(LL_DEBUG, "mxmlIndexFind(ind=%p, element=\"%s\", value=\"%s\")\n", ind,
              element ? element : "(null)", value ? value : "(null)");
#endif /* DEBUG */
...
#ifdef DEBUG
        _WriteLog(LL_DEBUG, "    find first time, num_nodes=%d...\n", ind->num_nodes);
#endif /* DEBUG */
...
#ifdef DEBUG
            _WriteLog(LL_DEBUG, "    first=%d, last=%d, current=%d\n", first, last, current);
#endif /* DEBUG */
...
#ifdef DEBUG
                _WriteLog(LL_DEBUG, "    returning first match=%d\n", current);
#endif /* DEBUG */
...
#ifdef DEBUG
                _WriteLog(LL_DEBUG, "    returning only match %d...\n", current);
#endif /* DEBUG */
...
#ifdef DEBUG
        _WriteLog(LL_DEBUG, "    returning next match %d...\n", ind->cur_node);
#endif /* DEBUG */
             ...
             const char *attr)    /* I - Attribute to index or @code NULL@ for none */
{
...
#ifdef DEBUG
    _WriteLog(LL_DEBUG, "mxmlIndexNew(node=%p, element=\"%s\", attr=\"%s\")\n", node,
              element ? element : "(null)", attr ? attr : "(null)");
#endif /* DEBUG */
    ...
    if ((ind = calloc(1, sizeof(mxml_index_t))) == NULL) {
        mxml_error("Unable to allocate %d bytes for index - %s", sizeof(mxml_index_t),
                   strerror(errno));
        return (NULL);
    }
        ...
        if (ind->num_nodes >= ind->alloc_nodes) {
            ...
            if (!temp) {
                ...
                mxml_error("Unable to allocate %d bytes for index: %s",
                           (ind->alloc_nodes + 64) * sizeof(mxml_node_t *), strerror(errno));
...
#ifdef DEBUG
    {
        ...
        _WriteLog(LL_DEBUG, "%d node(s) in index.\n\n", ind->num_nodes);

        if (attr) {
            _WriteLog(LL_DEBUG, "Node      Address   Element         %s\n", attr);
            puts("--------  --------  --------------  ------------------------------");

            for (i = 0; i < ind->num_nodes; i++)
                _WriteLog(LL_DEBUG, "%8d  %-8p  %-14.14s  %s\n", i, ind->nodes[i],
            ...
            for (i = 0; i < ind->num_nodes; i++)
                _WriteLog(LL_DEBUG, "%8d  %-8p  %s\n", i, ind->nodes[i], ind->nodes[i]->value.element.name);
        }
...
{
#ifdef DEBUG
    _WriteLog(LL_DEBUG, "mxmlIndexReset(ind=%p)\n", ind);
#endif /* DEBUG */
                       ...
                       int right)         /* I - Right node in partition */
{
    ...
    do {
        ...
        for (templ = left, tempr = right; templ < tempr;) {
            ...
            while ((templ < right) && index_compare(ind, ind->nodes[templ], pivot) <= 0) templ++;

            /*
             * Move right while right node > pivot node...
             */

            while ((tempr > left) && index_compare(ind, ind->nodes[tempr], pivot) > 0) tempr--;
