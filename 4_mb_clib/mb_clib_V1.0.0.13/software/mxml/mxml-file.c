...
static mxml_node_t *mxml_load_data(mxml_node_t *top, void *p, mxml_load_cb_t cb,
                                   _mxml_getc_cb_t getc_cb, mxml_sax_cb_t sax_cb, void *sax_data);
...
static int mxml_write_node(mxml_node_t *node, void *p, mxml_save_cb_t cb, int col,
                           _mxml_putc_cb_t putc_cb, _mxml_global_t *global);
static int mxml_write_string(const char *s, void *p, _mxml_putc_cb_t putc_cb);
static int mxml_write_ws(mxml_node_t *node, void *p, mxml_save_cb_t cb, int ws, int col,
             ...
             int *encoding) /* IO - Encoding */
{
    ...
    switch (*encoding) {
        case ENCODE_UTF8:
            ...
            if (!(ch & 0x80)) {
#if DEBUG > 1
                _WriteLog(LL_DEBUG, "mxml_fd_getc: %c (0x%04x)\n", ch < ' ' ? '.' : ch, ch);
#endif /* DEBUG > 1 */
...
#if DEBUG > 1
                _WriteLog(LL_DEBUG, "mxml_file_getc: %c (0x%04x)\n", ch < ' ' ? '.' : ch, ch);
#endif /* DEBUG > 1 */
                ...
                int *line) /* IO - Current line number */
{
    ...
    while ((ch = (*getc_cb)(p, encoding)) != EOF) {
        if (ch > 126 || (!isalnum(ch) && ch != '#'))
            ...
            *entptr++ = ch;
            ...
            mxml_error("Entity name too long under parent <%s> on line %d.",
                       parent ? parent->value.element.name : "null", *line);
            break;
        }
    }
    ...
    *entptr = '\0';
    ...
    if (ch != ';') {
        mxml_error("Character entity '%s' not terminated under parent <%s> on line %d.", entity,
                   parent ? parent->value.element.name : "null", *line);
    ...
    if (entity[0] == '#') {
        ...
        mxml_error("Entity name '%s;' not supported under parent <%s> on line %d.", entity,
                   parent ? parent->value.element.name : "null", *line);

    if (mxml_bad_char(ch)) {
        mxml_error("Bad control character 0x%02x under parent <%s> on line %d not allowed by XML "
                   "standard.",
                   ch, parent ? parent->value.element.name : "null", *line);
        return (EOF);
    }
               ...
               void *sax_data)          /* I - SAX user data */
{
    ...
    while ((ch = (*getc_cb)(p, &encoding)) != EOF) {
        if ((ch == '<' || (mxml_isspace(ch) && type != MXML_OPAQUE && type != MXML_CUSTOM)) &&
            bufptr > buffer) {
            /*
            ...
            *bufptr = '\0';
                ...
                case MXML_CUSTOM:
                    if (global->custom_load_cb) {
                        ...
                        if ((*global->custom_load_cb)(node, buffer)) {
                            mxml_error("Bad custom value '%s' in parent <%s> on line %d.", buffer,
                                       parent ? parent->value.element.name : "null", line);
                            mxmlDelete(node);
                            node = NULL;
                        }
            ...
            if (*bufptr) {
                ...
                mxml_error("Bad %s value '%s' in parent <%s> on line %d.",
            ...
            if (!node && type != MXML_IGNORE) {
                ...
                mxml_error("Unable to add value node of type %s to parent <%s> on line %d.",
                           types[type], parent ? parent->value.element.name : "null", line);
                goto error;
            }
        ...
        if (ch == '<') {
            ...
            *bufptr = '\0';
            ...
            if (!strcmp(buffer, "!--")) {
                ...
                while ((ch = (*getc_cb)(p, &encoding)) != EOF) {
                    if (ch == '>' && bufptr > (buffer + 4) && bufptr[-3] != '-' &&
                        bufptr[-2] == '-' && bufptr[-1] == '-')
                        break;
                    else if (mxml_add_char(ch, &bufptr, &buffer, &bufsize))
                        goto error;
                ...
                *bufptr = '\0';
                ...
                if (!parent && first) {
                    ...
                    mxml_error("<%s> cannot be a second root node after <%s> on line %d.", buffer,
                               first->value.element.name, line);
                    goto error;
                }
                ...
                if ((node = mxmlNewElement(parent, buffer)) == NULL) {
                    ...
                    mxml_error("Unable to add comment node to parent <%s> on line %d.",
                               parent ? parent->value.element.name : "null", line);
                    break;
                }
                ...
                *bufptr = '\0';
                ...
                if ((node = mxmlNewElement(parent, buffer)) == NULL) {
                    ...
                    mxml_error("Unable to add CDATA node to parent <%s> on line %d.",
                               parent ? parent->value.element.name : "null", line);
                    goto error;
                }
                ...
                *bufptr = '\0';
                ...
                if ((node = mxmlNewElement(parent, buffer)) == NULL) {
                    ...
                    mxml_error(
                        "Unable to add processing instruction node to parent <%s> on line %d.",
                        parent ? parent->value.element.name : "null", line);
                    goto error;
                }
                ...
                *bufptr = '\0';
                ...
                if ((node = mxmlNewElement(parent, buffer)) == NULL) {
                    ...
                    mxml_error("Unable to add declaration node to parent <%s> on line %d.",
                               parent ? parent->value.element.name : "null", line);
                    goto error;
                }
                ...
                if (!parent || strcmp(buffer + 1, parent->value.element.name)) {
                    ...
                    mxml_error("Mismatched close tag <%s> under parent <%s> on line %d.", buffer,
                               parent ? parent->value.element.name : "(null)", line);
                    goto error;
                }
                ...
                while (ch != '>' && ch != EOF) ch = (*getc_cb)(p, &encoding);
                ...
                if ((node = mxmlNewElement(parent, buffer)) == NULL) {
                    ...
                    mxml_error("Unable to add element node to parent <%s> on line %d.",
                               parent ? parent->value.element.name : "null", line);
                    goto error;
                }
                ...
                if (mxml_isspace(ch)) {
                    ...
                    if ((ch = (*getc_cb)(p, &encoding)) != '>') {
                        mxml_error("Expected > but got '%c' instead for element <%s/> on line %d.",
                                   ch, buffer, line);
                        mxmlDelete(node);
                        goto error;
                    }
    ...
    if (parent) {
        ...
        while (parent != top && parent->parent) parent = parent->parent;

        if (node != parent) {
            mxml_error("Missing close tag </%s> under parent <%s> on line %d.",
                       node->value.element.name,
                       node->parent ? node->parent->value.element.name : "(null)", line);
                   ...
                   int *line)               /* IO - Current line number */
{
    ...
    while ((ch = (*getc_cb)(p, encoding)) != EOF) {
        ...
        if (ch == '/' || ch == '?') {
            ...
            if (quote != '>') {
                mxml_error("Expected '>' after '%c' for element %s, but got '%c' on line %d.", ch,
                           node->value.element.name, quote, *line);
                goto error;
            }
        ...
        *ptr = '\0';
        ...
        if (mxmlElementGetAttr(node, name)) {
            mxml_error("Duplicate attribute '%s' in element %s on line %d.", name,
                       node->value.element.name, name, *line);
            goto error;
        }
        ...
        if (ch == '=') {
            ...
            if (ch == EOF) {
                mxml_error("Missing value for attribute '%s' in element %s on line %d.", name,
                           node->value.element.name, *line);
                goto error;
            }
    ...
    if ((ch = (*s)[0] & 255) != 0 || *encoding == ENCODE_UTF16LE) {
        ...
        switch (*encoding) {
            case ENCODE_UTF8:
                if (!(ch & 0x80)) {
#if DEBUG > 1
                    _WriteLog(LL_DEBUG, "mxml_string_getc: %c (0x%04x)\n", ch < ' ' ? '.' : ch, ch);
#endif /* DEBUG > 1 */
                    ...
                    *encoding = ENCODE_UTF16LE;
                    ...
                    ch =
                        ((((((ch & 0x07) << 6) | ((*s)[0] & 0x3f)) << 6) | ((*s)[1] & 0x3f)) << 6) |
                        ((*s)[2] & 0x3f);
...
mxml_write_ws(mxml_node_t *node,       /* I - Current node */
              ...
              _mxml_putc_cb_t putc_cb) /* I - Write callback */
{
    char *pRet = NULL; /* Whitespace string */
    if (cb && (pRet = (char *)(*cb)(node, ws)) != NULL) {
        char *s = pRet;
        while (*s) {
            if ((*putc_cb)(*s, p) < 0)
                return (-1);
